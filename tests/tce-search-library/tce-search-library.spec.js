...
test.describe('Core (ADD-LIB-01..07)', () => {
  ...
  test(
    'ADD-LIB-01: Library opens with the search box pre-filled from the current Topic',
    { tag: '@positive' },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      const pl = new PlaylistPage(page);
  ...
  test(
    'ADD-LIB-02: The pre-filled search returns results related to the current Topic',
    { tag: '@positive' },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
  ...
  test(
    'ADD-LIB-03: Clear empties the search box but leaves prior results displayed until a new search runs',
    { tag: '@ui-state' },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
  ...
  test(
    'ADD-LIB-04: Searching a term with no matches shows an explicit "No result found" message',
    { tag: '@positive' },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
  ...
  test(
    "ADD-LIB-06: Selecting a Library search result attaches it to the current Topic's playlist",
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
  ...
  test(
    'ADD-LIB-07: A virtual QWERTY keyboard opens when the search box is focused',
    { tag: '@ui-state' },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
...
test.describe('Extended coverage (gap-analysis pass, 29-row workbook)', () => {
  ...
  function previewButtons(page) {
    return {
      ...
      close: page
        .locator('[data-qa-id^="tce-library-"][data-qa-id*="-close-btn"]:not([data-qa-id="tce-library-close-btn"])')
        .first(),
    };
  }
  ...
  test(
    'TCE-PREV-01: Selecting a search result opens a preview dialog with title, media preview, and action buttons',
    { tag: ['@ui-state', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
  ...
  test(
    'TCE-PREV-02: "Open in Whiteboard" from a preview dialog places the content directly on the board',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      const opened = await searchAndOpenFirstResult(page, ar, 'Worksheet');
  ...
  test(
    'TCE-PREV-03: "Add to Playlist" from a preview dialog attaches the content to the current topic (reachability only -- not executed)',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        !addPlaylistVisible,
        "Add to Playlist button not reachable on this preview dialog this pass -- the actual click is separately deliberately not executed to avoid growing the shared QA account's real Playlist strip, same reasoning as ADD-LIB-06, but reachability itself should still hold"
      );
  ...
  test(
    'TCE-PREV-04: Close (X) on a preview dialog exits cleanly without side effects',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
  ...
  test(
    'TCE-BOUND-01: A single- or two-character search query is handled gracefully -- Search stays disabled, not an error',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.librarySearchInput.fill('');
  ...
  test(
    'TCE-BOUND-02: Special/script-like characters in a search query are safely escaped, not executed',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      ...
      await ar.librarySearchBtn.click();
      await page.waitForTimeout(1200);
      const noResultText = await page
        .getByText(/no result found/i)
        .isVisible()
        .catch(() => false);
      const literalPayloadShown = await page
  ...
  test(
    "TCE-SEC-01: Search results are scoped to the signed-in teacher's own school/curriculum access (blocked -- needs a second school account)",
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Confirming school/curriculum scoping needs a second school/tenant account to compare search result sets against -- only this project's one school (Goyal Brothers) is available, same blocker class as GAL-SEC-01/TB-EXP-15/17 elsewhere in this suite"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'TCE-XCUT-01: Closing content opened via "Open in Whiteboard" also triggers the known Playlist-vanishing bug (cross-ref: PL-BUG-01)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      ...
      if (!openWbVisible) {
        expect(openWbVisible).toBe(true);
        return;
      }
      await btns.openWhiteboard.click({ force: true });
      await page.waitForTimeout(1500);
      ...
      const playlistTriggerVisible = await page
  ...
  test(
    "LIB-TYPE-MIX-01: A single search's results are not guaranteed to be one uniform content type",
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
  ...
  test(
    "LIB-AUTOSEARCH-RACE-01: A manual search issued immediately on open is not silently overwritten by Library's own background auto-search",
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      ...
      const noResultTextShown = await page
        ...
        .isVisible()
        .catch(() => false);
      const genericNoResultShown = await page
        .getByText(/no result found/i)
        .isVisible()
        .catch(() => false);
      console.log(
  ...
  test(
    'LIB-WORKSHEET-POOL-01: The "Worksheet" search term\'s result pool is finite and non-paginated (~22 results)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
  ...
  test(
    "LIB-STUCK-PREVIEW-01: A silent-failure attach leaving its preview stuck open (not independently reproduced -- depends on LIB-WORKSHEET-POOL-01's un-executed attach step)",
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Reproducing this requires deliberately triggering the specific silent-failure attach documented in LIB-WORKSHEET-POOL-01, which this pass does not execute (would mean attaching real resources to the shared QA account's Playlist to find the one broken result) -- not independently reproduced this pass"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'LIB-PREVTYPE-GAP-01: Attempting to find search terms surfacing additional preview-dialog content types (Image, TCE, Weblink, Code)',
    { tag: ['@ui-state', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      ...
      test.fail(
        !anyResults,
        `None of the candidate terms (${candidateTerms.join(', ')}) returned any results this pass -- could not narrow the gap on which of the 5 unconfirmed preview types (Image, Video, TCE, Weblink, Code) has a working search term`
      );
  ...
  test(
    'LIB-FOOTERBTN-01: Preview dialog footer-action button IDs are type-prefixed (confirmed pattern: tce-library-pdf-*)',
    { tag: ['@ui-state', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      const opened = await searchAndOpenFirstResult(page, ar, 'Worksheet');
      ...
      const pdfOpenWb = await page
        .locator('[data-qa-id="tce-library-pdf-open-whiteboard-btn"]')
        .isVisible({ timeout: 5000 })
        .catch(() => false);
      const pdfAddPlaylist = await page
        .locator('[data-qa-id="tce-library-pdf-add-playlist-btn"]')
        .isVisible()
        .catch(() => false);
      const pdfClose = await page
  ...
  test(
    'LIB-BOUND-03: Rapidly double-clicking "Add to playlist" (reachability only -- not executed, see TCE-PREV-03)',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        !addPlaylistVisible,
        'Add to Playlist button not reachable on this preview dialog this pass -- a real double-click is separately deliberately not executed (see TCE-PREV-03), but reachability itself should still hold'
      );
      expect(addPlaylistVisible).toBe(true);
    }
  );
  ...
  test(
    "LIB-XCUT-02: Existing Playlist resource cards expose a standard Remove/Edit overflow menu (positive contrast to Gallery's no-asset-record behavior)",
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      ...
      test.fail(
        count === 0,
        'No existing Playlist resource cards found on this account/topic to check the overflow menu against'
      );
      if (count === 0) {
        expect(count).toBeGreaterThan(0);
        return;
      }
      const overflowVisible = await pl.assetOverflowIconBtn
        .first()
  ...
  test(
    'TCE-EXP-01: A Library search request returning a 5xx error shows a distinguishable error state, not a silent zero-match "no results"',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await page.route('**/*search*', (route) => {
        if (route.request().method() === 'GET' || route.request().method() === 'POST') {
          route.fulfill({
            ...
            body: JSON.stringify({ error: 'forced 500' }),
          });
        } else {
          route.continue();
        }
      ...
      await ar.librarySearchBtn.click();
      await page.waitForTimeout(1500);
      const genericNoResults = await page
        .getByText(/no result found/i)
        .isVisible()
        .catch(() => false);
      const distinctErrorShown = await page
  ...
  test(
    'TCE-EXP-02: Add to Playlist on a since-removed resource fails clearly (hard-to-force race, not reliably reproducible)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "A genuinely hard-to-force race condition (resource removed server-side between preview-open and Add click) -- flagged as a design consideration rather than a reliably reproducible live test, per this workbook's own note"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'TCE-EXP-03: A purely numeric/decimal search query (e.g. a chapter number like "2.4") is treated as normal text search',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      ...
      await ar.librarySearchBtn.click();
      await page.waitForTimeout(1200);
      const errorOrCrash = await page
        .getByText(/invalid|error/i)
        .isVisible()
        .catch(() => false);
      const resultCount = await ar.libraryResults.count();
      const noResultShown = await page
        .getByText(/no result found/i)
        .isVisible()
        .catch(() => false);
      console.log(
  ...
  test(
    'TCE-EXP-04: Opening a second preview while one is already open does not stack overlapping dialogs',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.librarySearchInput.fill('Worksheet');
      ...
      test.fail(
        count < 2,
        'Fewer than 2 "Worksheet" results this pass -- cannot test opening a second preview over a first'
      );
      if (count < 2) {
        expect(count).toBeGreaterThanOrEqual(2);
        return;
  ...
  test(
    'TCE-EXP-05: A search returning many matches (e.g. a common word) remains correctly paginated with no duplicate/missing results',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      ...
      const ids = await ar.libraryResults.evaluateAll((els) =>
  ...
  test(
    "TCE-EXP-06: A crafted search term cannot surface a different school's private curriculum content (blocked -- needs a second school account)",
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        'Extends TCE-SEC-01 with a targeted/crafted-search-term angle -- still blocked on the same second-school/tenant-account gap, none available this pass'
      );
      expect(true).toBe(false);
    }
  );
