...
test(
  'TCE-BREAK-01: a 500-character search query does not crash or hang the Library results panel',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
...
test(
  "TCE-BREAK-02: typing a real query character-by-character with minimal delay settles on the FULL query's results, not a stale partial-substring response",
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
...
test(
  'TCE-BREAK-03: rapidly filling then clearing then refilling the search box 4 times in a row leaves the UI in a consistent, non-stuck final state',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
