...
test(
  'AIA-TABS-01: Three content tabs (Exercise, Videos, Teaching Tips) are present and each loads distinct content',
  { tag: '@positive' },
  async ({ page }) => {
    ...
    const teachingContentVisible = await page
      .getByText(/activities|explanation|real life example/i)
      .first()
...
test(
  'AIA-TABS-02: BUG check -- switching tabs requires two clicks, the first only updates the selection dot',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await openAiAssist(page, ar);
    ...
    test.fail(
      bugReproduced,
      'CONFIRMED LIVE BUG (matches workbook): the first click on a tab updates only the selection state, not the content -- content only renders after a second click on the same tab'
    );
...
test(
  'AIA-EXERCISE-01: Checking a question reveals Add to Playlist; the chevron reveals the answer',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await openAiAssist(page, ar);
...
test(
  'AIA-EXERCISE-02: Header instructional text should reference the ACTUAL current topic, not a mismatched placeholder',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await openAiAssist(page, ar);
    ...
    const currentTopicLabel =
      (await page
        .locator('[data-qa-id="playlist-chapter-topic-btn"]')
    ...
    if (!headerText.trim()) {
      headerText =
        (await page
          .getByText(/create an exercise|add to the topic/i)
          .first()
          .textContent({ timeout: 3000 })
          .catch(() => '')) || '';
    }
    ...
    const topicWords = currentTopicLabel
      .replace(/[^a-zA-Z ]/g, ' ')
      .split(/\s+/)
      .filter((w) => w.length > 4);
    ...
    test.fail(
      ...
      "CONFIRMED LIVE (matches workbook): the Exercise tab's header instructional text does not reference the actual current topic -- looks like a stale/hardcoded placeholder string"
    );
...
test(
  'AIA-EXERCISE-03: Add to Playlist actually adds the generated exercise to the class playlist',
  { tag: '@positive' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    const ar = new AddResourcePage(page);
...
test(
  'AIA-VIDEOS-02: Playing a video from AI Assist -- confirm whether it visibly works or remains an inconclusive/broken hit-region',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await openAiAssist(page, ar);
    ...
    const stillShowingStaticThumb = await ar.aiAssistVideoThumbs
      .first()
      .isVisible()
      .catch(() => false);
    test.info().annotations.push({
...
test(
  'AIA-TEACH-01: Teaching Tips shows Activities / Explanation / Real Life Example sections',
  { tag: '@positive' },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await openAiAssist(page, ar);
    ...
    const activitiesVisible = await page
      .getByText(/activities/i)
      .first()
      .isVisible({ timeout: 5000 })
      .catch(() => false);
    const explanationVisible = await page
      .getByText(/explanation/i)
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
    const realLifeVisible = await page
      .getByText(/real life example/i)
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
    test.info().annotations.push({
...
test(
  'AIA-MINMAX-01: The window-control icon(s) produce a visible, non-broken repositioning effect',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await openAiAssist(page, ar);
    ...
    const titleChanged =
      boxBefore &&
      boxAfter &&
      (boxBefore.x !== boxAfter.x || boxBefore.y !== boxAfter.y || boxBefore.width !== boxAfter.width);
    const closeChanged =
      closeBoxBefore && closeBoxAfter && (closeBoxBefore.x !== closeBoxAfter.x || closeBoxBefore.y !== closeBoxAfter.y);
    const changed = titleChanged || closeChanged;
    test.fail(
      !changed,
      'The minimize/maximize control produced no detectable position/size change -- its intended effect remains unclear, matching the workbook\'s own "behavior unclear, needs follow-up" finding'
    );
...
test(
  'AIA-ERROR-01: A real HTTP-driven quota-exhaustion error (429/400/403/500) shows a clear, actionable message',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    ...
    await page.route('**/*', async (route) => {
      ...
      if (/ai[-_]?assist|ai[-_]?generat/i.test(url) && route.request().method() !== 'OPTIONS') {
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({ message: 'quota exceeded' }),
        });
      } else {
    ...
    const naturalErrorVisible = await page
      .getByText(/surpassed|allowable limits|quota/i)
    ...
    test.fail(
      !intercepted,
      'No AI-generation request matched the interception pattern -- could not force the 429 path this way, and the shared quota was not naturally exhausted this pass either'
    );
    expect(intercepted).toBe(true);
    if (intercepted && contentLoadedAnyway) {
      test.fail(
        true,
        'A forced 429 response did not stop real content from still rendering -- likely served from a client-side cache for this topic rather than the intercepted request, so the quota-exhaustion UI path was not actually exercised this pass'
      );
    ...
    if (intercepted) {
      test.fail(
        !errorScreenVisible && !naturalErrorVisible,
        'A forced 429 response did not produce any visible error state in the UI -- the panel may hang silently on a failed generation instead of showing a clear message'
      );
...
test(
  'AIA-VIDEOPOOL-01: Repeatedly adding the same video to the Playlist eventually stops growing the resource count (silent backend de-dup)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    const ar = new AddResourcePage(page);
...
test(
  'AIA-LOADSIGNAL-01: The loading spinner does not clear before tab content is actually fully populated',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    const { stillStuck } = await ar.openPickerReliably(ar.actions.aiAssist);
...
test(
  'AIA-FLAKE-01: This session\'s own AI Assist tests ran with real, non-"pending" outcomes (historical whole-file-flake risk noted, not reproduced)',
  { tag: '@cross-cutting' },
  async ({ page }) => {
...
test(
  'AIA-ADV-01: Rapid double-click on Add to Playlist does not create a duplicate exercise card',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await openAiAssist(page, ar);
...
test(
  'AIA-ADV-02: AI Assist opened on a topic with effectively zero curriculum content',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      !foundThinTopic,
      "None of the sampled class/subject combos this pass produced a zero-content AI Assist state -- every one tried had substantial real content, matching the workbook's own note that no specific zero-content topic has been identified yet"
    );
...
test(
  'AIA-ADV-03: Switching Class/Topic while AI Assist remains open',
  { tag: '@state-persistence' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    const ar = new AddResourcePage(page);
    await openAiAssist(page, ar);
    const topicABeforeSwitch =
      (await page
        .locator('[data-qa-id="playlist-chapter-topic-btn"]')
        .textContent()
        .catch(() => '')) || '';
...
test(
  'AIA-ADV-04: Rapid multi-tab-switch spam (Exercise/Videos/Teaching Tips/Exercise) does not break the content area',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await openAiAssist(page, ar);
    ...
    for (const tab of [
      ar.aiAssistTabExercise,
      ar.aiAssistTabVideos,
      ar.aiAssistTabTeachingTips,
      ar.aiAssistTabExercise,
    ]) {
      await tab.click({ force: true, timeout: 5000 }).catch(() => {
        anyClickTimedOut = true;
      });
    }
    await page.waitForTimeout(1500);

    const modalStillPresent = await page
      .getByText('AI Assist', { exact: true })
      .isVisible()
      .catch(() => false);
    const anyJsErrorDialog = await page
...
test(
  'AIA-ADV-05: Generated content does not leak between classes/subjects under quick back-to-back generation',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      identicalContent,
      "Class B's generated content is byte-identical to Class A's -- a possible cross-class caching/leak issue rather than freshly generated content"
    );
...
test(
  'AIA-EXP-01: A network failure during the initial content-generation fetch shows a clear error, not an indefinite spinner',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    let aborted = false;
    ...
    const errorShown =
      (await ar.aiAssistErrorScreen.isVisible().catch(() => false)) ||
      (await page
    ...
    if (aborted && contentLoadedAnyway) {
      ...
      test.fail(
        true,
        'Aborting the AI-generation request did not stop real content from still rendering -- either a client-side cache for this topic, a duplicate request the interception pattern missed, or genuine fetch-retry resilience; not the plain "network failure -> error message" scenario this case originally targeted'
      );
      expect(contentLoadedAnyway).toBe(false);
      return;
    }
    if (aborted) {
      test.fail(
        stillSpinnerOnly && !errorShown,
        'A failed content-generation fetch left the panel on an indefinite spinner/blank state with no clear error message'
      );
...
test(
  'AIA-EXP-02: The Teaching Tips tab loads real content without error (not independently tested elsewhere in this suite until AIA-TEACH-01 above)',
  { tag: '@negative' },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await openAiAssist(page, ar);
...
test(
  'AIA-EXP-03: Reopening AI Assist on a different Chapter shows fresh, correctly-scoped content, not a stale cached response',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    ...
    if (chapterCount > 1) {
      ...
      if (topicCount > 0) {
        await pl._tryClick(pl.topicItems.first());
        switched = true;
      }
    }
    await page.waitForTimeout(1000);
    ...
    test.fail(
      !switched,
      'Only one chapter is reachable in this account/class context -- could not switch to a genuinely different chapter to compare content against'
    );
    ...
    test.fail(
      staleIdentical,
      "Chapter 2's content is byte-identical to Chapter 1's -- a possible stale-cache cross-chapter leak"
    );
...
test(
  'AIA-BREAK-01: Rapid open/close AI Assist 5x in immediate succession leaves exactly one clean modal instance',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
...
test(
  'AIA-BREAK-02: Clicking Add to Playlist 10x rapidly on the same exercise does not create 10 duplicate playlist entries',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.setTimeout(90000);
    const pl = new PlaylistPage(page);
...
test(
  'AIA-BREAK-03: Reloading mid-generation does not leave AI Assist permanently stuck on the next open',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.setTimeout(90000);
    const pl = new PlaylistPage(page);
...
test(
  'AIA-BREAK-04: Selecting ALL available exercise checkboxes and clicking Add to Playlist does not crash or silently no-op',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    await page.waitForTimeout(400);
    const countBefore = await pl.resourceCards.count();
    const clicked = await ar.aiAssistAddToPlaylistBtn
...
test(
  'AIA-BREAK-05: Three consecutive forced network failures do not progressively degrade the modal',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.setTimeout(90000);
    const ar = new AddResourcePage(page);
...
test(
  'AIA-BREAK-06: Instant-close x3 right after opening does not leave the 4th legitimate open permanently blank',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.setTimeout(90000);
    const ar = new AddResourcePage(page);
...
test(
  'AIA-BREAK-07: An exercise added to Playlist under Class A is NOT visible in a different Class B Playlist',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.setTimeout(90000);
    const pl = new PlaylistPage(page);
...
test(
  'AIA-BREAK-08: Rapidly interleaved tab/close/reopen cycles leave tab selection and displayed content mutually consistent',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    test.setTimeout(60000);
    const ar = new AddResourcePage(page);
    ...
    const checkboxesVisible = await ar.aiAssistExerciseCheckboxes
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
    const videoThumbsVisible = await ar.aiAssistVideoThumbs
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
    test.info().annotations.push({
    ...
    test.fail(
      ...
      'CONFIRMED: after rapid interleaved tab/close/reopen spam, BOTH Exercise checkboxes and Video thumbnails render simultaneously -- tab content did not get torn down on tab switch, a real state-bleed bug'
    );
...
test(
  'AIA-BREAK-09: Rapidly pressing Escape 10x while AI Assist is open does not corrupt the DOM into a half-torn-down state',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.setTimeout(60000);
    const ar = new AddResourcePage(page);
    ...
    const modalStillThere = await page
      .getByText('AI Assist', { exact: true })
      .isVisible({ timeout: 2000 })
      .catch(() => false);
