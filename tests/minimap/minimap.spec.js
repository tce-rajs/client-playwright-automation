...
test(
  'MM-ACCESS-01: Minimap entry point (Zoom tool -> submenu -> Minimap toggle) opens the panel with the expected controls',
  { tag: '@positive' },
  async ({ page }) => {
...
test(
  'MM-DOM-01: The Minimap container is always present in the DOM; open/closed is the .visible class, not element presence',
  { tag: '@ui-state' },
  async ({ page }) => {
    const mm = new MinimapPage(page);
...
test(
  'MM-TOGGLE-01: The Player-toggle icon produces no error on an empty canvas (no active Player to toggle)',
  { tag: '@positive' },
  async ({ page }) => {
    const mm = new MinimapPage(page);
    await mm.open();
...
test(
  'MM-RESET-01: Reset View is clickable and produces no error when already at default 100%',
  { tag: '@positive' },
  async ({ page }) => {
...
test(
  'MM-RESET-VERIFY-01: Reset View actually restores 100% zoom after deliberately zooming away',
  { tag: '@positive' },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const mm = new MinimapPage(page);
...
test(
  'MM-TOGGLE-BUG-01: ADVERSARIAL -- reopening Minimap after Reset via the same click sequence can silently revert the active tool',
  ...
  async ({ page }) => {
    ...
    test.fail(
      ...
      'CONFIRMED (matches workbook, cross-repo): the documented 3-way toggle interaction (Zoom tool / Zoom submenu / Minimap panel button) reverted the active tool from Pen back to the default Select as a side effect, with no error or explanation shown -- a genuine, unresolved app-behavior puzzle'
    );
...
test(
  'MM-CONTENT-01: Minimap accurately reflects the current viewport as the main canvas zoom changes',
  { tag: '@positive' },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const mm = new MinimapPage(page);
...
test(
  'MM-PAN-01: Clicking inside the Minimap pans the main canvas',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const mm = new MinimapPage(page);
    ...
    test.fail(
      !panned,
      'Clicking inside the Minimap did not visibly change its own canvas render -- either clicking inside the Minimap does not pan the main canvas at all, or the click landed on a point that produced no detectable movement'
    );
    expect(before).not.toBe(after);
...
test(
  'MM-NEG-01: Minimap responsiveness on an extremely large accumulated canvas',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Constructing a canvas with an extremely large amount of accumulated content is not practically reachable through normal UI-driven browser automation in a single pass -- would need either a long-lived, already-heavily-used shared canvas or direct data injection, neither available here'
    );
...
test(
  "MM-SEC-01: Minimap reflects only the CURRENTLY active class's content after a class switch, not stale content from before",
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      beforeSwitch === afterSwitch,
      'The Minimap rendered IDENTICAL content after switching to a different class -- possible stale-cache/no-refresh issue. Full cross-teacher scoping still needs a second account, not available here.'
    );
...
test(
  'MM-BOUND-01: Rapidly double-clicking Reset does not cause an error or visual glitch',
  { tag: '@boundary' },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const mm = new MinimapPage(page);
...
test(
  "MM-STATE-01: Minimap's open state does not persist across a hard refresh",
...
test(
  'MM-EXP-01: Panning beyond content bounds via the Minimap stays clean and Reset still recovers a normal view',
  { tag: '@negative' },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const mm = new MinimapPage(page);
...
test(
  'MM-EXP-02: The Minimap shows a clean thumbnail immediately after Clear Whiteboard, with no stale ghost content',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const mm = new MinimapPage(page);
    ...
    test.fail(
      !clearBtnVisible,
      'Could not locate a reachable Clear Whiteboard control this pass to set up the "just cleared" precondition'
    );
...
test(
  'MM-EXP-03: Rapid zoom in/out spam while Minimap is open does not desync it from the real canvas state',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const mm = new MinimapPage(page);
    ...
    test.fail(
      !minimapStillOpen,
      'Rapid zoom in/out spam while the Minimap is open closed/desynced it with no error shown -- same class of issue as the confirmed MM-TOGGLE-BUG-01 finding'
    );
...
test(
  'MM-EXP-04: Opening the Minimap on an empty whiteboard does not error or show a broken thumbnail',
  { tag: '@negative' },
  async ({ page }) => {
    const mm = new MinimapPage(page);
    await mm.open();
...
test(
  'MM-EXP-05: The Minimap viewport rectangle remains non-degenerate at minimum and maximum zoom',
  { tag: '@boundary' },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const mm = new MinimapPage(page);
...
test(
  'MM-EXP-06: The Minimap panel does not overlap the Playlist strip or Add Resources button at a mobile viewport',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    const mmBox = await page
      .locator('[data-qa-id="minimap-container"].visible, [data-qa-id="minimap-container"]')
      .first()
      .boundingBox()
      .catch(() => null);
    const addResBox = await page
...
test(
  "MM-EXP-07: The Minimap thumbnail updates cleanly to a new class's content with no crash during the transition",
  { tag: '@security' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    const mm = new MinimapPage(page);
