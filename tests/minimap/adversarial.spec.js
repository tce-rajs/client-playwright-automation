...
test(
  "MM-BREAK-01: switching Class while the Minimap panel is open does not leave it stuck visible over the new class's whiteboard",
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
...
test(
  'MM-BREAK-02: rapidly opening and closing the Minimap 8 times in immediate succession does not leave a stuck or duplicated panel',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      containerCount > 1,
      'Rapidly opening/closing the Minimap 8 times mounts more than one container instance'
    );
    expect(containerCount).toBeLessThanOrEqual(1);
    test.fail(
      finalOpen,
      'After an even number (8) of open/close cycles ending on close(), the Minimap is still marked visible'
    );
...
test(
  'MM-BREAK-03: pressing the browser Back button while the Minimap is open does not leave a stuck overlay behind',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const mm = new MinimapPage(page);
    const opened = await mm.open();
...
test(
  'MM-BREAK-04: rapidly clicking many different points inside the Minimap in quick succession (pan spam) keeps the canvas responsive, never desyncing the viewport rectangle off-canvas',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    const resetStillWorks = await mm.resetBtn
      .click({ timeout: 3000 })
      .then(() => true)
      .catch(() => false);
    test.info().annotations.push({
    ...
    test.fail(
      ...
      'Rapidly clicking 10 different points across the Minimap in quick succession desyncs/breaks the panel (canvas gone or Reset unclickable)'
    );
