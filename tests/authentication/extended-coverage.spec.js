...
test.describe('Extended coverage (gap-analysis pass)', () => {
  ...
  test(
    'SESS-08: CRITICAL -- the logged-in session may expire extremely quickly during active use (re-check)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
  ...
  test(
    'SESS-09: Re-entering the PIN after a session drop via the on-screen keypad (reachability re-check)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
      ...
      test.fail(
        repositioned,
        'PREVIOUSLY FLAGGED (recommend human confirmation): the Sign In modal/keypad visibly repositions itself, which can cause a click aimed at one digit to land elsewhere'
      );
  ...
  test(
    'MFA-01: MFA setup/verification flow (blocked -- no MFA enrollment path in this account)',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Same blocker already confirmed elsewhere in this suite (USR-LOGIN-03/04 in the User Profile module) -- no MFA enrollment option was found anywhere in this account's reachable Account/Profile tabs"
      );
  ...
  test(
    'MFA-02: MFA Verify login-time challenge (blocked -- same reason as MFA-01)',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        'Same blocker as MFA-01 -- no MFA is registered on this account, so there is no login-time verify challenge to test against'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'AUTH-XREF-01: Change Password / Change PIN from the Profile menu are covered under the User Profile module',
    { tag: '@cross-cutting' },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
      ...
      const drilldownVisible = await page
  ...
  test(
    'AUTH-CYP-01: The confirmed Sign-Out control chain (avatar -> profile menu -> Sign Out) clears the session fully',
    { tag: '@cross-cutting' },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
  ...
  test(
    'AUTH-CYP-02: Clearing the token/clientId localStorage keys simulates session loss gracefully',
  ...
  test(
    'AUTH-CYP-03: The inactivity timeout duration is not overridable via a URL/query param',
    { tag: '@cross-cutting' },
    async ({ page }) => {
  ...
  test(
    'AUTH-GAP-01: An inactivity session-timeout warning popup with a countdown appears, and "Stay Signed In" extends the session (opportunistic re-check)',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        'This "Are you still there?" countdown warning was originally encountered opportunistically during unrelated testing, not deterministically triggered -- reproducing it on demand needs a reliable idle-timing trigger not established in this pass'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'AUTH-GAP-02: 30-minute inactivity forces a full session expiry (blocked -- impractically slow for an automated pass)',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        'Waiting a real 30 idle minutes is impractical to run as part of this automated suite without materially slowing down the whole run -- not attempted this pass'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'AUTH-GAP-03: A rapid double-tap on the 5th PIN digit box fires exactly one login request via auto-submit',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
  ...
  test(
    "AUTH-GAP-04: Toggling PIN <-> Password view mid-entry does not corrupt either form's state",
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
      ...
      const box0Value = await login
        ...
        .inputValue()
        .catch(() => null);
      const box1Value = await login
        .pinDigitBox(1)
        .inputValue()
        .catch(() => null);
      const invalidStateShown = await login
        .pinDigitFormField(0)
  ...
  test(
    'AUTH-GAP-05: Copying the auth localStorage keys into a second, unauthenticated browser context',
    { tag: ['@security', '@bug'] },
    async ({ page, browser }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
      ...
      test.fail(
        !authValues.token,
        'No "token" localStorage key found on this account/session this pass -- cannot construct the hijack scenario against a confirmed key name'
      );
      if (!authValues.token) {
        expect(authValues.token).not.toBeNull();
        return;
      }
      ...
      const hijackSucceeded = await page2
  ...
  test(
    'AUTH-EXP-06: Account lockout cannot be bypassed by spoofing client headers between failed attempts (blocked -- no header-manipulation tooling)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        'Needs request-header manipulation tooling to vary User-Agent/client-ID headers across repeated failed attempts while confirming server-side lockout tracking -- not available in this environment, and deliberately not attempted live to avoid risking real account lockout (same caution as SEC-01/SEC-02 elsewhere in this suite)'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'AUTH-EXP-07: No alternate input path lets a non-numeric injection-style string reach the PIN field',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
  ...
  test(
    "AUTH-EXP-08: Signing in as a second, different account in a new tab does not corrupt the first tab's active session",
    { tag: ['@negative', '@bug'] },
    async ({ page, context }) => {
      const login = new LoginPage(page);
      ...
      await page.waitForTimeout(1000);
      const tab1StillAuthenticated = await page
        .locator('[data-qa-id="toolbar-user-avatar"]')
        .isVisible({ timeout: 5000 })
        .catch(() => false);
      const classTextTab1After =
        (await page
          .locator('[data-qa-id="playlist-current-grade-subject-btn"]')
          .textContent()
          .catch(() => '')) || '';
  ...
  test(
    'AUTH-EXP-09: A malformed/oversized PIN submission is rejected cleanly, not just relied on client-side box-count limiting',
    { tag: '@negative' },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
  ...
  test(
    'AUTH-EXP-10: A replayed old token cannot re-authenticate after Sign Out (blocked -- no token-capture/replay tooling)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        'Needs token-capture and raw HTTP request-replay tooling beyond this browser-automation environment to directly verify server-side token invalidation on Sign Out -- not attempted this pass'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'AUTH-EXP-11: Entering the correct PIN digits in a shuffled order does not accidentally succeed',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
      ...
      test.fail(
        shuffled.join('') === correctDigits.join(''),
        'The PIN is a palindrome -- reversing it does not actually produce a different order this pass, cannot test this boundary meaningfully'
      );
  ...
  test(
    "AUTH-EXP-12: A real school paired with credentials for a different school is rejected (blocked -- needs a second school's credentials)",
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Needs a second school's known-working User ID/Password to construct this cross-tenant scenario -- only this project's one school (Goyal Brothers) is available, same blocker class as GAL-SEC-01/TCE-SEC-01 elsewhere in this suite"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'AUTH-EXP-13: Whitespace-only User ID/Password values are rejected as invalid, not silently trimmed and accepted',
    { tag: '@negative' },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
  ...
  test(
    'AUTH-EXP-14: No plaintext password/PIN is exposed in the console or browser storage after a login attempt',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const consoleTexts = [];
  ...
  test(
    'AUTH-EXP-15: Rapidly toggling PIN <-> Password views while a login request is in flight does not produce a stuck or mismatched state',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
      await login.enterPin(process.env.INVALID_PIN || '12745');
      const submitPromise = login
        .pinDigitBox(4)
        .press('Enter')
        .catch(() => {});
      ...
      const stuckSpinner = await page
        ...
        .catch(() => false);
      const modalStillResponsive = await login
        .pinDigitBox(0)
        .isVisible({ timeout: 3000 })
        .catch(() => false);
      console.log(
  ...
  test(
    'AUTH-EXP-16: Browser autofill does not corrupt the numeric PIN box structure',
    { tag: '@negative' },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
      const autocompleteAttr = await login
        .pinDigitBox(0)
  ...
  test(
    'AUTH-EXP-17: Navigating directly to a deep authenticated route while logged out redirects to Sign In, exposing no content',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
  ...
  test(
    'AUTH-EXP-18: An extremely long Password (200+ chars) is handled without a crash',
    { tag: '@negative' },
    async ({ page }) => {
      const login = new LoginPage(page);
      await login.openSignIn();
  ...
  test(
    'AUTH-EXP-19: A tampered/forged session cookie grants no access without a valid server-side session (blocked -- cookie name/format not confirmed)',
    { tag: ['@negative', '@bug'] },
    async ({ page, context }) => {
