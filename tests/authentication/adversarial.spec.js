...
test(
  'AUTH-BREAK-01: signing in with the SAME account concurrently from two genuinely independent browser contexts both resolve to a real, usable session',
  ...
  async ({ page, browser }) => {
    ...
    test.fail(
      !tab1Ok || !tab2Ok,
      'Signing in with the same account concurrently from two independent browser contexts leaves at least one unable to reach a usable signed-in state'
    );
...
test(
  'AUTH-BREAK-02: reloading the instant after a successful PIN submit (before the dashboard fully renders) recovers to a real usable session, not a half-loaded state',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      !cleanOutcome,
      'Reloading the instant after a PIN submit leaves the app in neither a signed-in dashboard nor a clean Guest Mode login prompt'
    );
...
test(
  'AUTH-BREAK-03: rapidly toggling the PIN <-> Password view 10 times in immediate succession (pure UI spam, no request in flight) does not corrupt either form',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const login = new LoginPage(page);
    ...
    test.fail(
      bothVisibleAtOnce || (!pinFormVisible && !pwdFormVisible),
      'Rapidly toggling PIN<->Password 10 times leaves both forms visible at once, or neither visible at all'
    );
...
test(
  'AUTH-BREAK-04: an emoji/Unicode-heavy User ID does not crash the Password-view form or its validation',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const login = new LoginPage(page);
    await login.openSignIn();
    ...
    test.fail(
      ...
      'An emoji/Unicode-heavy User ID crashes the Password-view form or breaks its layout'
    );
