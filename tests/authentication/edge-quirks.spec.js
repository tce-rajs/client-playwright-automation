...
test(
  "EDGE-01: Settings-menu Virtual Keyboard toggle stays in sync with the modal's own toggle",
  { tag: '@ui-state' },
  async ({ page }) => {
...
test(
  'EDGE-02: Resizing the browser window mid-PIN-entry does not lose entered digits',
  { tag: '@boundary' },
  async ({ page }) => {
    await page.goto('./');
    const login = new LoginPage(page);
...
test(
  'EDGE-03: Switching keyboard language mid-session does not corrupt entered value',
  { tag: '@boundary' },
  async ({ page }) => {
    await page.goto('./');
    const login = new LoginPage(page);
