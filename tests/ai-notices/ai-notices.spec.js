...
test(
  'AIN-ACCESS-01: The Magnet -> Notice selection mode is reachable; drag-select produces a selection rectangle with Approve/Discard controls',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
...
test(
  'AIN-TITLE-01: A notice Title is required before Send; entering one unblocks it',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
    ...
    test.fail(
      !titleVisible,
      "AI Notices compose dialog not reachable this pass (blocked upstream on OCR/Approve, matching AIN-ACCESS-01/AIN-OCR-02's own documented entry path)"
    );
    if (!titleVisible) {
...
test(
  'AIN-TITLE-02: An extremely long Title (200+ chars) does not break the compose layout and has no client-side max-length',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
...
test(
  'AIN-EDIT-01: The notice body Editor is a real Quill.js rich-text editor -- Bold formatting works',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
...
test(
  'AIN-EDIT-02: An XSS-style payload typed into the body editor is sanitized, not executed',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
...
test(
  'AIN-REPHRASE-01: Rephrase button is confirmed dead code -- clicking it produces no real AI rephrase',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
...
test(
  'AIN-TRANS-01: Translate button is confirmed dead code -- same root cause as Rephrase',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
...
test(
  'AIN-GRAM-01: Grammar check button is confirmed dead code -- same root cause as Rephrase/Translate',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
...
test(
  'AIN-DEADCODE-SUMMARY-01: Rephrase/Translate/Grammar share one root cause -- a single fix, not three',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
...
test(
  'AIN-SHARE-01: A notice can be targeted to a specific class via a checkbox in Share with...',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
...
test(
  'AIN-SHARE-02: Only classes the signed-in teacher actually teaches appear in the Share with Classes list (blocked -- needs a multi-class account)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'This account/session only has one class-teacher assignment (Class 11A) reachable -- confirming a teacher with 2+ assigned classes sees exactly their own set needs a second account, not available in this project'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'AIN-SEND-01: Send is reachable up to the point of a real dispatch (not executed -- irreversible, student/parent-facing)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
...
test(
  'AIN-SEND-03: Rapid double-click on Send does not dispatch the notice twice (reachability only -- not executed for real)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires a real Send dispatch to observe double-submit behavior -- deliberately not executed against real students/parents, same reasoning as AIN-SEND-01'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'AIN-SEND-04: Closing the composer with unsent, edited content does NOT warn before discarding (confirmed UX gap)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
...
test(
  'AIN-RECAP-01: Recapture does NOT preserve already-typed/edited Title or body -- it fully replaces both with fresh OCR (confirmed, contradicts original premise)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    ...
    if (!recaptureVisible) {
      expect(recaptureVisible).toBe(true);
      return;
    }
...
test(
  'AIN-OCR-01: A failed OCR call (empty selection) shows a clear error toast, not a silent hang',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
...
test(
  'AIN-OCR-02: The OCR success path works end-to-end -- real text is captured and opens a pre-filled compose dialog',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
...
test(
  "AIN-TITLE-BUG-01: BUG -- the Notice Title field's Backspace and Delete keys do nothing",
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
    ...
    test.fail(
      backspaceDidNothing,
      'CONFIRMED BUG: Backspace/Delete in the Notice Title field do not remove characters -- only select-all-then-retype works as a workaround'
    );
...
test(
  'AIN-EXP-01: Drag-selecting a region with both real text and a real image produces a sensible OCR result, not a crash',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires placing both a text object and an image object close together then confirming OCR handles the mix sensibly -- not independently exercised this pass given time constraints on this module'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'AIN-EXP-02: An extremely large drag-selected region does not crash or hang the OCR call ungracefully',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires zooming out and selecting the largest possible region to test this OCR boundary -- not independently exercised this pass given time constraints on this module'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'AIN-EXP-03: Pasting a large rich-formatted text block into the Notice body does not corrupt the Quill.js editor state',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const an = new AiNoticesPage(page);
    const tb = new ToolbarPage(page);
