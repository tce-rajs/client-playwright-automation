...
test(
  'AIN-BREAK-01: an HTML/script-tag string in the Notice Title field is treated as literal text, never executed',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
...
test(
  "AIN-BREAK-02: switching Class while the Notice compose dialog is open does not leave it stuck visible over the new class's whiteboard",
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const tb = new ToolbarPage(page);
    ...
    test.fail(
      switchThrew && dialogStillVisible,
      'CONFIRMED (2/2 reproductions): with the Notice compose dialog open, the Current Class control cannot be clicked at all (10s timeout) -- the dialog fully blocks class-switching rather than allowing it through cleanly or with a clear guard/warning'
    );
...
test(
  'AIN-BREAK-03: rapidly opening and closing the compose dialog 5 times in a row leaves exactly one clean instance',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
...
test(
  'AIN-BREAK-04: pressing the browser Back button while the compose dialog is open does not leave a stuck overlay behind',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const tb = new ToolbarPage(page);
