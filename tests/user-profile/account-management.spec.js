...
test(
  'ACC-ACCESS-01: The Account Management screen is reachable via the avatar/profile drilldown',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      ...
      'Clicking the "Account" tab never reveals its own panel -- the Profile tab\'s content stays visibility:visible and Account\'s (Change Password/PIN) stays visibility:hidden regardless of the click, checked from 0ms to 3000ms after clicking. Change Password/PIN are genuinely unreachable via this UI right now.'
    );
...
test(
  'ACC-PLAN-01: The Change Password form requires the current password before a new one',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
    ...
    if (!linkVisible) {
      test.fail(
        true,
        'Blocked by ACC-ACCESS-01 -- the Change Password link is never revealed by clicking the Account tab'
      );
...
test(
  'ACC-PLAN-02: No rate-limiting/lockout confirmation attempted -- deliberately not executed',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      true,
      "Deliberately not executed -- repeated wrong-password/PIN submission risks locking the shared QA account that every other module's tests depend on"
    );
...
test(
  'ACC-PLAN-03: Cross-session invalidation on credential change -- deliberately not executed',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Deliberately not executed -- requires actually changing the shared account's password/PIN, which is excluded from automation (destructive credential risk)"
    );
    expect(true).toBe(false);
  }
);
...
test(
  'ACC-PLAN-04: A successful credential change usable on next login -- deliberately not executed',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Deliberately not executed -- same reason as ACC-PLAN-03: requires a real credential change on the shared account'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'ACC-PLAN-05: New PIN validation (format rules) is enforced client-side',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
...
test(
  'ACC-PLAN-06: MFA setup/verification flow -- deliberately not executed',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Deliberately not executed -- enrolling/verifying MFA on the shared QA account risks locking out every other module's tests that depend on simple PIN login"
    );
    expect(true).toBe(false);
  }
);
...
test(
  'ACC-SESSION-01: The logged-in session remains valid through a sustained period of active use',
  ...
  async ({ page }) => {
    ...
    while (Date.now() - start < 120000) {
      await page
        .locator('[data-qa-id="playlist-resource-nav-filter-menu"]')
        .click({ force: true })
        .catch(() => {});
      await page.waitForTimeout(500);
...
test(
  'ACC-SESSION-02: Re-entering the PIN after a session drop is reliable via the on-screen keypad',
  { tag: ['@state-persistence', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      true,
      "Cannot force a session drop on demand to set up this case's precondition; the underlying keypad-click-reliability question was already flagged in the Login module as needing human confirmation on a real device"
    );
    expect(true).toBe(false);
  }
);
