...
test('ACC-ACCESS-01: The Account Management screen is reachable via the avatar/profile drilldown', { tag: ['@positive', '@bug'] }, async ({ page }) => {
...
test('ACC-PLAN-01: The Change Password form requires the current password before a new one', { tag: ['@negative', '@bug'] }, async ({ page }) => {
  const acc = new AccountManagementPage(page);
  await acc.openAccountTab();
...
test('ACC-PLAN-02: No rate-limiting/lockout confirmation attempted -- deliberately not executed', { tag: ['@security', '@bug'] }, async ({ page }) => {
...
test('ACC-PLAN-03: Cross-session invalidation on credential change -- deliberately not executed', { tag: ['@security', '@bug'] }, async ({ page }) => {
...
test('ACC-PLAN-04: A successful credential change usable on next login -- deliberately not executed', { tag: ['@security', '@bug'] }, async ({ page }) => {
...
test('ACC-PLAN-05: New PIN validation (format rules) is enforced client-side', { tag: ['@boundary', '@bug'] }, async ({ page }) => {
  const acc = new AccountManagementPage(page);
  await acc.openAccountTab();
...
test('ACC-PLAN-06: MFA setup/verification flow -- deliberately not executed', { tag: ['@security', '@bug'] }, async ({ page }) => {
...
test('ACC-SESSION-01: The logged-in session remains valid through a sustained period of active use', { tag: ['@state-persistence', '@bug'] }, async ({ page }) => {
...
test('ACC-SESSION-02: Re-entering the PIN after a session drop is reliable via the on-screen keypad', { tag: ['@state-persistence', '@bug'] }, async ({ page }) => {
