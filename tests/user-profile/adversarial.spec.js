...
test(
  'USR-BREAK-01: rapidly clicking the avatar 6 times in quick succession never stacks more than one profile popover',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      ...
      'Rapidly clicking the avatar 6 times stacks more than one profile popover instance'
    );
...
test(
  'USR-BREAK-02: pressing the browser Back button while the User Profile modal is open does not leave a stuck overlay behind',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    ...
    test.fail(
      !modalOpenedBefore,
      'User Profile modal did not open this run -- could not exercise the Back-button case'
    );
...
test(
  'USR-BREAK-03: rapidly toggling Dark Mode 10 times in a row settles on a consistent final state, not a flickering/desynced one',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    ...
    test.fail(
      initialChecked !== null && finalChecked !== initialChecked,
      'An even number (10) of rapid Dark Mode toggle clicks does not return to the original state, suggesting a dropped/desynced click'
    );
...
test(
  'USR-BREAK-04: opening Change Password then rapidly Cancel+reopen 5 times in a row leaves exactly one clean form instance',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
    ...
    test.fail(
      !linkVisible,
      'Change Password link not reachable this run -- could not exercise the rapid open/cancel case'
    );
...
test(
  'USR-BREAK-05: signing out while the Add Subjects picker is open (mid-action interruption) leaves the app in a clean, re-loginable state',
  ...
  async ({ page }) => {
    ...
    test.fail(
      !addSubjectsVisible,
      'Add Subjects control not reachable this run -- could not exercise the mid-action Sign Out case'
    );
    ...
    if (!signOutVisible) {
      ...
      test.fail(
        true,
        'With the Add Subjects picker still open, reopening the avatar menu does not surface a reachable Sign Out control -- mid-action interruption via Sign Out cannot be exercised at all in this state'
      );
    ...
    const guestModeVisible = await page
