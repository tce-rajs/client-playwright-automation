...
test(
  'USR-ACCESS-01: User Profile entry point -- avatar -> Signed in as chevron -> modal with Account/Profile tabs',
  { tag: '@positive' },
  async ({ page }) => {
...
test(
  'USR-ACCESS-02: The outer flat menu and inner Account/Profile tab group are two distinct popovers',
  { tag: '@ui-state' },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
...
test(
  'USR-ACCOUNT-01: Account tab shows Preferred Resource Type and Subjects',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
...
test(
  'USR-PROFILE-01: Profile tab shows Change Password and Change PIN entries',
  { tag: '@positive' },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
...
test(
  'USR-PWD-02: A weak new password is flagged in real time, before Save is attempted',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
...
test(
  'USR-PWD-04: Cancel safely discards the form without changing the password',
  { tag: '@positive' },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
...
test(
  'USR-PWD-05: A wrong Current Password is rejected server-side (not executed -- destructive)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires a real Save submit against the shared QA account (arjun.reddy) -- deliberately not executed to avoid risking the working credential relied on by every other test/session'
    );
...
test(
  'USR-PWD-06: New Password / Repeat New Password mismatch is caught (not executed -- destructive)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires a real Save submit against the shared QA account -- deliberately not executed for the same credential-safety reason as USR-PWD-05'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'USR-PWD-07: A successful password change works next login and invalidates the old one (not executed -- destructive)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
...
test(
  'USR-PIN-01: Change PIN form fields render correctly, including Auto-Generate PIN',
  { tag: '@positive' },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
...
test(
  'USR-PIN-02: New PIN boxes only accept numeric characters via a restricted keypad',
  { tag: '@negative' },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
...
test(
  'USR-PIN-04: Auto-Generate PIN displays the generated value in plaintext (security-relevant finding)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
    ...
    await page.waitForTimeout(500);
    const newPinValue = await acc
      .newPinBox(0)
      .inputValue()
      .catch(async () => (await acc.newPinBox(0).textContent()) || '');
    const inputType = await acc
      .newPinBox(0)
      .getAttribute('type')
      .catch(() => null);
    ...
    test.fail(
      isPlaintext,
      'Auto-Generate PIN displays the generated PIN in plaintext (unmasked) on screen -- a real shoulder-surfing risk in a classroom setting'
    );
...
test(
  'USR-PIN-05: PIN boxes are a fixed digit-count group, structurally preventing overflow',
  { tag: '@negative' },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
...
test(
  'USR-SEC-07: A trivially weak/sequential New PIN pattern (not executed -- destructive)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires a real Save submit of a weak PIN against the shared QA account -- deliberately not executed for the same credential-safety reason as USR-PWD-05'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'USR-PIN-03: A successful PIN change is usable next login (not executed -- destructive)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Requires actually changing the shared QA account's real PIN (currently 26826, documented and relied on throughout this project) -- deliberately not executed"
    );
    expect(true).toBe(false);
  }
);
...
test(
  'USR-BUG-01: Change Password and Change PIN forms can both be open simultaneously (naming implies mutual exclusion)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
    ...
    if (changePinLinkStillVisible) {
      await acc.openChangePinLink.click({ force: true });
      await page.waitForTimeout(500);
      const bothFormsVisible =
        (await acc.newPasswordInput.isVisible().catch(() => false)) &&
        (await acc
          .newPinBox(0)
          .isVisible()
          .catch(() => false));
...
test(
  'USR-UI-01: Dark Mode / Virtual Keyboard toggles are raw checkboxes, not Material toggles',
  { tag: '@ui-state' },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
...
test(
  'USR-UI-02: Release Notes dialog content renders inside a Shadow DOM web component',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
    ...
    test.fail(
      !buildBtnVisible,
      "Build-info/Release Notes link not found in this account's outer profile menu this pass"
    );
    if (!buildBtnVisible) {
...
test(
  'USR-UI-03: Account/Profile tab labels carry identical classes regardless of which is selected',
  { tag: '@ui-state' },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
...
test(
  'USR-LOGIN-01: Server-flagged forced-password-change flow at login (not available)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "No account in this project's known credential set is server-flagged with a forced-password-change-at-login state"
    );
    expect(true).toBe(false);
  }
);
...
test(
  'USR-LOGIN-02: Server-flagged forced-PIN-set flow at login (not available)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
...
test(
  'USR-LOGIN-03: MFA Register flow (no MFA enrollment path found in this account)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
    const mfaOptionVisible = await page
...
test(
  'USR-LOGIN-04: MFA Verify flow blocks wrong codes and enforces limits (blocked -- no MFA path exists)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Same blocker as USR-LOGIN-03 -- no MFA enrollment path was found, so there is no MFA Verify flow to test against'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'USR-SEC-01: Repeated wrong-Current-Password attempts are rate-limited (not executed -- risks account lockout)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Deliberately not attempted -- repeated wrong-password submissions risk actually locking out the shared QA account used throughout this project'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'USR-SEC-02: Account lockout/CAPTCHA after repeated failed sign-in attempts (not executed -- risks account lockout)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Same lockout risk as USR-SEC-01, at the sign-in surface instead of the in-app form -- deliberately not attempted'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'USR-SEC-03: A credential change invalidates other logged-in sessions (not executed -- needs a real credential change + second session)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Needs an actual password/PIN change on the shared QA account plus a second logged-in session to check invalidation -- neither set up, and the credential change itself is deliberately avoided'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'USR-SEC-04: New Password identical to Current Password is rejected as a no-op (not executed -- destructive)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
...
test(
  'USR-SEC-05: An extremely long/Unicode New Password does not crash the form (client-side length check only)',
  { tag: '@boundary' },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
...
test(
  'USR-SEC-06: Leading/trailing whitespace in New Password is preserved, not silently trimmed',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
...
test(
  'USR-SUBJ-01: Adding a subject via Add Subjects persists immediately',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
    ...
    test.fail(
      optionCount === 0,
      'The Add Subjects picker opened with zero selectable options despite this account currently having zero existing Subject chips -- expected the picker to offer the full curriculum subject list when nothing is already added'
    );
...
test(
  'USR-SUBJ-02: Removing a subject chip is handled gracefully, one at a time',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
...
test(
  'USR-SUBJ-03: Adding the same subject twice is prevented at the picker level',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
...
test(
  "USR-SUBJ-04: Rapidly double-clicking a chip's remove icon does not remove two chips",
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
...
test(
  'USR-RES-01: Preferred Resource Type selection persists across a full page refresh',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
...
test(
  'USR-CLASSMODE-01: Classroom Mode toggle (Teaching/Planning) may silently change the active class',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
...
test(
  'USR-CROSS-01: Sign Out clears local auth such that a stale tab cannot resume an authenticated action',
  { tag: '@cross-cutting' },
  async ({ page, context }) => {
    const acc = new AccountManagementPage(page);
    ...
    const secondAvatarVisible = await secondPage
...
test(
  'USR-CROSS-02: A credential change does not disrupt unrelated in-progress unsaved work (not executed -- needs a real credential change)',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Needs an actual credential change (deliberately avoided, see USR-PWD-05) at the exact moment of unrelated unsaved work elsewhere -- not attempted this pass'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'USR-EXP-01: Removing the LAST remaining Subject chip is not an ambiguous silent failure (not executed -- would leave account with zero subjects)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Reducing this account's real Subjects list down to zero is not safely reversible in an automated run -- deliberately not executed to protect the shared QA account's working state"
    );
    expect(true).toBe(false);
  }
);
...
test(
  'USR-EXP-02: New PIN identical to Current PIN is rejected as a no-op (not executed -- destructive)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
...
test(
  'USR-EXP-03: Saving Preferred Resource Type with ZERO types selected is blocked',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
    ...
    test.fail(
      !dropdownVisible,
      'Preferred Resource Type control not found on the Account tab this pass -- cannot test the zero-selection boundary'
    );
    expect(dropdownVisible).toBe(true);
...
test(
  'USR-EXP-04: Change Password/PIN forms do not retain stale values if reopened after cancelling',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openProfileMenu();
    ...
    await acc.openChangePinLink.click({ force: true });
    await expect(acc.newPinBox(0)).toBeVisible({ timeout: 5000 });
    const valueAfterReopen = await acc
      .newPinBox(0)
      .inputValue()
      .catch(async () => (await acc.newPinBox(0).textContent()) || '');
...
test(
  'USR-EXP-05: Adding 10+ subjects does not break the chip-wrapping layout (not executed -- would require adding many real subjects)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Adding 10+ real subjects to the shared QA account is not safely reversible in an automated run -- deliberately not executed'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'USR-EXP-06: Rapidly switching Account/Profile tabs does not leave stale content visible',
  { tag: '@boundary' },
  async ({ page }) => {
    const acc = new AccountManagementPage(page);
    await acc.openAccountTab();
