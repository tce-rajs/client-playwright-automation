...
test(
  "AIH-BREAK-01: switching Class while the AI Homework composer is open does not leave it stuck visible over the new class's whiteboard",
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      !composerOpened,
      'AI Homework composer did not open this run -- could not exercise the mid-open class-switch case'
    );
    ...
    const classPopupOpened = await nav.currentClassBtn
      .click({ timeout: 8000 })
      .then(() => true)
      .catch((e) => {
    ...
    test.fail(
      ...
      'With the AI Homework composer open, the Current Class control cannot be clicked at all -- the composer fully blocks class-switching rather than allowing it through or showing a clear guard'
    );
...
test(
  'AIH-BREAK-02: rapidly opening and closing the composer 5 times in a row leaves exactly one clean instance',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
...
test(
  'AIH-BREAK-03: pressing the browser Back button while the composer is open does not leave a stuck overlay behind',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const ah = new AiHomeworkPage(page);
    await ah.open();
...
test(
  "AIH-BREAK-04: rapidly alternating clicks between the objective counter's plus and minus buttons 20 times settles on a value matching the actual net clicks, not a desynced one",
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const ah = new AiHomeworkPage(page);
    ...
    test.fail(
      !counterVisible,
      'Homework objective counter not reachable this run -- could not exercise the rapid-alternating-click case'
    );
    ...
    test.fail(
      endValue !== startValue,
      `20 alternating plus/minus clicks (net zero) left the objective counter at ${endValue} instead of back at its starting value ${startValue} -- suggests dropped/desynced clicks, consistent with the existing AIH-CNT-02 double-click finding`
    );
