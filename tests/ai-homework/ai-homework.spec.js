...
test.describe('AI Homework -- independent checks', () => {
  ...
  test(
    'AIH-TYPE-01: Worksheet type picker offers Homework and Revise with live counters',
    { tag: '@ui-state' },
    async ({ page }) => {
      const ah = new AiHomeworkPage(page);
      await ah.open();
  ...
  test(
    'AIH-TYPE-02: A "lower grade" curriculum swaps the builder for an undocumented component (source-confirmed)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        true,
        'CONFIRMED cross-repo (source + QA reference doc): "lower grade" curricula render an undocumented, zero-selector template component (ai-homework lower-grade templates are explicitly "display-only, not instrumented") -- no UI path exists to assert against this component\'s content even if reached'
      );
  ...
  test(
    'AIH-TOPIC-01: Composer is pre-scoped to the active class with no extra selection needed',
    { tag: '@positive' },
    async ({ page }) => {
      const ah = new AiHomeworkPage(page);
      const nav = new NavigationPage(page);
  ...
  test(
    'AIH-TOPIC-02: Changing Grade/Subject inside the Topics picker does not silently detach the composer from its own class',
    { tag: '@cross-cutting' },
    async ({ page }) => {
      const ah = new AiHomeworkPage(page);
      const nav = new NavigationPage(page);
  ...
  test(
    'AIH-CNT-01: Question-count counters have working increment/decrement with a sensible default',
    { tag: '@ui-state' },
    async ({ page }) => {
      const ah = new AiHomeworkPage(page);
      await ah.open();
  ...
  test(
    'AIH-CNT-02: A fast double-click on the increment button registers as zero net increments (confirmed bug)',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const ah = new AiHomeworkPage(page);
      await ah.open();
      ...
      test.fail(
        netChange !== 2,
        `CONFIRMED BUG: a fast double-click on the Objective counter's + button registered a net change of ${netChange} instead of +2`
      );
  ...
  test(
    'AIH-DEAD-01: ai-homework-select-* and ai-homework-preview-* components are confirmed dead code',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
  ...
  test(
    'AIH-QUOTA-01: Generate surfaces a clear error when the AI-generation quota is exhausted',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ah = new AiHomeworkPage(page);
      await ah.open();
      ...
      await page.route(/rag\/worksheet|rag\/visual-worksheet|ai[-_]?homework/i, (route) =>
        route.fulfill({ status: 429, body: JSON.stringify({ error: 'quota_exceeded' }) })
      );
      await ah.generateBtn.click({ force: true });
      await page.waitForTimeout(4000);

      const questionsRenderedAnyway = await ah.builderQuestions
        .first()
        .isVisible({ timeout: 8000 })
        .catch(() => false);
      const errorTextVisible = await page
        .getByText(/error|failed|quota|try again|something went wrong/i)
        .first()
  ...
  test(
    'AIH-VALID-01: Grade/subject validation is subject-specific, not account-wide',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ah = new AiHomeworkPage(page);
      const nav = new NavigationPage(page);
  ...
  test(
    'AIH-EXP-04: Cross-session content bleed under concurrent Generate calls (tooling limitation, needs 2 real sessions)',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        true,
        "Needs two independent teacher sessions generating concurrently on the same Chapter/Topic -- only one account (VALID_PIN_2) is available to this suite without violating the session's account-isolation rule against the concurrently-running verifier agent's VALID_PIN"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'AIH-EXP-05: The Generate request cannot be forged to target an unauthorized Grade/Subject/Chapter',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        true,
        "Blocked by the same forging-tooling gap as NAV-SEC-01/EXP-06/ATT-EXP-05 -- needs the real Generate request's exact shape plus a known unauthorized Grade/Subject/Chapter id, neither available without a second reference account"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'AIH-EXP-01: Setting the Objective counter to its absolute minimum still produces a valid Generate call and a correctly-sized result',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
  ...
  test(
    'AIH-EXP-02: Setting the Objective counter to its absolute maximum produces a valid Generate call within a reasonable time, without timing out',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
...
test.describe('AI Homework -- generated worksheet flow', () => {
  ...
  test.beforeEach(async ({}, testInfo) => {
    ...
    if (!reachedAssignStep) {
      const alreadyOnBuilder = await ah.builderQuestions
        .first()
  ...
  test(
    'AIH-QB-02: A question can be regenerated from the builder and the change is retained',
    { tag: ['@positive', '@bug'] },
    async ({}) => {
      test.setTimeout(60000);
      ...
      if (!match) {
        test.fail(
          true,
          'Could not parse the question kind/index from its own data-qa-id to locate the matching regenerate control'
        );
      ...
      await ah.regenerateBtn.click({ force: true, timeout: 5000 }).catch(async () => {
        ...
        await page
          .locator(`[data-qa-id="ai-homework-builder-${qKind}-regenerate-${qIndex}"]`)
  ...
  test(
    'AIH-QB-03: Swiping past the first/last question does not crash or wrap unexpectedly',
    { tag: '@boundary' },
    async ({}) => {
      const count = await ah.builderQuestions.count();
      const firstKindMatch = ((await ah.builderQuestions.first().getAttribute('data-qa-id')) || '').match(
        /builder-(scq|mcq|subjective)-question-(\d+)/
      );
      const lastKindMatch = ((await ah.builderQuestions.last().getAttribute('data-qa-id')) || '').match(
        /builder-(scq|mcq|subjective)-question-(\d+)/
      );
      let pageCrashed = false;
      page.once('crash', () => {
      ...
      if (firstKindMatch) {
        await ah
          .swipeLeft(firstKindMatch[1], firstKindMatch[2])
      ...
      if (lastKindMatch) {
        await ah
          .swipeRight(lastKindMatch[1], lastKindMatch[2])
          .click({ force: true, timeout: 3000 })
          .catch(() => {});
      }
      await page.waitForTimeout(500);
  ...
  test(
    'AIH-QB-04: A rapid double-click on Regenerate does not fire two overlapping AI/RAG requests',
    { tag: ['@negative', '@bug'] },
    async ({}) => {
      test.setTimeout(60000);
      let ragRequestCount = 0;
      const onReq = (req) => {
        if (/rag\/worksheet|rag\/visual-worksheet/i.test(req.url())) ragRequestCount++;
      };
  ...
  test(
    'AIH-FORM-01: The full Assignment form is reachable and pre-filled correctly after Generate',
    { tag: ['@positive', '@bug'] },
    async ({}) => {
  ...
  test(
    'AIH-SEC-01: No exposed class checkbox targets a class this teacher is not assigned to teach',
    { tag: ['@security', '@bug'] },
    async ({}) => {
      ...
      test.fail(
        anyBlank,
        'A blank/unlabeled class-share option was exposed at the Assign step -- possible data-scoping leak'
      );
  ...
  test(
    'AIH-FORM-02: Send is disabled while a required field (Title) is empty (button state inspected, never clicked)',
    { tag: ['@negative', '@bug'] },
    async ({}) => {
      await ah.ensureAssignStep();
      ...
      test.fail(
        !blocked,
        'Send button does not report a disabled state with a required field (Title) empty -- deliberately NOT clicking it to confirm since that would risk a real send if this finding is wrong'
      );
  ...
  test(
    'AIH-FORM-03: Send is disabled with zero classes selected (button state inspected, never clicked)',
    { tag: ['@negative', '@bug'] },
    async ({}) => {
      await ah.ensureAssignStep();
      ...
      test.fail(
        !blocked,
        'Send button does not report a disabled state with zero classes selected -- deliberately NOT clicking it to confirm since that would risk a real send if this finding is wrong'
      );
      expect(blocked).toBe(true);
    }
  );
  ...
  test(
    'AIH-FORM-05: Previous preserves generated questions and Assign-step edits when navigating back and forward',
  ...
  test(
    'AIH-EXP-03: Max "Due in" option alongside multi-class sharing produces a correctly-configured assignment before Discard',
    { tag: ['@boundary', '@bug'] },
    async ({}) => {
      await ah.ensureAssignStep();
      ...
      await page.waitForTimeout(400);
      const selected = await ah
        .assignDueRadio(dueOptionCount - 1)
        .isChecked()
        .catch(() => false);
  ...
  test(
    'AIH-STATE-01: Refreshing mid-composer does not silently lose a fully-generated worksheet',
    ...
    async ({}) => {
      ...
      const composerStillOpen = await page
        .locator('[data-qa-id^="ai-homework-"]')
        .first()
        .isVisible({ timeout: 3000 })
        .catch(() => false);
      const warningShown = await page
        .getByText(/unsaved|lose|discard|draft/i)
        .first()
        .isVisible({ timeout: 2000 })
        .catch(() => false);
      console.log(
  ...
  test(
    'AIH-FORM-04: Discard on the Assign step cleanly exits with no leftover draft',
    { tag: '@positive' },
    async ({}) => {
      await ah.open();
      const typePickerVisible = await ah.homeworkTypeCard.isVisible({ timeout: 8000 }).catch(() => false);
