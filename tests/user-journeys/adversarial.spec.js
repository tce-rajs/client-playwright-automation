...
test(
  'UJ-BREAK-01: a realistic lesson journey -- draw notes, open Minimap to check layout, open Attendance, then switch class -- leaves at most ONE stuck panel, and confirms whether the drawing survived',
  ...
  async ({ page }) => {
    ...
    await nav
      ...
      .catch((e) => console.log('Class switch threw:', e.message.split('\n')[0]));
    ...
    test.fail(
      minimapStuck || attendanceStuck,
      'CONFIRMED end-to-end consequence: after a realistic sequence of opening Minimap then Attendance then switching class, at least one panel is left stuck visible over the new class (compounds the individually-confirmed MM-BREAK-01/ATT-BREAK-02 findings into a single real workflow)'
    );
...
test(
  "UJ-BREAK-02: opening AI Homework's composer then, WITHOUT closing it, trying to open AI Notices via the Magnet menu -- two different composers colliding",
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    ...
    const opened = await magnetToolBtn
    ...
    test.fail(
      !pageAlive,
      "Attempting to open a second module (AI Notices) while AI Homework's composer is already open crashes the page"
    );
    expect(pageAlive).toBe(true);
  }
);
...
test(
  "UJ-BREAK-03: signing out immediately after switching class (before the new class's content has finished loading) recovers to a clean, re-loginable state",
