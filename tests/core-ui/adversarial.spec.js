...
test(
  'CORE-BREAK-01: an extremely small viewport (320x480, a real low-end phone size) does not overlap the logo/date/toolbar',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      ...
      'At a real low-end phone width (320px), the header logo and date/time overlap or overflow horizontally instead of stacking/shrinking responsively'
    );
...
test(
  'CORE-BREAK-02: an unusually large 4K viewport (3840x2160) keeps the logo pinned left and clock pinned right, not floating in the middle',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      ...
      'On a 4K-class viewport, the header logo and/or clock drift away from their pinned corners instead of staying anchored'
    );
...
test(
  'CORE-BREAK-03: rapidly reloading the page 8 times in immediate succession never leaves duplicate header elements mounted',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      anyDuplicated,
      'Rapidly reloading the app 8 times in a row leaves more than one instance of a header element mounted at once'
    );
...
test(
  'CORE-BREAK-04: a nonexistent local time inside a DST spring-forward gap does not crash or blank the clock',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      !looksSane,
      'The header clock renders an empty/garbled/NaN string when the system clock is set to a DST spring-forward-gap instant, instead of a sane fallback'
    );
...
test(
  'CORE-BREAK-05: an Arabic (RTL) locale + Asia/Riyadh timezone context renders the header without breaking layout direction',
  { tag: ['@boundary', '@bug'] },
  async ({ browser }) => {
    ...
    test.fail(
      !logoLeftOfCalendar,
      'Under an Arabic (RTL) browser locale, the header logo and date/time swap position or overlap instead of keeping a stable layout'
    );
...
test(
  'CORE-BREAK-06: two browser tabs opening Guest Mode at the exact same instant both render a complete, independent header',
  { tag: '@cross-cutting' },
  async ({ browser }) => {
