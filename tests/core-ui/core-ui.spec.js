...
test(
  'CORE-03: date & time are visible top-right and match the device clock',
  { tag: '@ui-state' },
  async ({ page }) => {
...
test(
  'CORE-06: toggle button moves the toolbar without moving logo/date',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
...
test(
  'CORE-07: date/time format is correct across the AM/PM and midnight boundary',
  { tag: '@boundary' },
  async ({ page }) => {
    ...
    const expectedNextDay = justAfterMidnight.toLocaleDateString('en-US', {
      weekday: 'short',
      month: 'short',
      day: '2-digit',
    });
...
test(
  'CUI-EXP-01: a missing/404 logo image falls back gracefully, not as a broken-image icon',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      collapsed,
      'Logo div collapses to zero size when its background-image request fails, instead of holding its layout space'
    );
...
test(
  'CUI-EXP-02: an extreme/invalid system clock does not crash the header date/time display',
  { tag: '@negative' },
  async ({ page }) => {
...
test(
  'CUI-EXP-03: a network failure during the initial app-shell load shows an error state, not a silent blank screen',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      bodyIsBlank,
      'A fully failed initial load leaves an indefinitely blank white page with no error/retry affordance'
    );
...
test(
  'CUI-EXP-04: an unusually long version string does not break the header layout',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const logo = page.locator('[data-qa-id="wb-header-logo-image"]');
    const version = page.locator('[data-qa-id="wb-header-version-text"]');
    ...
    const logoMoved =
      logoBoxAfter &&
      logoBoxBefore &&
      (Math.abs(logoBoxAfter.x - logoBoxBefore.x) > 2 || Math.abs(logoBoxAfter.y - logoBoxBefore.y) > 2);
    test.fail(
      Boolean(logoMoved) || overflowsViewport,
      'An unusually long version string pushes the logo out of position or overflows the header instead of truncating/wrapping'
    );
...
test(
  'CUI-EXP-05: a spoofed browser timezone only affects display, not server-side date logic',
  { tag: '@security' },
  async ({ browser }) => {
    ...
    const expectedHour = new Date().toLocaleString('en-US', {
