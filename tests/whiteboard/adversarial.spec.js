...
test(
  'WB-BREAK-01: reloading mid-drag (mouse down, moving, before mouseup) leaves no corrupt half-drawn stroke after the page comes back',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
...
test(
  "WB-BREAK-02: two browser tabs on the same account drawing on the SAME class/topic concurrently do not corrupt each other's strokes",
  ...
  async ({ page, context }) => {
    ...
    await Promise.all([tb1.penStroke(at(150, 300), at(350, 300)), tb2.penStroke(at(600, 300), at(800, 300))]);
    ...
    // register flake).
    test.fail(
      afterReloadCount <= before1,
      "Neither concurrent tab's stroke persisted after a reload -- concurrent same-topic drawing may be silently dropping strokes"
    );
...
test(
  'WB-BREAK-03: drawing a stroke then IMMEDIATELY switching Class (before any autosave could plausibly fire) -- does the stroke survive?',
  ...
  async ({ page }) => {
    ...
    test.fail(
      !strokeRegistered,
      'The pen stroke did not register at all this run -- could not exercise the immediate-class-switch data-loss case'
    );
    ...
    await nav
      ...
      .catch((e) => console.log('switch away threw:', e.message.split('\n')[0]));
    await nav
      .resetToClass('Class 12', 'A', 'Physics')
      .catch((e) => console.log('switch back threw:', e.message.split('\n')[0]));
    ...
    test.fail(
      afterRoundTripCount < afterDrawCount,
      'CONFIRMED: a stroke drawn immediately before switching Class (with no deliberate wait for autosave) is LOST on returning to the same class -- a real data-loss risk following directly from the existing WB-SAVE-DEAD-01 finding'
    );
