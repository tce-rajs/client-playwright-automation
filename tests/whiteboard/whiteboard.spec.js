...
test(
  'WB-ACCESS-01: Whiteboard renders as the live canvas substrate under the active class/topic',
  { tag: '@cross-cutting' },
  async ({ page }) => {
...
test(
  'WB-HDR-01: Header shows logo/version/live clock, and the bottom-left bar shows the active class + chapter/topic',
  { tag: '@positive' },
  async ({ page }) => {
    const wb = new WhiteboardPage(page);
...
test(
  'WB-WELCOME-01: Welcome Back container -- computed style vs. rendered-visibility paradox',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const wb = new WhiteboardPage(page);
    ...
    if (count === 0) {
      ...
      test.fail(
        true,
        'wb-welcome-back-container was not present in the DOM this run -- could not re-attempt the computed-style-vs-rendered-pixels paradox check this pass (it is a transient post-login panel)'
      );
    ...
    test.fail(
      paradox,
      'Confirmed unresolved paradox (cross-repo): computed style reports hidden/opacity:0 for wb-welcome-back-container while it renders visibly on screen -- no binding/animation trigger found in two rounds of DOM investigation'
    );
...
test(
  'WB-DRAW-01: Drawing surface is a real SVG -- strokes are genuine <path> elements, not raster content',
  { tag: '@ui-state' },
  async ({ page }) => {
...
test(
  'WB-CHOOSE-01: First-time "Choose a class" control is a confirmed dead no-op (onChooseAClass has an empty method body)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const wb = new WhiteboardPage(page);
    ...
    if (!present) {
      ...
      test.fail(
        true,
        'CONFIRMED cross-repo (source-read): onChooseAClass() has an empty method body -- clicking .choose-class does nothing. Not independently re-clickable on this account since it already has an active class and the first-time prompt does not render.'
      );
    ...
    const anyPopupOpened = await page
...
test(
  'WB-SAVE-DEAD-01: WhiteboardSaveService.save() has zero callers -- the "save Whiteboard to Playlist" pipeline is unreachable from any UI path',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      true,
      'CONFIRMED cross-repo (source search): WhiteboardSaveService.save() has zero callers anywhere in the app -- the entire "local unsaved Whiteboard card" pipeline (Playlist card -> preview dialog -> Download PDF / Send Notice / Open in Whiteboard) is unreachable through any UI flow'
    );
...
test(
  "WB-SAVE-DEAD-02: Add Resource -> Whiteboard card's Save-to-Playlist / Download PDF buttons are dead ends (same root cause as WB-SAVE-DEAD-01)",
  ...
  async ({ page }) => {
    ...
    if (stillStuck) {
      test.fail(
        true,
        'Add Resource picker stuck with pointer-events:none across reload attempts (see LIVE_FINDINGS.md) -- could not even reach the Whiteboard action card this run'
      );
    ...
    if (!cardReady) {
      test.fail(
        true,
        "Add Resource picker's Whiteboard action card did not settle into a visible/clickable state this run (picker-open animation timing flake, not the WB-SAVE-DEAD-01 finding itself)"
      );
...
test(
  'WB-PANZOOM-01: Two distinct pan/zoom mechanisms exist -- the outer wb-drawing-container transform vs. the inner panGroup',
  { tag: '@ui-state' },
  async ({ page }) => {
    const wb = new WhiteboardPage(page);
...
test(
  'WB-TEXT-01: Whether a newly inserted text object is auto-focused (re-checked live -- contradicts the cross-repo workbook claim)',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const wb = new WhiteboardPage(page);
    ...
    test.fail(
      true,
      "CONFIRMED (this pass): whether a newly inserted text object is auto-focused is non-deterministic across runs (observed both focused and not-focused) -- contradicts the workbook's claim of a consistent behavior in either direction"
    );
    expect(true).toBe(false);
  }
);
...
test(
  'WB-TEXT-02: Whether any gesture reopens the editor on already-committed text (re-checked live -- contradicts the cross-repo workbook claim)',
  { tag: '@negative' },
  async ({ page }) => {
    const wb = new WhiteboardPage(page);
...
test(
  'WB-CLEAR-01: Clear Whiteboard asks for confirmation, removes canvas content, leaves Playlist resources untouched',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const wb = new WhiteboardPage(page);
...
test(
  'WB-ANCH-01: Header anchors stay in place across interactions; the class/chapter context bar does NOT (new finding)',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const wb = new WhiteboardPage(page);
    ...
    test.fail(
      classBarShifted,
      'CONFIRMED (new finding): opening the Class popup permanently shifts the class-bar and chapter/topic-bar ~130px down and never restores, contradicting the general "structural anchors are stable" claim'
    );
...
test(
  'WB-DISPATCH-01: A raw pointer event must target the INNER svg, not the outer wrapper, to draw a stroke',
  { tag: '@ui-state' },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const wb = new WhiteboardPage(page);
...
test(
  'WB-CONFLICT-NOTE-01: Documentation note -- this rewrite targets the broader 100-case workbook scope, not the original 22/23-case one',
  { tag: '@cross-cutting' },
  async ({ page }) => {
...
test(
  'WB-EXP-01: Canvas remains responsive after 500 rapid pen strokes (memory/performance stress)',
  { tag: '@boundary' },
  async ({ page }) => {
...
test(
  'WB-EXP-02: Canvas does not corrupt/overflow at max zoom, and content is not permanently mispositioned after zooming back out',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const wb = new WhiteboardPage(page);
    ...
    await page.waitForTimeout(500);
    const overflowsAtMaxZoom = await page.evaluate(
    ...
    test.fail(
      ...
      'Zooming to maximum overflows the viewport or hides the toolbar, instead of scaling cleanly within it'
    );
...
test(
  'WB-EXP-03: Two overlapping simulated pointer inputs at different canvas locations do not corrupt drawing state into one merged stroke',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const wb = new WhiteboardPage(page);
    ...
    test.fail(
      oneMergedStroke,
      'Two overlapping simulated pointers merged into a single corrupted stroke spanning both locations, instead of being handled independently or cleanly ignored'
    );
...
test(
  'WB-EXP-04: Switching tools mid-stroke does not leave a stuck/orphaned partial stroke',
  { tag: '@boundary' },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    const wb = new WhiteboardPage(page);
...
test(
  'WB-EXP-05: A script/HTML payload typed into a Text object renders as literal text, never executes (XSS)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const wb = new WhiteboardPage(page);
...
test(
  'WB-EXP-06: Shapes tool has no raw-markup input surface to attempt SVG-script injection through',
  { tag: '@security' },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
...
test(
  'WB-DRAW-XREF-01: Toolbar-level drawing behavior (Pen/Eraser/Shapes/Widgets/dock) is exercised on this SAME whiteboard surface, not a separate one',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    await tb.waitForBoardToSettle();
...
test(
  'WB-ERASER-01: Eraser cannot reliably remove a Pen stroke once it exceeds roughly 700px (cross-ref: TB-CYP-03)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
...
test(
  'WB-ERASER-02: Erasing one word of connected cursive handwriting deletes an adjacent word too (cross-ref: TB-CYP-04)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
...
test(
  'WB-DOCK-01: The dock toggle switches which side the toolbar rail sits on (cross-ref: TB-GAP-02)',
  { tag: '@positive' },
  async ({ page }) => {
