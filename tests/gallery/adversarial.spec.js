...
test(
  'GAL-BREAK-01: a 250-character search query does not crash the Gallery grid or hang the search request',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    ...
    test.fail(
      !searchStillResponsive,
      'A 250-character Gallery search query crashes/hides the search box instead of returning a graceful (likely empty) result'
    );
...
test(
  'GAL-BREAK-02: an HTML/script-tag string in the Gallery search box is rendered as literal text, never executed',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
...
test(
  'GAL-BREAK-03: emoji/Unicode input in the Gallery search box produces a graceful empty state, not a crash',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
...
test(
  'GAL-BREAK-04: rapidly clicking Search 5 times in a row on the same query does not fire 5 overlapping requests that race and show inconsistent results',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
