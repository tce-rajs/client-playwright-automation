// Gallery.
// Source: CEP_TestCases/Gallery_Module_Test_Cases_Final.xlsx.
// Split out of tests/add-resource/ into its own module folder — Gallery is
...
test.describe('Core (ADD-GAL-01..07)', () => {
  test(
    'ADD-GAL-01: Gallery opens showing an image grid, category filters, and a search box',
    { tag: '@ui-state' },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
  ...
  test(
    "ADD-GAL-02: Gallery's default category filter is independent of the current class's subject",
    { tag: '@ui-state' },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      const currentClassText = (
        await page.locator('[data-qa-id="playlist-current-grade-subject-btn"]').textContent()
      ).trim();
  ...
  test(
    'ADD-GAL-03: Searching a term with no matches shows a completely blank grid with no message',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      ...
      const noResultsMsgVisible = await page
  ...
  test(
    'ADD-GAL-04: Certain Gallery thumbnails fail to load, showing a broken-image placeholder',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
  ...
  test(
    'ADD-GAL-05: Changing the category filter dropdowns updates the displayed image grid',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      ...
      if (optionCount < 2) {
        test.fail(
          true,
          `Only ${optionCount} filter option(s) available -- can't confirm the grid changes on a different selection`
        );
  ...
  test(
    "ADD-GAL-06: Selecting a Gallery image attaches it to the current Topic's playlist",
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
  ...
  test(
    'ADD-GAL-07: Pagination controls navigate additional pages of images',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
...
test.describe('Extended coverage (gap-analysis pass, 19-row workbook)', () => {
  ...
  test(
    'GAL-CLOSE-01: Gallery\'s own close (X) control -- re-testing an earlier "not found" finding',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      ...
      if (!closeBtnVisible) {
        test.fail(
          true,
          'gallery-close-btn not found/visible this pass -- matches this workbook\'s own GAL-CLOSE-01 "no working close control" finding (see GAL-CLOSE-02 for the conflicting cross-repo claim that it does exist and work)'
        );
      ...
      const galleryStillOpen = await ar.galleryImageCards
        .first()
        .isVisible()
        .catch(() => false);
      test.info().annotations.push({
  ...
  test(
    "GAL-CLOSE-02: CONFLICT FLAG resolution -- cross-repo claims gallery-close-btn works, contradicting GAL-CLOSE-01's finding",
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
  ...
  test(
    'GAL-ASSET-01: A Gallery-inserted image has no removable/editable Playlist asset record, unlike every other Add Resource path',
    ...
    async ({ page }) => {
      ...
      test.fail(
        !anyCanvasIncrease,
        'Could not detect the inserted Gallery image landing on the canvas via any of the probed selectors this pass -- cannot independently confirm the insert itself happened'
      );
      ...
      test.fail(
        !noPlaylistCardCreated,
        'A Gallery-inserted image DID create a Playlist asset card this pass -- contradicts the cross-repo-confirmed "no asset record" finding'
      );
  ...
  test(
    "GAL-SYNC-01: Gallery's whiteboard-persistence network call is a distinct endpoint pattern from the Playlist/asset endpoints",
    { tag: ['@ui-state', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      const requestUrls = [];
  ...
  test(
    'GAL-BOUND-01: BUG CHECK -- double-clicking a Gallery thumbnail may insert TWO duplicate copies instead of one',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      ...
      test.fail(
        bestDiff === 0,
        'Could not detect any new image landing on canvas after a double-click this pass -- cannot confirm the duplication bug either way'
      );
      if (bestDiff === 0) {
  ...
  test(
    'GAL-STATE-01: Gallery-inserted images survive a full page reload',
    ...
    async ({ page }) => {
      ...
      const bestSelector = Object.keys(afterInsert).reduce(
        (best, sel) =>
          afterInsert[sel] - (beforeInsert[sel] ?? 0) > afterInsert[best] - (beforeInsert[best] ?? 0) ? sel : best,
  ...
  test(
    'GAL-XCUT-01: Reopening the Add Resource "+" FAB while Gallery is still open stacks a second popup instead of replacing the first',
    { tag: ['@cross-cutting', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      ...
      const galleryImagesStillVisible = await ar.galleryImageCards
        .first()
        .isVisible()
        .catch(() => false);
      test.info().annotations.push({
      ...
      test.fail(
        stacked,
        'Reopening the "+" Add Resources FAB while Gallery is still open stacks a second Add Resources popup instead of closing/replacing the first'
      );
  ...
  test(
    "GAL-SEC-01: Gallery's shared/global image library never leaks another tenant's content (blocked -- needs a second school account)",
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Confirming no cross-tenant/cross-school image leakage into the shared Gallery grid requires a second school/tenant account to compare against -- only this project's one school (Goyal Brothers) is available, same blocker class as other cross-tenant checks elsewhere in this suite (e.g. TB-EXP-15/17, NAV-SEC-01)"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'GAL-EXP-01: A Gallery category/sub-category combination with zero images shows a clear empty state, not an indistinguishable blank grid',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
  ...
  test(
    'GAL-EXP-02: Rapidly triggering Load More several times in quick succession does not skip, duplicate, or corrupt the image order',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      ...
      test.fail(
        !loadMoreVisible,
        'No Load More control found in the Gallery panel this pass -- current category may fit on a single page, cannot test rapid-paging integrity'
      );
      if (!loadMoreVisible) {
  ...
  test(
    'GAL-EXP-03: Inserting the same Gallery image 10+ times does not degrade whiteboard performance or compound the duplication bug',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
  ...
  test(
    "GAL-EXP-04: Gallery's shared image library cannot be used to insert an unauthorized tenant's asset by manipulating the insert request (blocked -- no forging tooling)",
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Same IDOR-class blocker as PLR-GAP-SEC-01/09 and TB-EXP-15/17 elsewhere in this suite -- needs the Gallery-insert request's exact asset-reference shape plus a known unauthorized asset ID from a different tenant, neither available without request-crafting tooling and a second tenant account"
      );
      expect(true).toBe(false);
    }
  );
