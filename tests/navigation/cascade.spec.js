...
test(
  'NAV-CAS-01: Grade, Division and Subject are shown together in one 3-column layout',
  { tag: '@ui-state' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
...
test('NAV-CAS-03: A Grade with exactly one Division auto-selects it', { tag: '@positive' }, async ({ page }) => {
  ...
  test.skip(
    divisionCount !== 1,
    `Class 12 has ${divisionCount} division(s) in this account, not exactly 1 — need a real single-division grade`
  );
  await expect(nav.divisionButtons.first()).toHaveClass(/btn--active/);
});

test(
  'NAV-CAS-04: Selecting a Subject immediately commits the class switch, no separate confirm step',
  { tag: '@positive' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
...
test(
  'NAV-CAS-05: Switching Grade after a Division/Subject are already chosen',
  { tag: '@positive' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    await nav.gradeButton('Class 9').click();
...
test(
  'NAV-CAS-06: A Grade with zero Divisions or Subjects assigned to this teacher',
  { tag: '@negative' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
...
test(
  'NAV-CAS-07: Reselecting the already-active Grade/Division/Subject',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      true,
      'The All My Classes cascade sometimes shows no grade marked active at all — confirmed reproducible, not a timing flake'
    );
...
test(
  'NAV-CAS-08: Rapid repeated clicks across different Grade pills before columns finish updating',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    await nav.gradeButton('Class 9').click();
...
test(
  "NAV-CAS-09: Grade/Division/Subject lists only ever show this teacher's own assigned classes",
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
...
test(
  'NAV-CAS-10: Very long or visually similar Grade/Subject names render without breaking the layout',
  { tag: '@boundary' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
