...
test(
  'NAV-BREAK-01: pasting a 300-character string into the Chapter/Topic search box does not crash the popup or freeze the list',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    ...
    test.fail(
      !stillResponsive,
      'A 300-character search string crashes or hides the Chapter/Topic search popup instead of showing a graceful zero-results state'
    );
...
test(
  'NAV-BREAK-02: emoji/Unicode input in the Chapter/Topic search box is handled gracefully with a real empty state, not a crash',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    await nav.openChaptersPopup();
...
test(
  'NAV-BREAK-03: an HTML/script-tag string in the Chapter/Topic search box is rendered as literal text, never executed or injected raw',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    ...
    test.fail(
      xssRan || dialogFired,
      'An HTML/script-tag string typed into the Chapter/Topic search box executes as real markup instead of being treated as literal text'
    );
...
test(
  'NAV-BREAK-04: rapidly toggling the Chapter/Topic search open/closed 6 times in immediate succession leaves exactly one clean state, not a stuck/duplicated input',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    const visibleCount = await nav.chapterTpSearchInput
    ...
    test.fail(
      searchInputCount > 1,
      'Rapidly toggling the Chapter/Topic search control mounts more than one search input instance'
    );
    expect(searchInputCount).toBeLessThanOrEqual(1);
    test.fail(
      visibleCount > 1,
      'Rapidly toggling the Chapter/Topic search control leaves more than one visible input at once'
    );
...
test(
  "NAV-BREAK-05: using the browser's native Back button after a class switch does not resurrect stale content over the new class's playlist",
  ...
  async ({ page }) => {
    ...
    await page
      ...
      .catch((e) => console.log('goBack threw:', e.message));
    ...
    const labelCameBack = await nav.currentClassBtn
      .waitFor({ state: 'visible', timeout: 8000 })
      .then(() => true)
      .catch(() => false);
    const afterBackLabel = labelCameBack
    ...
    test.fail(
      !stillShowsAClass,
      "CONFIRMED: using the browser's native Back button after an in-app class switch leaves the Current Class label unreachable (the SPA does not push real history entries for in-app navigation, so Back navigates to a broken/pre-app state) instead of showing either the old or new class cleanly"
    );
...
test(
  'NAV-BREAK-06: opening the Class Popup and immediately the Chapters Popup (no wait in between) does not stack two open overlays at once',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    ...
    test.fail(
      bothOpenAtOnce,
      'Rapidly clicking Current Class then Current Chapter/Topic opens BOTH popups stacked at once instead of the second closing/replacing the first'
    );
...
test(
  'NAV-BREAK-07: a whitespace-only Chapter/Topic search query is treated as empty/no-filter, not as a literal unmatched string',
  { tag: '@boundary' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    await nav.openChaptersPopup();
...
test(
  'NAV-BREAK-08: clicking a chapter item immediately after clearing an active search filter selects the CURRENTLY shown chapter, not a stale pre-search reference',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    ...
    const clicked = await nav.chapterItems
      .first()
      .click({ timeout: 4000 })
      .then(() => true)
      .catch((e) => {
    ...
    test.fail(
      !selectedSomething,
      `Clicking a chapter the instant a search filter is cleared does not register a real chapter selection (click succeeded: ${clicked}, item count right after clear: ${countRightAfterClear}, search input still visible: ${searchInputStillThere}, popup still visible: ${popupStillThere})`
    );
