...
test.describe('Extended coverage (gap-analysis pass)', () => {
  ...
  test(
    'GSD-LVL-01: "Early Childhood Education" is one more Grade entry, not a separate Level selector',
    { tag: '@ui-state' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
  ...
  test(
    'NAV-SEC-03: A teacher can reach a Grade/Subject combination outside their real assignment via the normal UI',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        true,
        'No known limited-assignment teacher account available in this project — only VALID_PIN (broad assignment) is configured; the temporary Teacher Four PIN noted in the reference project was scoped to Attendance only and its access window has passed'
      );
  ...
  async function selectChapterAndLandOnTopic(nav, page, index) {
    ...
    const topicsShown = await nav.topicItems
      .first()
  ...
  test(
    'GSD-CYP-01: Chapter/Topic index mapping resolves correctly at the very first and very last chapter',
    { tag: '@boundary' },
    async ({ page }) => {
  ...
  test(
    'GSD-CYP-02: Clicking a Recent Classes item on this account switches the Class, not just the Topic',
    { tag: '@negative' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
  ...
  test(
    'GSD-CYP-03: A second click on Current Class while its own popup is already open closes it',
    { tag: '@ui-state' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openClassPopup();
      ...
      await page.waitForTimeout(500);
      const stillOpen = await nav.recentClassButtons
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test(
    'GSD-CYP-04: A curriculum book already fetched this session cannot be forced fresh, even via a full reload',
    { tag: '@cross-cutting' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.resetToClass('Class 9', 'A', 'Hindi Language');
  ...
  test(
    'GSD-GAP-01: Multiple unassigned Grade/Subject combinations load only generic content, no roster/gradebook leakage',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        true,
        'Same blocker as NAV-SEC-03 — no known limited-assignment teacher account available in this project to identify an out-of-assignment combination against'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'NAV-EXP-01: A Grade/Division with zero Subjects mapped shows a clear empty state',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openClassPopup();
      ...
      test.fail(
        !foundZeroSubjectCombo,
        "No Grade/Division with zero Subjects exists in this account's real assignment — cannot exercise the empty-state rendering without one"
      );
  ...
  test(
    'NAV-EXP-02: The class switch does not proceed to a half-switched state if the content fetch fails',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      ...
      test.fail(
        halfSwitched,
        'Current Class label updates even though the follow-up content fetch was aborted, with no error state shown'
      );
  ...
  test(
    'NAV-EXP-03: A Recent Classes entry for a since-revoked assignment is rejected server-side',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Needs admin-side test-data control to revoke this teacher's assignment to a class already in their Recent Classes history — not available in this environment"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'NAV-EXP-04: Rapidly clicking different Subjects settles on the LAST clicked one, not a stale earlier response',
    { tag: '@negative' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openClassPopup();
  ...
  test(
    'NAV-EXP-05: A Subject name with special characters (e.g. "&") renders and selects correctly',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openClassPopup();
      ...
      test.fail(
        !specialSubjectText,
        "No Subject name containing special characters exists in this account's real curriculum — cannot exercise this rendering path without one"
      );
  ...
  test(
    'NAV-EXP-06: Forging a class-switch API request for an unauthorized classId is rejected server-side',
    { tag: ['@negative', '@bug'] },
    async ({ page, request }) => {
      ...
      test.fail(
        true,
        "No known out-of-scope classId available, and the class-switch endpoint's exact request shape was not reverse-engineered — needs a second reference account to determine both"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'NAV-EXP-07: Switching class while a Magnet-gated panel is open does not leave it stale',
    { tag: '@negative' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
  ...
  test(
    'NAV-EXP-08: The Recent Classes list caps at a reasonable size rather than growing unbounded',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      test.setTimeout(90000);
      const nav = new NavigationPage(page);
  ...
  test(
    'NAV-EXP-09: A Grade/Subject combo whose curriculum content was removed shows a clear message, not a blank canvas',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        'Needs admin-side test-data control to remove curriculum content for a previously-valid combination — not available in this environment'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'NAV-EXP-10: Rapidly toggling between Recent Classes and All My Classes tabs does not desync which tab is actually interactive',
    { tag: '@negative' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openClassPopup();
  ...
  test(
    "NAV-EXP-11: Class 11's full Subject list remains scrollable and the last entry is selectable",
    { tag: '@boundary' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openClassPopup();
  ...
  test(
    'NAV-EXP-12: Opening the class popup immediately after a hard refresh never shows an empty/half-populated picker',
    { tag: '@boundary' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await page.reload();
