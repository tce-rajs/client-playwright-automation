...
test(
  'NAV-CHP-01: Chapters Popup shows a two-column layout: Chapter tree left, Topic list right',
  { tag: '@ui-state' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
...
test(
  'NAV-CHP-02: Chapters are shown as an expandable/collapsible nested tree',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const nav = new NavigationPage(page);
...
test(
  'NAV-CHP-03: The currently active Chapter and Topic are visually highlighted',
  { tag: '@ui-state' },
  async ({ page }) => {
...
test(
  'NAV-CHP-04: Selecting a different Topic under the same Chapter updates the playlist immediately',
  { tag: '@positive' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
...
test(
  'NAV-CHP-05: Selecting a different Chapter refreshes the Topic list on the right',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      true,
      'Topic list gets stuck on a stale entry after a few chapter switches in quick succession — reproduced across 6+ attempts'
    );
...
test(
  'NAV-CHP-06: Reopening the Chapters Popup preserves the last-selected Chapter/Topic highlight',
  ...
  async ({ page }) => {
    ...
    test.fail(
      true,
      'Clicking through chapters to find one with topics reliably hangs — a chapter item becomes unstable/unclickable mid-search, reproduced across 3 attempts'
    );
    ...
    const selectedChapterText = (
      await page.locator('[data-qa-id="playlist-select-chapter"].active').textContent()
    ).trim();
...
test(
  'NAV-CHP-08: Opening the Chapters Popup for a class/subject with no curriculum content mapped yet',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
