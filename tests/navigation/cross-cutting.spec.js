...
test(
  'NAV-STATE-01: Current Class/Chapter/Topic persist across a full page refresh',
  { tag: '@positive' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
...
test(
  'NAV-STATE-02: Selection persists after navigating to another module and back',
  { tag: '@positive' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    const beforeClass = await nav.currentClassBtn.textContent();
...
test(
  'NAV-STATE-03: Dismissing a popup without selecting anything leaves the prior state unchanged',
  { tag: '@positive' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    const beforeClass = await nav.currentClassBtn.textContent();
...
test(
  'NAV-STATE-04: Switching Class while the Chapters Popup is still open for the previous class',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    await nav.openChaptersPopup();
    ...
    test.fail(
      !classPopupOpened,
      'Clicking Current Class while the Chapters Popup is open never actually opens the Class Popup'
    );
...
test(
  'NAV-STATE-05: Session/token expiring while a Navigation popup is open',
  { tag: '@security' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    await nav.openClassPopup();
...
test(
  'NAV-NET-01: Grade/Division/Subject data fetch fails when opening All My Classes',
  ...
  async ({ page }) => {
    ...
    test.fail(
      ...
      'Curriculum fetch failure leaves All My Classes looking like a legitimate empty list, no error/retry shown'
    );
...
test(
  'NAV-NET-02: Chapter/Topic data fetch fails when opening the Chapters Popup',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    const nav2 = new NavigationPage(page);
    await page.route('**/tce-school-api/1/api/1/curriculum', (route) => route.fulfill({ status: 500, body: '{}' }));
...
test(
  'NAV-NET-03: Class switch succeeds but the follow-up content fetch fails',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    ...
    expect(
      labelChangedWithNoErrorShown,
      'Current Class label updated with no error state shown while its content fetch failed'
    ).toBe(false);
  }
);
...
test(
  'NAV-RACE-01: Out-of-order network responses across a fast double class switch',
  { tag: '@boundary' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    ...
    await nav.openClassPopup();
    await nav.recentClassButtons
      .filter({ hasText: classC.split('|')[2].trim() })
      .first()
      .click(); // -> C (fast)
...
test(
  'NAV-RACE-02: Two browser tabs, same account, switching to different classes nearly simultaneously',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ context }) => {
    ...
    test.fail(
      true,
      'Logging in via context.newPage() reliably times out in this environment — reproduced across 3 attempts'
    );
...
test(
  'NAV-SEC-01: Forging/replaying the class-switch request with a classId this teacher is not assigned to',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      true,
      'No known out-of-scope classId available to forge the request with — needs a second reference account or back-office data'
    );
...
test(
  'NAV-SEC-02: Curriculum data containing a Chapter/Topic name with embedded script/markup',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const nav = new NavigationPage(page);
...
test(
  'NAV-DUP-01: A single normal click fires exactly one network request, not a duplicate',
  { tag: '@negative' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
...
test(
  'NAV-E2E-01: Switching Current Class stops any actively playing Player content',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    ...
    test.fail(
      true,
      'No actively-playing Video resource reachable from this test in isolation — needs Player-module setup first'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'NAV-E2E-02: Switching Current Class with unsaved in-progress work in another module',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
