...
test(
  'NAV-REC-01: Recent Classes lists previously used class/division/subject combinations',
  { tag: '@positive' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
...
test(
  'NAV-REC-02: Selecting a Recent Classes item instantly switches the active class',
  { tag: '@positive' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    ...
    await expect(page.locator('.mat-mdc-tab-labels, [data-qa-id="playlist-chapter-tp-popup"]'))
...
test(
  'NAV-REC-03: Switching to a class via Recent Classes moves it to the top of the list',
  { tag: '@positive' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
...
test(
  'NAV-REC-04: Recent Classes list does not visually mark which class is currently active',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      ...
      'The active class entry has no distinguishing indicator from any other entry in the Recent Classes list'
    );
...
test(
  'NAV-REC-05: Recent Classes tab with zero history (brand-new teacher)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
...
test(
  'NAV-REC-06: Recent Classes list scroll behavior with many distinct classes',
  { tag: '@boundary' },
  async ({ page }) => {
    const nav = new NavigationPage(page);
