...
test(
  'LS-BREAK-01: an HTML/script-tag string in the Title field is treated as literal text, never executed',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
...
test(
  'LS-BREAK-02: rapidly opening the composer, discarding, and reopening 5 times leaves exactly one clean composer instance',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
...
test(
  'LS-BREAK-03: rapidly checking/unchecking all class-selection checkboxes 3 times in a row settles on a consistent final state',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const pl = new PlaylistPage(page);
    ...
    test.fail(
      count === 0,
      'No class-selection checkboxes were reachable this run -- could not exercise the rapid-toggle case'
    );
    ...
    const stillResponsive = await ls.classCheckboxes
      .first()
...
test(
  'LS-BREAK-04: pressing the browser Back button while the composer is open does not leave a stuck overlay behind',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const pl = new PlaylistPage(page);
...
test(
  'LS-BREAK-05: clicking Save to Playlist and Save Revision in immediate succession does not fire both simultaneously',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const pl = new PlaylistPage(page);
    ...
    test.fail(
      !saveVisible || !revisionVisible,
      'One of the two Save controls (Save to Playlist / Save Revision) is not simultaneously reachable with a filled Title -- documents actual composer button-availability behavior'
    );
