...
async function openComposerViaAltEntry(page, pl, ls) {
  const bootstrapped = await ls.ensureOwnedVideoAsset(
    pl,
    new (require('../../pages/add-resource.page').AddResourcePage)(page)
  );
...
test(
  'LS-ACCESS-01: Existing Learning-Shorts-type recordings are playable from the Playlist',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    ...
    for (let i = 0; i < count && !found; i++) {
      const html = await pl.resourceCards
        .nth(i)
...
test(
  "LS-ALT-ENTRY-01: An owned Video asset's overflow -> Send reaches the same composer as the gated Record flow",
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
...
test(
  'LS-REC-01: The Magnet -> Learning Shorts recording panel is reachable and renders correctly',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const ls = new LearningShortsPage(page);
    await ls.openMagnetSubmenu();
    const itemVisible = await ls.magnetLearningShortsItem
      .first()
...
test(
  'LS-EXIT-01: Exit without recording closes the panel cleanly',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const ls = new LearningShortsPage(page);
    await ls.openMagnetSubmenu();
    ...
    test.fail(
      !itemVisible,
      "The Learning Shorts item was not found in this account/class's Magnet menu -- same gating as LS-REC-01"
    );
    expect(itemVisible).toBe(true);
    if (!itemVisible) return;
...
test(
  'LS-TITLE-01: A Title is required before Save/Send, and once entered unblocks it',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    const ls = new LearningShortsPage(page);
    ...
    test.fail(
      ...
      "Could not determine the Send button's disabled state at all -- selector may not match this account's composer markup"
    );
    expect(sendDisabledEmpty === null ? true : sendDisabledEmpty === true && sendDisabledAfterTitle === false).toBe(
      true
    );
  }
);
...
test(
  'LS-ATT-01: An attachment can be removed and re-captured/re-attached',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    const ls = new LearningShortsPage(page);
...
test(
  'LS-SHARE-01: The class-selection list is reachable and shows checkable options',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    const ls = new LearningShortsPage(page);
...
test(
  "LS-SEC-01: The class-selection list is limited to this teacher's own assigned classes",
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    const ls = new LearningShortsPage(page);
...
test(
  'LS-SAVE-DISTINCT-01: The composer exposes two distinct save controls (Save to Playlist vs Save Revision)',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    const ls = new LearningShortsPage(page);
...
test(
  'LS-SAVE-01: Save (to Playlist) is reachable up to the point of a real dispatch (not executed on the shared QA account)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    const ls = new LearningShortsPage(page);
...
test(
  'LS-SEND-01: Send is reachable up to the point of a real dispatch (not executed on the shared QA account)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    const ls = new LearningShortsPage(page);
...
test(
  "LS-FILTER-BUG-01: A non-Video owned card's overflow does not offer the same Send composer path",
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    const ls = new LearningShortsPage(page);
    ...
    for (let i = 0; i < count; i++) {
      ...
      if ((html.includes('pdf') || html.includes('worksheet')) && !html.includes('video')) {
        nonVideoOwnedCard = card;
        break;
      }
    }
...
test(
  'LS-STATE-01: Discarding or navigating away mid-composition may silently lose in-progress work with no warning',
  ...
  async ({ page }) => {
    ...
    await page.waitForTimeout(500);
    const confirmDialogShown = await page
...
test(
  "LS-BOUND-01: An extremely long or special-character Title does not break the composer's layout",
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    const ls = new LearningShortsPage(page);
...
test(
  'LS-XCUT-01: Exit-without-recording is a clean, always-working baseline path (cross-check of LS-EXIT-01)',
  ...
  async ({ page }) => {
    ...
    test.fail(
      !itemVisible,
      "The Learning Shorts item was not found in this account/class's Magnet menu -- same gating as LS-REC-01/LS-EXIT-01"
    );
    expect(itemVisible).toBe(true);
    if (!itemVisible) return;
    await ls.magnetLearningShortsItem.first().click({ force: true });
...
test(
  'LS-EXP-01: Camera/mic permission REJECTION is handled with a clear message, not a silent hang',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires reaching the real getUserMedia() permission prompt and denying it -- this browser-automation environment blocks camera/mic access outright before any prompt is reachable, confirmed live'
    );
...
test(
  'LS-EXP-02: A camera/microphone hardware failure is handled gracefully',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires a real device with a genuinely occupied/unavailable camera -- not reproducible via this browser-automation environment, which blocks all camera/mic access outright'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'LS-EXP-03: A network loss during an active recording upload is handled with a retry, not silent data loss',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Requires a completed real recording to be mid-upload when network is cut -- blocked upstream by this environment's camera/mic restriction, so there is no real upload in progress to interrupt"
    );
    expect(true).toBe(false);
  }
);
...
test(
  'LS-EXP-04: Submitting a zero-duration/corrupted recording is rejected with a clear message',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Requires a real, genuinely zero-duration recording produced by actually starting and immediately stopping the camera -- blocked upstream by this environment's camera/mic restriction"
    );
    expect(true).toBe(false);
  }
);
...
test(
  'LS-EXP-05: A recording at the maximum allowed duration boundary is handled correctly',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Requires a real recording run to (or near) its maximum duration -- blocked upstream by this environment's camera/mic restriction"
    );
    expect(true).toBe(false);
  }
);
...
test(
  'LS-EXP-06: Rapidly starting/stopping the recording does not corrupt state or leave the panel stuck',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const ls = new LearningShortsPage(page);
    await ls.openMagnetSubmenu();
    ...
    test.fail(
      !itemVisible,
      "The Learning Shorts item was not found in this account/class's Magnet menu to even reach the recording panel"
    );
    expect(itemVisible).toBe(true);
    if (!itemVisible) return;
    ...
    for (let i = 0; i < 4; i++) {
      await ls.magnetLearningShortsItem
        .first()
        .click({ force: true })
        .catch(() => {});
...
test(
  "LS-EXP-07: A saved Learning Short is scoped to the correct class, never visible from another class's Playlist",
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires an actual saved recording (a real Send/Save dispatch) to compare across classes -- deliberately never dispatched against the shared QA account (see LS-SAVE-01/LS-SEND-01), so there is no real saved Short yet to check for cross-class leakage'
    );
    expect(true).toBe(false);
  }
);
