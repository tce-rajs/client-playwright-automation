...
test(
  'PL-ADD-01: Floating "+" opens a picker with all six resource-source options',
  { tag: '@positive' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
...
test(
  'PL-ADD-02: Closing the Add Resources picker returns cleanly with no side effects',
  { tag: '@positive' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
...
test(
  'PL-ADD-03: Each Add Resources option actually opens its respective flow (smoke check)',
  ...
  async ({ page }) => {
    ...
    const failedOnes = Object.entries(results)
      .filter(([, opened]) => !opened)
      .map(([name]) => name);
    test.fail(
      failedOnes.length > 0,
      `These Add Resources options didn't visibly open their own flow: ${failedOnes.join(', ')}`
    );
