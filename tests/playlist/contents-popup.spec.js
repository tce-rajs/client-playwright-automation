...
test(
  'PL-TOC-02: The Contents popup reuses the same Chapter/Topic model as top-level Navigation',
  { tag: '@positive' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
...
test(
  'PL-TOC-03: Selecting a Topic from Contents updates Current Chapter/Topic and refreshes the playlist',
  { tag: '@positive' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
...
test(
  'PL-TOC-04: Search Table of Contents matches by case-insensitive substring',
  { tag: '@positive' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
...
test('PL-TOC-05: Search Table of Contents with no matches', { tag: ['@negative', '@bug'] }, async ({ page }) => {
  ...
  const noResultsMessageVisible = await page
  ...
  test.fail(
    resultsCount === 0 && !noResultsMessageVisible,
    'Zero-match search shows a blank panel with no "No results found" message'
  );
  expect(noResultsMessageVisible).toBe(true);
});

test(
  'PL-TOC-06: Cancel restores the full Chapter/Topic list after a search',
  { tag: '@positive' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
...
test(
  'PL-TOC-07: Search also matches Chapter titles, not only Topic titles',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
