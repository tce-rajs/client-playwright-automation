...
test(
  'PL-STATE-01: Playlist content reloads correctly after a full page refresh',
...
test(
  'PL-STATE-02: Removing a resource requires confirmation before it disappears',
  { tag: '@state-persistence' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    ...
    await expect(
      page.getByText(/are you sure you'd like to remove this resource/i).filter({ visible: true })
...
test(
  'PL-STATE-03: A failed remove-resource call leaves no inconsistent client state',
  ...
  async ({ page }) => {
    ...
    test.fail(
      silentlyRemovedWithNoError,
      'A resource card disappeared even though its delete request failed, with no error shown'
    );
...
test(
  'PL-NET-01: Resource-list fetch fails when switching to a Topic',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    ...
    test.fail(
      ...
      'A failed resource-list fetch shows a silent empty playlist, indistinguishable from a topic with genuinely zero resources'
    );
...
test(
  "PL-NET-02: A resource's thumbnail or asset fails to load",
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    ...
    test.fail(
      brokenImageCount > 0,
      `${brokenImageCount} resource thumbnail(s) are already showing as broken images on this pass`
    );
...
test(
  "PL-RACE-01: Rapidly switching Topics before the previous Topic's resource list finishes loading",
  { tag: '@boundary' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    await pl.openContentsPopup();
...
test(
  'PL-SEC-01: Playlist content is always scoped to the currently authorized class',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
...
test(
  'PL-DUP-01: A single click on Add Resources or a filter checkbox fires exactly one action',
  { tag: '@negative' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    let requestCount = 0;
