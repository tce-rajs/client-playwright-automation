...
test(
  'PL-BREAK-01: an HTML/script-tag string in the Contents/TOC search box is treated as literal text, never executed',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
...
test(
  'PL-BREAK-02: rapidly toggling ALL resource-type filter checkboxes off then on, 3 full cycles fast, leaves a consistent final resource-card count',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
...
test(
  'PL-BREAK-03: opening Playlist Options and immediately the Contents popup (no wait in between) does not stack two open overlays at once',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    await pl.openOptionsMenu();
    ...
    const optionsVisible = await pl.filterOptions
      .first()
    ...
    test.fail(
      bothOpenAtOnce,
      'Rapidly clicking Playlist Options then Contents opens BOTH popups stacked at once instead of the second closing/replacing the first'
    );
...
test(
  'PL-BREAK-04: rapidly clicking through 8 different Chapters in quick succession settles on exactly the last-clicked one with no stuck/duplicated resource cards',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const pl = new PlaylistPage(page);
    ...
    test.fail(
      chapterCount < 3,
      'Fewer than 3 chapters reachable this run -- could not exercise the rapid-multi-chapter-click case'
    );
    ...
    const pageAlive = await pl.resourceCards
      .first()
    ...
    test.fail(
      ...
      'Rapidly clicking through 8 chapters leaves more than one chapter marked active at once'
    );
