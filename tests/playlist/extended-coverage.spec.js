...
test.describe('Extended coverage (gap-analysis pass)', () => {
  ...
  test(
    'PL-CHP-01: Chapters Popup two-column layout -- cross-ref NAV-CHP-01 (tests/navigation/chapters-topics.spec.js)',
    { tag: '@ui-state' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
  ...
  test(
    'PL-CHP-02: Chapters as an expandable/collapsible nested tree -- cross-ref NAV-CHP-02',
    { tag: '@ui-state' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openChaptersPopup();
  ...
  test(
    'PL-CHP-03: Active Chapter/Topic visually highlighted -- cross-ref NAV-CHP-03',
    { tag: '@positive' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openChaptersPopup();
  ...
  test(
    'PL-CHP-04: Selecting a Topic under the same Chapter updates the playlist -- cross-ref NAV-CHP-04',
    { tag: '@positive' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      const pl = new PlaylistPage(page);
  ...
  test(
    'PL-CHP-05: Selecting a different Chapter refreshes the Topic list -- cross-ref NAV-CHP-05',
    { tag: '@positive' },
    async ({ page }) => {
      ...
      const opened = await nav.chapterItems
        .first()
      ...
      test.fail(
        chapterCount === 0,
        'The Chapters popup did not open -- likely the confirmed live-session-drop issue (see 00_PROGRESS_PLAN.md), not a defect in this behavior'
      );
      if (chapterCount === 0) {
        expect(chapterCount).toBeGreaterThan(0);
        return;
      ...
      for (let i = 0; i < chapterCount && topicsAfter.length === 0; i++) {
        ...
        await expect
          .poll(() => nav.topicItems.count(), { timeout: 3000 })
          .toBeGreaterThan(0)
          .catch(() => {});
  ...
  test(
    'PL-CHP-06: Reopening Chapters Popup preserves last-selected highlight -- cross-ref NAV-CHP-06',
    { tag: '@positive' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.goToChapterTopic(0, 0);
  ...
  test(
    'PL-CHP-07: A Chapter with zero mapped Topics -- cross-ref NAV-CHP-07',
    { tag: '@negative' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openChaptersPopup();
  ...
  test(
    'PL-CHP-08: Chapters Popup for a class/subject with no curriculum content mapped -- cross-ref NAV-CHP-08',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Cross-ref NAV-CHP-08 -- no class/subject with zero mapped curriculum content has been identified in this account's reachable classes"
      );
  ...
  test(
    'PL-CHP-09: Very long Topic names are truncated in the UI -- cross-ref NAV-CHP-09',
    { tag: '@boundary' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openChaptersPopup();
  ...
  test(
    'PL-XREF-01: Grade/Division/Subject (Class) selection is covered under the Navigation module, not duplicated here',
    { tag: '@cross-cutting' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
  ...
  test(
    'PL-CYP-01: Tapping the outer Quiz card wrapper is a no-op -- only the inner card responds',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      ...
      const dialogAfterOuterTap = await page
      ...
      await quizCardOuter
        ...
        .catch(() => {});
      await page.waitForTimeout(600);
      const dialogAfterInnerTap = await page
        .locator('[role="dialog"], .mat-dialog-container')
        .isVisible({ timeout: 3000 })
        .catch(() => false);
      test.info().annotations.push({
  ...
  test(
    "PL-CYP-02: The asset overflow menu's Edit option only appears for assets the signed-in teacher personally created",
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      ...
      let anyEditFound = false,
        anyNoEditFound = false;
      for (let i = 0; i < Math.min(assetCount, 6); i++) {
        ...
        const editVisible = await page
          .locator('[data-qa-id="playlist-asset-edit-btn"]')
          .first()
          .isVisible({ timeout: 1500 })
          .catch(() => false);
        if (editVisible) anyEditFound = true;
        else anyNoEditFound = true;
        await page.keyboard.press('Escape').catch(() => {});
  ...
  test(
    'PL-CYP-03: Toggle All does not cleanly restore a full selection from a partial one (confirmed cross-repo bug)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      ...
      test.fail(
        !toggleAllVisible,
        'No "Toggle All" control found this pass -- this account\'s filter menu may not expose one (contradicts the cross-repo-confirmed premise this case is built on)'
      );
      if (!toggleAllVisible) {
        expect(toggleAllVisible).toBe(true);
        return;
      }
      const optionCount = await pl.filterOptions.count();
      // Get to a single-type-checked state.
      for (let i = 1; i < optionCount; i++) {
        await pl.filterOptions
          .nth(i)
          .click({ timeout: 3000 })
          .catch(() => {});
      ...
      await toggleAll.click({ force: true, timeout: 3000 }).catch(() => {});
      await page.waitForTimeout(400);
      const checkedCount = await pl.filterOptions
  ...
  test(
    'PL-CYP-04: A second, separate Show/Hide Drawer control exists as a persistent account-level setting',
    { tag: ['@state-persistence', '@bug'] },
    async ({ page }) => {
  ...
  test(
    'PL-CYP-05: Resource cards stream onto the strip asynchronously -- an empty strip read immediately after a Topic switch is a legitimate transient state',
    { tag: '@boundary' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      const pl = new PlaylistPage(page);
  ...
  test(
    'PL-CYP-06: The local unsaved Whiteboard card -> Save to Playlist path is confirmed dead code (cross-ref WB-SAVE-DEAD-01/02)',
    { tag: ['@cross-cutting', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        'CONFIRMED cross-repo (source-read, same root cause as WB-SAVE-DEAD-01/02 in the Whiteboard module): WhiteboardSaveService.save() has zero callers anywhere in the app -- this feature is genuinely unreachable through any UI flow'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'PL-CYP-07: The resource-order save endpoint rejects a forged sequence payload for an unauthorized class (blocked -- no forging tooling)',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Needs the real PUT .../serve/tp/sequence request's exact shape plus a known classId/topicId this teacher is not authorized for, neither available without a second reference account -- same blocker class as NAV-SEC-01/TB-EXP-15/17 elsewhere in this suite"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'PL-GAP-01: Drag-and-drop reordering of resource cards persists the new order after reload',
    { tag: ['@state-persistence', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      ...
      test.fail(
        !orderChanged,
        'A drag-drop gesture on a resource card did not visibly change the order this pass -- cannot confirm persistence either way'
      );
      if (!orderChanged) {
      ...
      test.fail(
        !persisted,
        'The drag-reordered card order did NOT survive a page reload -- the PUT .../serve/tp/sequence save may have failed silently'
      );
  ...
  test(
    'PL-GAP-02: Rapid multi-card drag-reordering, and dropping a card outside any valid zone, do not corrupt the saved order',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      const cardCount = await pl.resourceCards.count();
  ...
  test(
    'PL-GAP-03: Removing a server-backed curriculum resource card (not a teacher-created asset)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      const cardCount = await pl.resourceCards.count();
  ...
  test(
    'PL-BUG-01: CRITICAL -- closing certain resource previews leaves the Playlist strip/Contents/Add-Resource controls invisible',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      ...
      test.fail(
        !weblinkVisible,
        'No Weblink-type resource card found on this topic this pass to reproduce the confirmed bug against'
      );
      if (!weblinkVisible) {
        expect(weblinkVisible).toBe(true);
        return;
      }
  ...
  test(
    'PL-DRW-01: The Contents/Chapters slide-out panel is the client-named "Drawer" feature',
    { tag: '@positive' },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openChaptersPopup();
  ...
  test(
    'PL-EXP-01: The Chapters Popup shows a clear error if its own chapter/topic data fetch fails (blocked -- no network-blocking tooling)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      ...
      const errorShown = await page
        .getByText(/error|failed to load|try again/i)
        .isVisible({ timeout: 3000 })
        .catch(() => false);
      const chapterCount = await nav.chapterItems.count();
  ...
  test(
    'PL-EXP-02: Removing a resource while its own thumbnail is still loading does not leave an orphaned loading spinner',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      const nav = new NavigationPage(page);
      ...
      test.fail(
        !removeBtnVisible,
        'No Remove control reachable fast enough on a possibly-still-loading card this pass'
      );
      if (!removeBtnVisible) {
        expect(removeBtnVisible).toBe(true);
        return;
      }
      await pl.resourceRemoveBtn
        .first()
        .click({ force: true, timeout: 3000 })
        .catch(() => {});
      await pl.resourceRemoveConfirmBtn
        .first()
        .click({ force: true, timeout: 3000 })
        .catch(() => {});
      await page.waitForTimeout(1000);
  ...
  test(
    'PL-EXP-03: Unchecking every resource-type filter checkbox leaves the checkboxes themselves interactive to recover from',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      await pl.openOptionsMenu();
  ...
  test(
    'PL-EXP-04: A drag-reorder that fails to save server-side reverts the visual order rather than showing a falsely-persisted state (blocked -- no network-blocking tooling)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Needs network-blocking tooling to cut connectivity at the precise moment right after a drag-drop release, not available in a way that reliably races the real save request this pass -- also depends on PL-GAP-01's own drag mechanics"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'PL-EXP-05: Adding the exact same resource a second time via Library is handled sensibly, not as an indistinguishable duplicate',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
  ...
  test(
    'PL-EXP-06: Typing into the Table of Contents search while chapter data is still loading does not search stale/incomplete data',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      ...
      if (!searchToggleVisible) {
        expect(searchToggleVisible).toBe(true);
        return;
      }
  ...
  test(
    'PL-EXP-07: A Topic with 20+ resources across multiple types renders without performance degradation (no such topic identified)',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "No Topic with an unusually large (20+) resource count has been identified in this account's reachable curriculum this pass -- most topics browsed had 4-6 resources"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'PL-EXP-08: A Chapter/Topic name at maximum realistic length has a way to reveal its full un-truncated text',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openChaptersPopup();
  ...
  test(
    'PL-EXP-09: Playlist strip horizontal scroll chevrons correctly disable/hide at both the first and last resource',
    { tag: '@boundary' },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
  ...
  test(
    'PL-EXP-10: Filter checkbox state survives a page refresh (as opposed to a Topic switch)',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      await pl.openOptionsMenu();
      await pl.filterOptions
        ...
        .catch(() => {});
      await page.waitForTimeout(400);
      const stateBefore = await pl.filterOptions
        .nth(0)
      ...
      await page.waitForTimeout(1500);
      await pl.openOptionsMenu();
      const stateAfter = await pl.filterOptions
        .nth(0)
        .getAttribute('aria-selected')
        .catch(() => null);
      test.info().annotations.push({
  ...
  test(
    'PL-EXP-11: Opening Edit mode, making no changes, and Finish Editing leaves the Playlist byte-for-byte unchanged',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      const titlesBefore = await pl.resourceCards.allTextContents();
      ...
      test.fail(
        !finishVisible,
        'No "Finish Editing" control reachable this pass -- Edit mode entry may not have worked as expected'
      );
      if (!finishVisible) {
  ...
  test(
    'PL-EXP-12: Rapidly clicking through every Chapter in quick succession settles correctly on the last-clicked chapter',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const nav = new NavigationPage(page);
      await nav.openChaptersPopup();
      ...
      if (chapterCount < 6) {
        expect(chapterCount).toBeGreaterThanOrEqual(6);
        return;
      }
  ...
  test(
    "PL-EXP-13: A resource-order save request cannot be forged to reorder another teacher's/class's Playlist (blocked -- no forging tooling, same as PL-CYP-07)",
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Same blocker as PL-CYP-07 -- needs the real PUT .../serve/tp/sequence request's exact shape plus a known unauthorized classId/topicId, neither available without a second reference account"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    "PL-EXP-14: Add Resources -> Library/Gallery search results never surface another school's private assets (blocked -- needs a second school account)",
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Confirming cross-tenant isolation needs a second school/tenant account to compare search result sets against -- only this project's one school (Goyal Brothers) is available, same blocker class as GAL-SEC-01/TCE-SEC-01 elsewhere in this suite"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    "PL-EXP-15: A teacher-created custom asset is never visible in another teacher's Playlist for the same class (blocked -- needs the Create file-upload flow plus a second teacher account)",
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Fully blocked pending both the file-upload tooling gap already documented for Add Resource's Create flow (see ADD-CRT-09) and a second teacher account assigned to the same class, neither available this pass"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'PL-EXP-16: Cancelling a Library/Gallery resource add mid-way through its own loading does not leave a corrupted Playlist entry',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      const ar = new AddResourcePage(page);
      ...
      await ar.librarySearchBtn.click();
      await page.waitForTimeout(1500);
      const resultVisible = await ar.libraryResults
        .first()
