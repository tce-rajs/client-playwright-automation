...
test(
  'PL-EBK-01: E-Books tile opens "Choose an eBook" showing the subject\'s textbook',
  { tag: '@positive' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
...
test('PL-EBK-03: A subject with more than one eBook available', { tag: ['@boundary', '@bug'] }, async ({ page }) => {
  ...
  if (count < 2) {
    test.fail(
      true,
      `Only ${count} eBook(s) mapped for this account's current subject — can't confirm multi-book listing without a 2+-book subject`
    );
