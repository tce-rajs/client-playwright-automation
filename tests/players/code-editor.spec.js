...
test(
  'PLR-CODE-01: Clicking a Code resource opens the Code Editor with a mounted Monaco editor',
  { tag: '@positive' },
  async ({ page }) => {
...
test(
  'PLR-CODE-05: Settings opens and shows Minimap/Text Size/Theme options',
  { tag: '@negative' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
...
test('PLR-CODE-06: The Minimap can be shown and hidden', { tag: '@positive' }, async ({ page }) => {
  const plr = new PlayerPage(page);
  await openCodeEditor(page, plr);
  const minimapWidthBefore = await page.evaluate(
...
test(
  'PLR-CODE-09: Force Stop stops a long-running execution (console-kind editors only)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
    ...
    if (isWeb) {
      test.fail(
        true,
        'This resource is the web (HTML/CSS/JS) editor, which has no Force Stop control -- nothing to exercise'
      );
...
test(
  'PLR-CODE-10: Invalid code is handled without freezing or crashing the player',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
...
test(
  'PLR-CODE-11: The Code Editor can be closed and the underlying whiteboard state returns',
  { tag: '@negative' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
...
test(
  'PLR-CODE-10: Code Editor mounts same-origin with zero iframes/shadow roots, fully queryable',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
...
test(
  'PLR-CODE-11: Force Stop only exists for the Python ("console") editor kind, never HTML/CSS/JS ("web")',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
    ...
    await plr.codeRunBtn.click({ force: true });
    await page.waitForTimeout(1500);
    const forceStopVisible = await page
    ...
    if (isWeb) {
      test.fail(
        forceStopVisible,
        'Force Stop unexpectedly appeared on the web (HTML/CSS/JS) editor kind -- contradicts the confirmed finding that this control only exists for the console/Python kind'
      );
...
test(
  'PLR-CODE-12: Monaco renders indentation as non-breaking spaces (test-design guardrail, re-verified)',
...
test(
  'PLR-CODE-13: Real shipped typo -- "Expand All" flips to "Collpase All" (misspelled) the instant it\'s clicked',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
    ...
    const misspeltVisible = await page
...
test(
  "PLR-CODE-14: The Whiteboard's own unrelated minimap coexists with the Code Editor's Monaco minimap without visual overlap",
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
    const wbMinimapCount = await page
      .locator('[data-qa-id="minimap-container"], [data-qa-id="minimap-canvas"]')
      .count();
    const monacoMinimapBox = await page
      .locator('.minimap')
      .first()
      .boundingBox()
      .catch(() => null);
...
test(
  'PLR-CODE-15: Editor settings (theme/font/minimap) persist on the account across sessions',
...
test(
  'PLR-CODE-16: Rapid double-click on Run/Rerun does not cause overlapping output races',
  { tag: '@boundary' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
...
test(
  'PLR-EXP-SEC-02: The Code Editor sandbox deny-list is bypassable via Python object-introspection (re-check of a partially-confirmed finding)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
    ...
    test.fail(
      isWeb,
      'This resource is the web (HTML/CSS/JS) editor kind, which has no Python interpreter to test this Brython sandbox-escape payload against'
    );
    if (isWeb) {
      expect(isWeb).toBe(false);
      return;
...
test(
  'PLR-EXP-04 (Code Editor variant): an infinite loop is terminated by a timeout rather than hanging indefinitely',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      isWeb,
      "This resource is the web editor kind -- an infinite JS loop in a preview iframe is a different failure mode from this row's intended Python/console-kind timeout check"
    );
    if (isWeb) {
      expect(isWeb).toBe(false);
      return;
    ...
    const timeoutMessageVisible = await page
      .getByText(/time.?out|terminated|force\s*stop/i)
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
...
test(
  "PLR-EXP-08: A deliberate syntax error's message specifically identifies the line number and error type",
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
    ...
    test.fail(
      isWeb,
      "This resource is the web editor kind -- refining the console/Python-kind PLR-CODE-05 finding specifically needs the Python interpreter's own error format"
    );
    if (isWeb) {
      expect(isWeb).toBe(false);
      return;
...
test(
  'PLR-EXP-09: Running HTML/CSS/JS-kind code that throws a runtime JS error surfaces the error somewhere visible',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCodeEditor(page, plr);
    ...
    test.fail(
      !isWeb,
      "This resource is the console/Python editor kind -- this row specifically needs the web (HTML/CSS/JS) editor kind, not confirmed reachable from this file's own fixed navigation target this pass"
    );
    if (!isWeb) {
    ...
    test.fail(
      ...
      'A runtime JS error in the web-kind preview surfaced NO visible indication anywhere -- neither an output frame nor a console error was observed'
    );
...
test(
  'PLR-EXP-23: Pasting a very large (several-thousand-line) code block does not crash or freeze the editor',
  { tag: '@boundary' },
  async ({ page }) => {
