...
async function openQuiz(page, plr) {
  ...
  const cameraBlocked = await page
...
test(
  'PLR-QZ-01: Play Quiz opens a launch screen with a counter and Launch button (confirmed working -- blocker is one step later)',
  { tag: '@ui-state' },
  async ({ page }) => {
...
test(
  'PLR-QZ-02: Launch AIR Card loads the first quiz question (BLOCKED -- camera-dependent, see file header)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
...
test(
  'PLR-QZ-03: A question shows prompt text and a grid of lettered options (BLOCKED -- camera-dependent)',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
...
test(
  'PLR-QZ-04: Submit Answer is disabled until an option is selected (BLOCKED -- camera-dependent)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-05: Selecting an option enables Submit Answer (BLOCKED -- camera-dependent)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-06: Submitting a correct answer highlights it and advances to Next Question (BLOCKED -- camera-dependent)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
    ...
    if (correctIndex === -1) {
      test.fail(true, 'Show Answer did not reveal a .correct-classed option');
      expect(correctIndex).not.toBe(-1);
      return;
    }
    await plr.quizCloseBtn.click({ force: true }).catch(() => {});
    await page.waitForTimeout(1000);
    const reopened = await openQuiz(page, plr);
    if (!reopened.reachedQuestion) {
      test.fail(true, CAMERA_BLOCK_REASON);
      expect(reopened.reachedQuestion).toBe(true);
      return;
    }
...
test(
  'PLR-QZ-07: Show Answer reveals the correct option without requiring a selection (BLOCKED -- camera-dependent)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-08: Bottom pagination shows a numbered dot per question and supports direct jump (BLOCKED -- camera-dependent)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-09: Prev/Next chevrons navigate one question at a time (BLOCKED -- camera-dependent)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-10: A Split Screen control opens a side-by-side comparison view (BLOCKED -- camera-dependent)',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
    ...
    await page.waitForTimeout(1500);
    const rollNumberVisible = await page
      .getByText(/roll number/i)
      .first()
...
test(
  "PLR-QZ-11: The quiz's own close control exits cleanly (BLOCKED -- camera-dependent)",
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-12: Submitting an incorrect answer shows Incorrect AND reveals the correct answer simultaneously (BLOCKED -- camera-dependent)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-13: Reopening Play Quiz after closing it fully restarts from Question 1 (BLOCKED -- camera-dependent)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-14: Answered progress is not lost when navigating between questions in the same session (BLOCKED -- camera-dependent)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-15: Rapid double-click on Submit Answer does not cause a double-submission (BLOCKED -- camera-dependent)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-16: A quiz with a very large number of questions keeps pagination usable (BLOCKED -- camera-dependent)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-17: Confirmed real selectors -- same-origin, zero iframes (BLOCKED -- camera-dependent)',
...
test(
  'PLR-QZ-18: Options are visually checkboxes but behave as single-answer (BLOCKED -- camera-dependent)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
    ...
    if (optionCount < 2) {
      test.fail(true, 'Fewer than 2 options');
    ...
    await page.waitForTimeout(400);
    const firstCheckedAfter2 = await plr.quizOptions
      .nth(0)
      .locator('input')
      .isChecked()
      .catch(() => false);
    const secondChecked = await plr.quizOptions
      .nth(1)
      .locator('input')
      .isChecked()
      .catch(() => false);
...
test(
  'PLR-QZ-19: CONFIRMED RACE -- rapid multi-click on a Quiz card can open duplicate stacked instances',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await expect(plr.quizCards.first()).toBeAttached({ timeout: 10000 });
    await plr.quizCards.first().evaluate((el) => {
      el.click();
      el.click();
      el.click();
    });
...
test(
  'PLR-QZ-20: A naive DOM read of question count undercounts on truncated pagination (BLOCKED -- camera-dependent)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-21: Never hardcode a "correct answer" index -- discovered live via Show Answer (BLOCKED -- camera-dependent)',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-QZ-RECONCILE-01: RESOLVED -- the "Launch AIR Card" flow genuinely requires camera access, confirming hypothesis (a)',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
...
test(
  'PLR-EXP-SEC-03: A quiz answer submission cannot be replayed after completion (BLOCKED -- camera-dependent)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  'PLR-EXP-01 (Quiz, network-loss variant): losing connectivity before Submit is handled with a clear error (BLOCKED -- camera-dependent)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  ...
  async ({ page }) => {
    ...
    await page.route(/quiz|question/i, (route) =>
      route.request().method() === 'GET' ? route.abort() : route.continue()
    );
    await plr.quizNextControl
      .locator('button')
      .first()
      .click({ force: true })
      .catch(() => {});
...
test(
  'PLR-EXP-18: A single-question quiz renders sensibly (BLOCKED -- camera-dependent, plus no such quiz confirmed to exist)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
...
test(
  "PLR-EXP-19: A large quiz's pagination dots scroll rather than overflow (BLOCKED -- camera-dependent)",
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { reachedQuestion } = await openQuiz(page, plr);
