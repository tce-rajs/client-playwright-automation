...
test('PLR-CKP-FND-05: The timer badge can be hidden', { tag: ['@ui-state', '@bug'] }, async ({ page }) => {
  ...
  test.fail(
    !hideBtnVisibleAfter,
    'No reachable control ever reveals the timer-hide button (data-qa-id exists in the reference doc but never becomes visible via direct display, or clicking the timer badge)'
  );
...
test(
  'PLR-CHK-04: Excel export is not available on a checkpoint with zero completions',
  { tag: '@negative' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCheckpoint(page, plr);
...
test(
  'PLR-CHK-05: Confirmed state machine -- Online launch is a one-shot irreversible transition (documentation-verification, no NEW launch performed)',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCheckpoint(page, plr);
    ...
    test.fail(
      onModeSelect,
      "This resource is still in the CREATED/mode-select state -- if true, this file's own earlier tests never actually launched it, worth a closer look"
    );
...
test(
  'PLR-CHK-06: Only ONE STARTED checkpoint is allowed per class at a time (observation only -- not provoking a real 2nd Start)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCheckpoint(page, plr);
...
test(
  "PLR-CHK-07: A checkpoint card's status label lags the real status by a couple of seconds after an action",
  { tag: '@negative' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCheckpoint(page, plr);
...
test(
  'PLR-CHK-08: The End button sits under a position:fixed timer overlay -- an un-forced click may not land reliably',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCheckpoint(page, plr);
    ...
    test.fail(
      !isStarted,
      "Could not reach a STARTED checkpoint this run to test the End button's clickability against"
    );
    if (!isStarted) {
    ...
    const elementAtCenter = endBtnBox
      ? await page.evaluate(
          ({ x, y }) => {
            const el = document.elementFromPoint(x, y);
            return el ? el.outerHTML.slice(0, 100) : null;
          },
          { x: endBtnBox.x + endBtnBox.width / 2, y: endBtnBox.y + endBtnBox.height / 2 }
        )
      : null;
...
test(
  'PLR-CHK-09: Open a checkpoint card by index or a short title substring, not its full blended card text (re-verified)',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const pl = new PlaylistPage(page);
...
test(
  'PLR-CHK-10: Excel score upload/download round-trip validation (invalid file rejected with a clear message)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCheckpoint(page, plr);
...
test(
  "PLR-CHK-12: Resource pool exhaustion -- this account's CREATED/PAUSED checkpoint pool is finite with no reset mechanism",
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const pl = new PlaylistPage(page);
...
test(
  "PLR-EXP-SEC-05: A Checkpoint's recorded score cannot be tampered with via client-side manipulation",
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Needs DOM/console-level score manipulation tooling against a checkpoint mid-attempt with a real student submitting answers -- this teacher-facing QA account has no way to submit answers AS a student to produce a real score to tamper with'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'PLR-EXP-10: Attempting to start a SECOND Checkpoint while one is already STARTED is blocked with a clear message (observation only)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCheckpoint(page, plr);
...
test(
  'PLR-EXP-11: Losing network connectivity during the offline-capable flow does not lose already-entered answers',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openCheckpoint(page, plr);
