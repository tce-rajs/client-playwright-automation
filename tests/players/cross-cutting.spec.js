...
test(
  'PLR-XCUT-01: Consolidated close-icon selector matrix -- three different close-button implementations across Player types',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    ...
    const isRealButton = await page
      .locator('button.closeIcon')
      .first()
    ...
    const isImgWithoutAlt = await page
...
test(
  'PLR-XCUT-02: Opening two DIFFERENT player types simultaneously (Worksheet then Weblink)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await plr.openResourceCard(plr.worksheetCards);
    await plr.closeIcon.first().waitFor({ state: 'visible', timeout: 15000 });
    const worksheetVisibleBefore = await plr.closeIcon
      .first()
      .isVisible()
      .catch(() => false);
    ...
    const worksheetStillOpen = await page
      .locator('button.closeIcon')
      .first()
...
test(
  'PLR-XCUT-03: Closing a player via Esc key, instead of its own X control',
  { tag: '@ui-state' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await plr.openResourceCard(plr.worksheetCards);
    ...
    await page.waitForTimeout(1000);
    const stillOpen = await plr.closeIcon
      .first()
      .isVisible({ timeout: 2000 })
      .catch(() => false);
    test.info().annotations.push({
...
test(
  'PLR-EXP-SEC-04: A briefly-cached player from the PREVIOUS class does not remain interactive after switching Current Class',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const nav = new NavigationPage(page);
    ...
    await page.waitForTimeout(1500);
    const oldPlayerStillThere = await plr.closeIcon
      .first()
      .isVisible({ timeout: 2000 })
      .catch(() => false);
    test.info().annotations.push({
...
test(
  'PLR-EXP-16: Closing a player via the browser Back button leaves the same clean state as its own X control',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await plr.openResourceCard(plr.worksheetCards);
    ...
    await page.waitForTimeout(1500);
    const stillOnAppShell = await page
...
test(
  'PLR-EXP-17: A player left open across the confirmed ~60-120s session-expiry window does not silently lose state (long-running, generalizes an AI Homework/Worksheet finding to this type)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    ...
    const stayInterstitialVisible = await page
      .getByText(/still there|stay signed in/i)
      .isVisible({ timeout: 3000 })
      .catch(() => false);
    const playerStillOpen = await plr.closeIcon
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
    test.info().annotations.push({
    ...
    test.fail(
      !playerStillOpen,
      'The player silently disappeared/lost state across the confirmed inactivity window, with or without the warning interstitial appearing'
    );
...
test(
  'PLR-EXP-25: Opening the same NON-Video player resource twice in rapid succession does not open two overlapping instances',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await plr.worksheetCards.first().evaluate((el) => {
      el.click();
      el.click();
    });
