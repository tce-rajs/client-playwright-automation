...
test(
  'PLR-WS-01: Clicking a Worksheet resource opens a PDF-style document viewer',
  { tag: '@positive' },
  async ({ page }) => {
...
test(
  'PLR-WS-02: Header displays worksheet metadata (Title/Subject/Topic and Name/Class/Div/Roll No/Date/Remarks)',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  'PLR-WS-04: A text-to-speech/audio control is available per question (presence only, per workbook)',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  'PLR-WS-05: An annotation toolbar lets the teacher draw directly on the worksheet',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
    ...
    const toolbarVisible = await page
      .locator('[class*="annotation-tool" i], [class*="pencil" i]')
      .first()
...
test(
  'PLR-WS-06: The pencil tool offers color and thickness options',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
    ...
    test.fail(
      !pencilVisible,
      'No pencil tool control found on this worksheet resource -- conditional per-resource per PLR-WS-19'
    );
    if (!pencilVisible) {
...
test(
  'PLR-WS-07: A drawn annotation persists when navigating away and back',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
    test.fail(
      true,
      'Needs a confirmed, reachable drawing gesture on this specific worksheet resource to test persistence against -- not independently confirmed reachable this pass (see PLR-WS-05/06)'
    );
...
test(
  'PLR-WS-08: Page navigation (Prev/Next and Go to Page) moves between pages',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
    ...
    test.fail(
      !nextVisible,
      'No confirmed Next-page control (.pagination-next .mypage-link) found -- this worksheet resource may have only 1 page'
    );
    if (!nextVisible) {
...
test(
  'PLR-WS-09: Zoom controls (+/-) change the document zoom level',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  "PLR-WS-10: A theme/background swatch changes the worksheet's display background",
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  'PLR-WS-11: The orientation toggle and Answer Key toggle are real, functioning controls',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  "PLR-WS-14: Annotations are per-teacher/session and do not leak into another class's copy (needs a 2nd account)",
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Needs a second teacher account/class to compare against -- not available in this project (this session uses a single confirmed working account, VALID_PIN_2)'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'PLR-WS-15: A very long (20+ page) worksheet remains navigable (boundary, no such resource confirmed to exist)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
    test.fail(
      true,
      'No worksheet resource with 20+ pages confirmed available on this account this pass -- the one confirmed Worksheet resource used throughout this file is not that large'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'PLR-WS-16: Eraser tool removes a drawn annotation correctly, including a long continuous stroke',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  'PLR-WS-17: No stable selectors exist on the PDF.js Prev/Next chrome -- only CSS classes work (test-design guardrail, re-verified)',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  'PLR-WS-18: The annotation overlay is real SVG with genuine path elements, not raster',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  "PLR-WS-19: Worksheet features are conditional per-resource flags; absence alone does not mean broken (documented via this file's own live checks)",
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  'PLR-WS-20: Annotations persist to localStorage keyed only by assetId, not by user (needs a 2nd account on the same device)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  'PLR-WS-21: A worksheet WITHOUT an answer key correctly hides the answer-key control (negative case)',
  { tag: '@boundary' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  'PLR-WS-22: A multi-page worksheet stress-tests pagination/zoom/orientation together (blocked, single-page resource only)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
    test.fail(
      true,
      'No multi-page (10+) worksheet resource confirmed available on this account this pass to stress-test'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'PLR-WS-23: CROSS-CUTTING -- the confirmed Toolbar eraser bugs apply to Worksheet annotations too (cross-reference)',
  { tag: '@negative' },
  async ({ page }) => {
...
test(
  'PLR-EXP-SEC-07: The Worksheet Answer Key is never accessible from a student-facing view (teacher-only enforcement)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'This app has no student-facing UI reachable from a teacher QA account -- server-side enforcement of teacher-only Answer Key access cannot be verified from this Playwright-only, single-role-account environment this pass'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'PLR-EXP-03 (Worksheet variant): a zero-page/malformed worksheet resource does not crash the player (no such resource confirmed to exist)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'No malformed/zero-page Worksheet resource exists in the real curriculum browsed this session -- this is a defensive/synthetic-data test needing a specially-crafted resource not available here'
    );
    expect(true).toBe(false);
  }
);
...
test(
  ...
  async ({ page }) => {
    ...
    test.fail(
      !nextVisible,
      'No confirmed Next-page control found -- cannot reach a genuine "last page" boundary on this resource'
    );
    if (!nextVisible) {
      expect(nextVisible).toBe(true);
      return;
...
test(
  'PLR-EXP-05: Typing an out-of-range page number into "Go to Page" is handled gracefully',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
...
test(
  'PLR-EXP-21: The Answer Key toggle state does NOT persist across closing and reopening -- always defaults OFF',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWorksheet(page, plr);
    ...
    if (!answerKeyVisible) {
      expect(answerKeyVisible).toBe(true);
      return;
    }
    await plr.worksheetAnswerKeyBtn.click({ force: true });
    await page.waitForTimeout(1000);
