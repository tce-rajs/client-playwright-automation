...
async function tryOpenDrawer(page, toggle) {
  const before = await page.evaluate(() =>
    Array.from(document.querySelectorAll('.mat-drawer-inner-container')).map((el) => getComputedStyle(el).display)
  );
...
test(
  'PLR-EBK-02: Launching the ebook opens the reader with real page content',
  { tag: '@positive' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
...
test(
  'PLR-EBK-03: The chapter drawer opens to allow switching chapters',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openEbook(page, plr);
    ...
    test.fail(
      !result.changed,
      'Clicking the chapter-drawer toggle has zero effect -- the chapter list drawer never opens (display:none before and after, confirmed via screenshot comparison too), so a different chapter can never be selected'
    );
...
test(
  'PLR-EBK-04: The chapter drawer can be toggled open and closed',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openEbook(page, plr);
    ...
    test.fail(
      !result.changed,
      'Clicking the chapter-drawer toggle has zero effect -- confirmed via DOM computed-style before/after and a visual screenshot comparison, not a timing issue'
    );
    expect(result.changed).toBe(true);
  }
);
...
test(
  'PLR-EBK-05: The resource drawer shows resources linked to the current chapter, or an explicit empty state',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openEbook(page, plr);
    ...
    test.fail(
      !result.changed,
      'Clicking the resource-drawer toggle has zero effect -- the linked-resources drawer never opens, so its contents (or empty state) can never be observed'
    );
    expect(result.changed).toBe(true);
  }
);
...
test(
  'PLR-EBK-03: Confirmed Ebook entry flow -- gated PER-CHAPTER, a genuinely different reach mechanism from every other Player type',
  { tag: '@ui-state' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await plr.openResourceCard(plr.ebookTriggerBtn);
...
test(
  'PLR-EBK-04: CONFIRMED BUG (re-check) -- the chapter-panel collapse/re-expand toggle does not reliably work on the second click',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openEbook(page, plr);
    ...
    test.fail(
      !result1.changed || !result2.changed,
      "CONFIRMED (matches this file's own PLR-EBK-03/04 header note): the chapter drawer toggle has no reliable effect across repeated clicks"
    );
...
test(
  'PLR-EBK-05: The chapter list loads one item at a time (sequential), not in parallel',
  { tag: '@ui-state' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openEbook(page, plr);
...
test(
  "PLR-EBK-06: Opening a resource from WITHIN the Ebook's own resource list (nested player)",
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openEbook(page, plr);
...
test(
  'PLR-EBK-07: Only one confirmed Ebook-linked resource exists -- multi-type-mix coverage is blocked',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openEbook(page, plr);
    ...
    test.fail(
      ...
      'Content-availability gap, not a defect: only ' +
        count +
        ' linked resource(s) confirmed on this chapter -- full coverage of a genuinely mixed resource-type set (Video+Quiz+Worksheet together) needs more variety than exists here'
    );
...
test(
  "PLR-EXP-SEC-08: Direct manipulation of the Ebook reader's page-number parameter cannot access out-of-range pages",
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openEbook(page, plr);
...
test(
  'PLR-EXP-22: Navigating to page 1 and clicking Prev again does not wrap to the last page',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openEbook(page, plr);
