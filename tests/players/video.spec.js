...
async function openVideoTrackingCrash(page, plr) {
  ...
  const playerInitialized =
    (await plr.videoPlayToggle.isVisible({ timeout: 3000 }).catch(() => false)) ||
    (await page
      .locator('video')
      .first()
...
test(
  'PLR-VID-01: Clicking a Video resource card opens the video player frame',
  { tag: '@positive' },
  async ({ page }) => {
    ...
    const frameOpened = await plr.closeIcon
      .first()
...
test(
  'PLR-VID-02: CRITICAL -- video fails to initialize; playback never starts (re-check of a previously confirmed crash)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
...
test(
  'PLR-VID-03: The failure reproduces consistently across a fresh page reload, not a one-off glitch',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
...
test(
  'PLR-VID-04: Clicking the stuck loading icon directly has no effect',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    ...
    test.fail(
      !crashed,
      'Video did not reproduce the confirmed crash this run -- cannot test the stuck-icon-click-does-nothing behavior against a genuinely stuck player'
    );
    if (!crashed) {
    ...
    const box = await page
      .locator('.player, [class*="video"]')
      .first()
      .boundingBox()
      .catch(() => null);
    if (box) await page.mouse.click(box.x + box.width / 2, box.y + box.height / 2);
    await page.waitForTimeout(1000);
    const stillNoPlayback = !(await page
      .locator('video')
      .first()
      .isVisible({ timeout: 1000 })
      .catch(() => false));
...
test(
  "PLR-VID-05 / PLR-VID-11: CONFIRMED SYSTEMIC -- the same crash affects this resource, matching the workbook's multi-resource confirmation",
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { crashed } = await openVideoTrackingCrash(page, plr);
...
test(
  'PLR-VID-06: Standard playback controls function correctly once initialized (blocked if the crash reproduces)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    ...
    test.fail(
      crashed || !playerInitialized,
      'Blocked because the player never initialized this run (crashed flag: see the console.log above -- may be the confirmed targetContainer crash, or a different/unconfirmed non-init failure) -- player never initialized, so play/pause/seek/volume/fullscreen controls cannot be exercised'
    );
    if (crashed || !playerInitialized) {
      expect(playerInitialized).toBe(true);
      return;
    }
...
test(
  'PLR-VID-07: Whiteboard/annotation tools work correctly over a playing video (blocked if the crash reproduces)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { crashed, playerInitialized } = await openVideoTrackingCrash(page, plr);
    test.fail(
      crashed || !playerInitialized,
      'Blocked because the player never initialized this run (crashed flag: see the console.log above -- may be the confirmed targetContainer crash, or a different/unconfirmed non-init failure) -- no playing video surface exists to annotate over'
    );
    expect(playerInitialized).toBe(true);
  }
);
...
test(
  'PLR-VID-08: Video resumes from (or restarts from) the last watched position on reopen (blocked if the crash reproduces)',
  ...
  async ({ page }) => {
    ...
    test.fail(
      crashed || !playerInitialized,
      'Blocked because the player never initialized this run (crashed flag: see the console.log above -- may be the confirmed targetContainer crash, or a different/unconfirmed non-init failure) -- cannot watch partway and reopen to check resume behavior'
    );
    expect(playerInitialized).toBe(true);
  }
);
...
test(
  'PLR-VID-09: Switching away from a playing video stops playback cleanly (blocked if the crash reproduces)',
  { tag: ['@state-persistence', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const nav = new NavigationPage(page);
    ...
    test.fail(
      crashed || !playerInitialized,
      'Blocked because the player never initialized this run (crashed flag: see the console.log above -- may be the confirmed targetContainer crash, or a different/unconfirmed non-init failure) -- no active playback exists to confirm cleanly stops on class switch'
    );
    if (crashed || !playerInitialized) {
      expect(playerInitialized).toBe(true);
      return;
...
test(
  'PLR-VID-10: Video playback under a throttled/interrupted network shows a buffering/error state, never a silent freeze',
  ...
  async ({ page }) => {
    ...
    test.fail(
      crashed || !playerInitialized,
      'Blocked because the player never initialized this run (crashed flag: see the console.log above -- may be the confirmed targetContainer crash, or a different/unconfirmed non-init failure) -- no working playback exists to throttle/interrupt'
    );
    expect(playerInitialized).toBe(true);
  }
);
...
test(
  'PLR-VID-12: Video resources are identified via the type-icon selector; the Playlist\'s own "Video" filter may silently no-op',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const pl = new PlaylistPage(page);
...
test(
  'PLR-VID-13: "tcevideo" vs "video" are separate underlying resource types (mapping fact -- checked for a console/network hint)',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const urls = [];
    page.on('request', (req) => {
      if (/tcevideo|video/i.test(req.url())) urls.push(req.url());
    });
    await expect(plr.videoCards.first()).toBeAttached({ timeout: 10000 });
    await plr.openResourceCard(plr.videoCards);
...
test(
  'PLR-VID-14: RECONCILIATION -- is the crash scoped to tcevideo-type resources, or does it affect this confirmed resource too?',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { crashed } = await openVideoTrackingCrash(page, plr);
...
test(
  'PLR-VID-15: Opening a second video resource REPLACES the first instead of stacking',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await expect(plr.videoCards.first()).toBeAttached({ timeout: 10000 });
...
test(
  "PLR-VID-16: A video's chrome can show a misleading NaN duration during the loading-metadata window (blocked if the crash reproduces)",
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { crashed, playerInitialized } = await openVideoTrackingCrash(page, plr);
    test.fail(
      crashed || !playerInitialized,
      'Blocked because the player never initialized this run (crashed flag: see the console.log above -- may be the confirmed targetContainer crash, or a different/unconfirmed non-init failure) -- no metadata-loading window to observe on a player that never initializes'
    );
    expect(playerInitialized).toBe(true);
  }
);
...
test(
  'PLR-VID-17: No "Close All Resources" control exists anywhere in the Video player chrome or surrounding UI',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await expect(plr.videoCards.first()).toBeAttached({ timeout: 10000 });
...
test(
  'PLR-VID-18: Rapid double-click on a Video resource card does not open duplicate stacked instances',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await expect(plr.videoCards.first()).toBeAttached({ timeout: 10000 });
    await plr.videoCards.first().evaluate((el) => {
      el.click();
      el.click();
    });
...
test(
  "PLR-EXP-SEC-01: A video's underlying media stream URL cannot be tampered with to access unauthorized content",
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Needs real request-crafting/replay tooling against the video media stream/manifest URL -- not available in this Playwright-only browser-automation environment this pass. The confirmed hybrid-player crash (PLR-VID-02) also means no real media URL is even reachable on this account's available resources to inspect."
    );
...
test(
  'PLR-EXP-02 (Video variant): a video source returning a server error (5xx) shows a clear playback-error state, not an infinite spinner',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
...
test(
  ...
  async ({ page }) => {
    ...
    test.fail(
      crashed || !playerInitialized,
      'Blocked because the player never initialized this run (crashed flag: see the console.log above -- may be the confirmed targetContainer crash, or a different/unconfirmed non-init failure) -- cannot pause a video that never starts playing'
    );
    expect(playerInitialized).toBe(true);
  }
);
...
test(
  'PLR-EXP-20: A very short video does not break the seek bar or control layout (boundary, no short-video resource confirmed to exist)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { crashed, playerInitialized } = await openVideoTrackingCrash(page, plr);
