...
test(
  'PLR-STU-01: Launching a Student Test opens the correct assessment for the class (scoping unresolved -- see PLR-STU-02)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
...
test(
  'PLR-STU-02: SCOPING QUESTION -- what does "Student Tests" mean as a teacher-facing Players sub-type, if anything?',
  { tag: '@cross-cutting' },
  async ({ page }) => {
...
test(
  "PLR-EXP-SEC-10: Launching a Student Test does not expose another student's attempt via a guessable ID (blocked on PLR-STU-01)",
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Blocked on PLR-STU-01 (Student Test launch itself) remaining unresolved -- no reachable UI path exists yet to inspect student-attempt ID scoping against'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'PLR-EXP-12: Launching a Student Test for a zero-student class shows a clear message (blocked on PLR-STU-01)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Blocked on both PLR-STU-01 (Student Test launch itself, unresolved) and finding a genuinely zero-student class on this account'
    );
    expect(true).toBe(false);
  }
);
