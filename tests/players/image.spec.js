...
async function openImageTrackingCrash(page, plr) {
  ...
  const imageLoaded = await plr.imageGalleryImg
    .first()
...
test(
  'PLR-IMG-02: CRITICAL -- Image fails with the same hybrid-player crash confirmed on Video (re-check)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
...
test(
  'PLR-IMG-03: Zoom/pan controls work on a loaded image (blocked if the crash reproduces)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    ...
    test.fail(
      crashed || !imageLoaded,
      'Blocked because the player never initialized this run (crashed flag: see the console.log above -- may be the confirmed targetContainer crash, or a different/unconfirmed non-init failure) -- no loaded image exists to zoom/pan'
    );
    expect(imageLoaded).toBe(true);
  }
);
...
test(
  'PLR-IMG-04: Annotation tools work over a loaded image (blocked if the crash reproduces)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { crashed, imageLoaded } = await openImageTrackingCrash(page, plr);
    test.fail(
      crashed || !imageLoaded,
      'Blocked because the player never initialized this run (crashed flag: see the console.log above -- may be the confirmed targetContainer crash, or a different/unconfirmed non-init failure) -- no loaded image exists to annotate over'
    );
    expect(imageLoaded).toBe(true);
  }
);
...
test(
  'PLR-IMG-05: Confirmed Image DOM -- third-party gallery element with no data-qa-id inside (blocked if the crash reproduces)',
  ...
  async ({ page }) => {
    ...
    test.fail(
      crashed || !imageLoaded,
      'Blocked because the player never initialized this run (crashed flag: see the console.log above -- may be the confirmed targetContainer crash, or a different/unconfirmed non-init failure) -- no rendered .image-gallery element exists to inspect'
    );
    if (crashed || !imageLoaded) {
      expect(imageLoaded).toBe(true);
      return;
    }
...
test(
  'PLR-IMG-06: The image annotation overlay is real SVG with genuine path elements (blocked if the crash reproduces)',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { crashed, imageLoaded } = await openImageTrackingCrash(page, plr);
    test.fail(
      crashed || !imageLoaded,
      'Blocked because the player never initialized this run (crashed flag: see the console.log above -- may be the confirmed targetContainer crash, or a different/unconfirmed non-init failure) -- no loaded image exists to draw an annotation over'
    );
    expect(imageLoaded).toBe(true);
  }
);
...
test(
  'PLR-IMG-07: Only one confirmed Image resource exists -- size/orientation variety and multi-image-open are blocked',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    ...
    test.fail(
      ...
      'Content-availability gap, not a defect: only ' +
        count +
        ' Image resource(s) confirmed on this account/topic -- size/orientation variety and multi-image-open scenarios need more resources than exist here'
    );
...
test(
  "PLR-IMG-08: RECONCILIATION -- is the crash universal to all Image resources, or scoped like Video's split?",
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
...
test(
  "PLR-EXP-SEC-09: An Image resource's underlying asset URL cannot be manipulated to load an unauthorized asset",
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Needs real request-crafting tooling against the image asset URL, not available in this Playwright-only environment this pass. The confirmed hybrid-player crash (PLR-IMG-02) also means no genuinely loaded image asset URL is reachable to inspect on this account's available resources."
    );
...
test(
  'PLR-EXP-06: An Image resource that fails to load shows a clear broken-image state within the player chrome',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { crashed, imageLoaded } = await openImageTrackingCrash(page, plr);
...
test(
  'PLR-EXP-24: A very high-resolution image loads and displays without breaking zoom/pan (boundary, blocked -- no such resource confirmed, and the crash blocks the only one that exists)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    const { crashed, imageLoaded } = await openImageTrackingCrash(page, plr);
