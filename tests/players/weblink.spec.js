...
test(
  'PLR-WL-02: The central play icon overlay does not embed playback in-app',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWeblink(page, plr);
    ...
    test.fail(
      !overlayVisible,
      "No confirmed play-icon overlay element found on this weblink card (heuristic selector may not match this app's exact markup)"
    );
    if (!overlayVisible) {
    ...
    await page.waitForTimeout(1000);
    const iframeStillJustPreview =
      (await plr.weblinkIframe.count()) === 0 ||
      (await plr.weblinkIframe
        .first()
        .isVisible()
        .catch(() => false));
    console.log(
...
test(
  'PLR-WL-04: Clicking "Watch on YouTube" opens the video externally (new tab)',
  { tag: ['@positive', '@bug'] },
  async ({ page, context }) => {
    const plr = new PlayerPage(page);
    await openWeblink(page, plr);
...
test('PLR-WL-05: Purpose of the link/chain icon on the card', { tag: ['@ui-state', '@bug'] }, async ({ page }) => {
  ...
  test.fail(
    !chainVisible,
    "No confirmed chain-link icon element found on this weblink card (heuristic selector may not match this app's exact markup)"
  );
  if (!chainVisible) {
...
test(
  'PLR-WL-06: An invalid/dead weblink URL shows a graceful error (needs a broken-URL resource, not confirmed to exist)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'No Weblink resource with a confirmed broken/dead target URL exists in this curriculum this pass -- only one working Weblink (YouTube) resource is confirmed available'
    );
...
test(
  'PLR-WL-08: Confirmed Weblink DOM -- no data-qa-id on the iframe itself, imperatively appended',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWeblink(page, plr);
    const iframeQaId = await plr.weblinkIframe
      .first()
...
test(
  'PLR-WL-09: Manual-only check -- does the Pan tool block interaction with the embedded YouTube player? (needs a human, cross-origin iframe)',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'CONFIRMED cross-repo: this check is genuinely unreachable to any automated tool -- Playwright (like Cypress) cannot see into or verify interaction blocking on a cross-origin YouTube iframe. Needs a human live tester.'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'PLR-WL-10: Only one confirmed Weblink resource exists -- multi-weblink and framing-restricted-site scenarios are blocked',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    ...
    test.fail(
      ...
      'Content-availability gap, not a defect: only ' +
        count +
        " Weblink resource(s) confirmed on this account/topic -- multi-weblink switching and a framing-restricted (X-Frame-Options: DENY) site scenario both need resources that don't exist here"
    );
...
test(
  'PLR-EXP-SEC-06: A Weblink resource cannot navigate the embedding frame to an arbitrary URL (frame-busting)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const plr = new PlayerPage(page);
    await openWeblink(page, plr);
    const sandboxAttr = await plr.weblinkIframe
      .first()
      .getAttribute('sandbox')
      .catch(() => null);
    const allowAttr = await plr.weblinkIframe
      .first()
      .getAttribute('allow')
      .catch(() => null);
    ...
    test.fail(
      hasTopNavAllowed,
      "The Weblink iframe's sandbox attribute explicitly allows top-level navigation (allow-top-navigation) -- embedded content could hijack the parent app's navigation"
    );
...
test(
  'PLR-EXP-07: An invalid/dead Weblink URL is checked for an app-branded error vs. a raw browser error page (needs a broken-URL resource, not confirmed to exist)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Refines the already-documented PLR-WL-06 blocker -- same root cause: no Weblink resource with a confirmed dead URL exists in this curriculum this pass'
    );
    expect(true).toBe(false);
  }
);
