...
test(
  'PLR-BREAK-01: rapidly cycling through 3 different player types (Worksheet -> Weblink -> Image) with no settle wait leaves each one genuinely functional, not a corrupted/blank stack',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
...
test(
  'PLR-BREAK-02: closing a player the instant it opens (before its own content/loading spinner settles) does not leave the Playlist strip broken for the next open',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    ...
    const reopened = await plr
      .openResourceCard(plr.weblinkCards)
      .then(() => true)
      .catch(() => false);
    ...
    test.fail(
      !weblinkOpened,
      'Instant-closing a player right as it opens leaves the Playlist strip unable to open a different player type afterward'
    );
...
test(
  "PLR-BREAK-03: switching Class while a player is open closes/hides it cleanly, not leaving it stuck over the new class's whiteboard",
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const plr = new PlayerPage(page);
    ...
    const playerStillVisible = await plr.closeIcon
      .first()
...
test(
  'PLR-BREAK-04: opening the SAME resource card 6 times in immediate succession (beyond the existing double-click check) never accumulates more than one player instance',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const plr = new PlayerPage(page);
