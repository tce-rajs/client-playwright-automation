...
test(
  'DRP-ACCESS-02: The picker action card, panel close button, and pairing text/graphic all use the confirmed selectors',
  { tag: '@ui-state' },
  async ({ page }) => {
...
test(
  "DRP-QR-01: The pairing QR is a real per-open canvas render, not a static shared image (corrects the workbook's original finding)",
  { tag: '@negative' },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await openDropit(page, ar);
...
test(
  'DRP-SCAN-01: Scanning the QR with a phone and sending a file completes the transfer',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires a real physical phone to scan the Drop It QR and initiate a genuine file transfer -- browser automation cannot simulate a phone camera or a second physical device, and no such device is available in this environment'
    );
...
test(
  'DRP-RETRY-01: Retry control appears and works after a failed/interrupted transfer',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires a real physical phone to establish and then interrupt a genuine file transfer -- no such device is available in this environment'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'DRP-FILETYPE-01: An unsupported file type from the phone is rejected with a clear message',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires a real physical phone connected via Drop It to attempt sending an unsupported file type -- no such device is available in this environment'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'DRP-SIZE-01: An oversized file from the phone is rejected or handled gracefully',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires a real physical phone connected via Drop It to attempt sending an oversized file -- no such device is available in this environment'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'DRP-CONN-01: Losing network connectivity mid-scan/mid-transfer is handled gracefully',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires a real physical phone mid-transfer plus control over ITS network connectivity -- neither is available in this environment'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'DRP-DOUBLE-01: Rapidly opening and closing Drop It does not leave a stuck or duplicated panel',
  { tag: '@boundary' },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await openDropit(page, ar);
...
test(
  'DRP-STATE-01: Hard-refreshing while Drop It is open cleanly resets it with no stuck state',
...
test(
  'DRP-SEC-01: The pairing mechanism uses real per-session backend channels, not a purely client-side/static token',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    ...
    test.fail(
      wsOrChannelUrls.length === 0,
      "No real-time backend channel activity observed -- would mean the pairing UI has no live backend to scope a session against at all, matching the workbook's original (now-corrected, see DRP-QR-01) static-mock concern. Full cross-session-rejection verification still needs a second real device, which is not available here."
    );
...
test(
  'DRP-CONVERGE-01: Entry point, pairing UI, and idle-state Close are all independently confirmed reachable and functional',
  { tag: '@cross-cutting' },
  async ({ page }) => {
...
test(
  'DRP-EXP-01: The three status lines update visibly if a real device connects (not just static defaults)',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      'Requires a real phone to attempt an actual scan and observe whether Connection/Transfer/Upload status text changes from its default values -- no such device is available in this environment'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'DRP-EXP-02: Leaving the panel open and idle does not desync its state from a fresh reopen',
  { tag: '@boundary' },
  async ({ page }) => {
...
test(
  'DRP-EXP-03: A future dynamic pairing token would need to be rejected across sessions/accounts',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      true,
      'The underlying premise (DRP-QR-01\'s "static QR") no longer holds per this session\'s live findings, but verifying cross-session token rejection still needs a second real device/account to attempt an actual pairing replay -- not available in this environment'
    );
    expect(true).toBe(false);
  }
);
