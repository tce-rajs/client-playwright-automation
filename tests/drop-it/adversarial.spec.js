...
test(
  'DRP-BREAK-01: switching class immediately after opening Drop It tears down its panel and Firestore connection cleanly, not leaving a stuck overlay',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    ...
    await nav
      ...
      .catch((e) => console.log('resetToClass threw:', e.message.split('\n')[0]));
    ...
    test.fail(
      ...
      "Switching class while Drop It is open leaves its panel/QR canvas stuck visible over the new class's whiteboard"
    );
...
test(
  'DRP-BREAK-02: rapidly opening and closing Drop It 8 times does not accumulate an unbounded number of Firestore realtime connections',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      firestoreRequestCount > 400,
      `8 rapid Drop It open/close cycles generated an excessive ${firestoreRequestCount} Firestore-related requests, suggesting connections are not being cleanly closed`
    );
...
test(
  'DRP-BREAK-03: two browser tabs on the same account opening Drop It at the same time each get their own independent QR code, not a shared/corrupted one',
  { tag: ['@security', '@bug'] },
  async ({ page, context }) => {
    test.setTimeout(45000);
    ...
    if (opened2) {
      ...
      test.fail(
        dataUrl1 !== null && dataUrl1 === dataUrl2,
        'Two concurrently-open Drop It panels (even in different tabs of the same account) render the IDENTICAL QR code instead of independent per-session codes'
      );
...
test(
  'DRP-BREAK-04: an extremely small viewport (375px mobile width) does not break the Drop It panel layout',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
