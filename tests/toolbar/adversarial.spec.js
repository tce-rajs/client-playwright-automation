...
test(
  'TB-BREAK-01: a 2000-character text object does not crash the canvas or corrupt its own rendering',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
...
test(
  'TB-BREAK-02: emoji + Arabic (RTL) text in a text object commits and renders without crashing the canvas',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
...
test(
  'TB-BREAK-03: rapidly alternating Undo/Redo clicks 16 times does not corrupt the stroke count or crash the canvas',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const tb = new ToolbarPage(page);
    ...
    test.fail(
      finalCount < 0,
      'Rapid Undo/Redo alternation corrupts the stroke count into a nonsensical negative value'
    );
...
test(
  'TB-BREAK-04: switching from Pen to Eraser mid-drag (before mouseup) does not leave a stray half-drawn stroke or crash',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    test.setTimeout(30000);
    const tb = new ToolbarPage(page);
...
test(
  'TB-BREAK-05: rapidly clicking through 5 different Background options in quick succession settles on exactly ONE final background, not a corrupted/stacked state',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.setTimeout(30000);
    const tb = new ToolbarPage(page);
    ...
    test.fail(
      ...
      'Rapidly clicking through multiple Background options leaves more than one marked active, or crashes the canvas'
    );
