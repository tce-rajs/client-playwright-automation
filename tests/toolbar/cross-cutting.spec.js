...
test(
  'TB-SEC-01: Whiteboard content is scoped per class/topic and never leaks across classes',
  { tag: '@security' },
  async ({ page }) => {
    ...
    await pl.openContentsPopup();
    await pl.topicItems
      .filter({ hasText: originalTopic.replace(/^\d+\.\d+\s*\|\s*/, '') })
      .first()
      .click()
      .catch(() => {});
    await page.waitForTimeout(1000);
...
test(
  'TB-STATE-01: Drawn content persists correctly across a page refresh',
