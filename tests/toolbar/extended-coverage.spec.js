...
test.describe('Extended coverage (gap-analysis pass)', () => {
  ...
  test(
    'TB-CYP-01: A single tap only selects a tool; a double tap is required to open its panel',
    { tag: ['@ui-state', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
  ...
  test(
    'TB-CYP-02: Clear Whiteboard control is reachable but its clearing effect is unconfirmed',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      ...
      test.fail(
        !clearBtnVisible,
        'Clear Whiteboard control (toolbar-eraser-clear-whiteboard) not found in the Eraser panel this pass'
      );
      if (!clearBtnVisible) {
  ...
  test(
    'TB-CYP-03: Eraser cannot reliably remove a Pen stroke once it exceeds roughly 700px',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      ...
      for (let i = 0; i <= 10; i++) {
        ...
        await page.mouse.move(
          box.x + longStrokeStart.x + (longStrokeEnd.x - longStrokeStart.x) * t,
          box.y + longStrokeStart.y + (longStrokeEnd.y - longStrokeStart.y) * t
        );
      }
      await page.mouse.up();
      await page.waitForTimeout(800);
  ...
  test(
    'TB-CYP-04: Erasing one word of connected cursive handwriting deletes an adjacent word too',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
  ...
  test(
    'TB-CYP-05: Opening a tool panel deactivates the previous tool; dismissing reverts to Select',
    { tag: '@ui-state' },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      ...
      const selectActive = await tb
        .isToolActive('gtSelect')
        .isVisible()
        .catch(() => false);
      const penActive = await tb
        .isToolActive('gtPen')
        .isVisible()
        .catch(() => false);
      console.log(
  ...
  test(
    "TB-CYP-06: Clicking a tool's icon again while its own panel is open closes the panel",
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      await tb.openToolPanel('gtZoom');
      ...
      test.fail(
        ...
        'Clicking a tool\'s own icon again while its panel is open does NOT close the panel (neither a single nor a repeated click did) -- only tapping outside the panel closes it, contradicting this case\'s expected "click again to close" behavior'
      );
  ...
  test(
    'TB-CYP-07: Undo skips a Move entirely and undoes the next-older tracked action instead',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      ...
      test.fail(
        countAfterUndo < countAfterDrawTwo,
        'Undo skipped past the most recent Move action and undid an older Draw action instead -- can look like unexpected data loss'
      );
  ...
  test(
    'TB-CYP-08: No gesture reopens the editor on already-committed text',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
  ...
  test(
    'TB-CYP-09: Bold produces no actual computed font-weight change',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
  ...
  test(
    'TB-CYP-10: No drag-to-resize handles exist on a selected object',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
  ...
  test(
    'TB-GAP-01: Long-press gestures cannot be triggered by synthetic browser automation (confirmed tooling limit)',
    { tag: ['@cross-cutting', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        'Confirmed hard automation-tooling limit: Playwright has discrete click/drag primitives with no press-and-hold gesture, so any feature gated specifically behind a long-press duration cannot be exercised here -- not an un-attempted case, would need a real physical device/manual test to close'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'TB-GAP-02: The toolbar dock toggle reaches a real LEFT-docked state, not a cosmetic shift',
    { tag: '@positive' },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      const rightDockedBefore = await page
      ...
      await page.waitForTimeout(800);
      const leftDockedAfter = await page
  ...
  test(
    'TB-GAP-03: A Pen colour/size picker and Eraser size control are reachable via double-click',
    { tag: '@positive' },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
  ...
  test(
    'TB-GAP-04: Absolute-pixel-position assertions are reliable in an isolated, single check',
    { tag: '@cross-cutting' },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
  ...
  test(
    'TB-MAGNET-01: Magnet is the real, gated trigger for Notice/Learning Shorts/Homework/Attendance panels (regression re-check)',
    { tag: '@positive' },
    async ({ page }) => {
      ...
      const noticeVisible = await page
        ...
        .catch(() => false);
      const shortsVisible = await page
        .getByText('Learning Shorts', { exact: false })
        .first()
        .isVisible({ timeout: 1000 })
        .catch(() => false);
      const homeworkVisible = await page
        .getByText('Homework', { exact: false })
        .first()
        .isVisible({ timeout: 1000 })
        .catch(() => false);
      const attendanceVisible = await page
        .getByText('Attendance', { exact: false })
        .first()
        .isVisible({ timeout: 1000 })
        .catch(() => false);
      console.log(
  ...
  test(
    'TB-SHAPE-01: The Shapes tool inserts a selectable geometric shape onto the canvas',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
  ...
  test(
    'TB-STATE-02: Rapid consecutive edits immediately followed by a refresh can lose the most recent unsaved changes',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      ...
      const countBeforeRefresh = await tb.wbContainer
  ...
  test(
    'TB-EXP-02: A failed autosave shows a visible failure indicator, not a false-success toast',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      ...
      const failureIndicatorShown = await page
  ...
  test(
    'TB-EXP-03: A zero-size Shape drag (click without dragging) does not insert a degenerate object',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
  ...
  test(
    'TB-EXP-04: Creating a Text object and clicking away without typing leaves no empty object behind',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
  ...
  test(
    'TB-EXP-05: The Pen size slider at its maximum ("Strong") draws a correct, non-corrupted stroke',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      await tb.openToolPanel('gtPen');
  ...
  test(
    'TB-EXP-06: The Eraser Size slider at maximum erases without over-erasing beyond its visible radius',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
  ...
  test(
    'TB-EXP-07: Zooming to the minimum level keeps toolbar controls and the context bar legible/clickable',
    { tag: '@boundary' },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      await tb.openToolPanel('gtZoom');
      ...
      await page.waitForTimeout(500);
      await tb.closePanelByTappingOutside();
      const contextBarVisible = await page
  ...
  test(
    'TB-EXP-08: Rapidly toggling the dock 10+ times settles cleanly with no broken intermediate state',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const toggleBtn = page.locator('.leftRightBtn.left button, .leftRightBtn.right button').first();
      for (let i = 0; i < 12; i++) {
      ...
      await page.waitForTimeout(800);
      const leftDocked = await page
        .locator('.toolbar-container.left')
        .isVisible()
        .catch(() => false);
      const rightDocked = await page
        .locator('.toolbar-container.right')
        .isVisible()
        .catch(() => false);
  ...
  test(
    'TB-EXP-09: The Widgets browser stays functional when its Discipline filter is switched rapidly',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      ...
      const options = await tb.widgetDisciplineSelect
        .locator('option')
        .allTextContents()
        .catch(() => []);
  ...
  test(
    'TB-EXP-10: Inserting 50+ Shape objects does not degrade canvas selection/drag performance',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        await page
          .getByText('Rectangle', { exact: false })
          .first()
          .click({ force: true })
          .catch(() => {});
      ...
      test.fail(
        selectDuration > 5000,
        'Selecting an object after 50+ Shape insertions took over 5 seconds -- real performance degradation'
      );
  ...
  test(
    'TB-EXP-11: The zoom percentage indicator matches actual rendered scale after rapid zoom clicks',
    { tag: '@boundary' },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      await tb.openToolPanel('gtZoom');
      ...
      await page.waitForTimeout(500);
      const displayedPercent = await page
        .locator('text=/%/')
        .first()
        .textContent()
        .catch(() => null);
  ...
  test(
    'TB-EXP-12: The Undo stack has reasonable depth -- 20+ sequential actions do not run out prematurely',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      ...
      for (let i = 0; i < 20; i++) {
        await tb.drawStroke(
          { x: basePoint.x + i * 15, y: basePoint.y },
          { x: basePoint.x + i * 15 + 10, y: basePoint.y + 10 }
        );
      }
      await page.waitForTimeout(500);
  ...
  test(
    'TB-EXP-13: A widget dragged from the browser lands at the drop location, not a fixed default position',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      ...
      const tileVisible = await widgetTiles
        .first()
  ...
  test(
    'TB-EXP-14: CRITICAL -- Whiteboard History leaks content across classes/chapters (global per-account, not per-class)',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      const nav = new NavigationPage(page);
      ...
      test.fail(
        !historyBtnVisible,
        'Whiteboard History entry point not found this pass -- cannot directly confirm the cross-class leak, but the workbook already documents this as a confirmed critical finding'
      );
  ...
  test(
    'TB-EXP-15: A forged autosave payload targeting an unauthorized class/topic ID is rejected (blocked -- no forging tooling)',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Same blocker class as NAV-SEC-01/EXP-06 elsewhere in this suite -- needs the autosave request's exact shape plus a known unauthorized class/topic ID, neither available without a second reference account"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    "TB-EXP-16: Pen/Eraser tool-preference panels do not leak another account's saved preferences",
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      const tb = new ToolbarPage(page);
      await tb.openToolPanel('gtPen');
  ...
  test(
    'TB-EXP-17: A forged Clear Whiteboard request cannot target a different class than the active one (blocked -- no forging tooling)',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        "Same blocker as TB-EXP-15 -- needs the Clear Whiteboard request's exact shape plus a target ID for a different, known class, neither available without a second reference account"
      );
      expect(true).toBe(false);
    }
  );
