...
test(
  'TB-OBJ-01: Clicking an inserted shape selects it with manipulation controls',
  { tag: '@positive' },
  async ({ page }) => {
...
test(
  'TB-OBJ-02: Deleting a canvas object has no confirmation step',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const tb = new ToolbarPage(page);
    ...
    const confirmDialogAppeared = await page
