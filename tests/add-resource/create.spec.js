...
test(
  'ADD-CRT-01: Create form shows Title, Grade & Subject, Chapter & Topic, File, and Share',
  { tag: '@ui-state' },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
...
test(
  'ADD-CRT-02: Grade & Subject and Chapter & Topic are pre-filled and read-only',
  { tag: '@positive' },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
...
test(
  'ADD-CRT-04: Typing fewer than 3 characters in Title shows a real-time error',
  { tag: '@positive' },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
...
test(
  'ADD-CRT-06: Submitting a completely untouched, empty Title shows no validation feedback',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
...
test(
  'ADD-CRT-07: Submitting a valid Title with no File selected shows no validation feedback for the missing File',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    await ar.titleInput.fill('abc');
    ...
    await ar.submitBtn.click({ force: submitDisabled }).catch(() => {});
    await page.waitForTimeout(500);
    const fileErrorVisible = await ar.createForm
...
test(
  'ADD-CRT-09: A fully valid submission creates the resource and attaches it to the current Topic',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
...
test(
  'ADD-CRT-10: A file larger than the stated 10MB limit is rejected',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
...
test(
  'ADD-CRT-12: Turning Share OFF before submitting is respected',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      true,
      'Toggle-OFF state itself verified; the actual created-resource privacy behavior needs a real submit, deliberately not executed on the shared QA account'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'ADD-CRT-13: A Title containing script/HTML markup is stored and rendered as literal text',
  { tag: '@security' },
  async ({ page }) => {
