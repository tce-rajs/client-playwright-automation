...
test(
  'ADD-STATE-01: Newly added resources appear in the Playlist strip immediately, without a manual refresh',
  { tag: '@negative' },
  async ({ page }) => {
    const pl = new PlaylistPage(page);
    ...
    await ar.fileInput.setInputFiles({
      name: 'autotest.txt',
      mimeType: 'text/plain',
      buffer: Buffer.from('automated test file'),
    });
...
test(
  'ADD-NET-01: The resource-creation/upload request fails mid-submit',
  ...
  async ({ page }) => {
    ...
    test.fail(
      ...
      'A failed create-resource request silently closes the form as if it succeeded, with no error shown'
    );
...
test(
  'ADD-NET-02: A Library search request itself fails (not just zero results)',
  { tag: ['@cross-cutting', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    ...
    test.fail(
      ...
      'A failed search request shows the same "No result found" empty state as a legitimate zero-result search -- indistinguishable from each other'
    );
...
test(
  'ADD-RACE-01: Rapidly double-clicking Submit on a valid Create form',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
...
test(
  "ADD-SEC-01: Added resources are scoped to the correct class/topic and never leak into another class's playlist",
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      true,
      'No second reference account available to cross-check for cross-class/cross-teacher resource leakage'
    );
...
test(
  'ADD-SEC-02: Tampering the create-resource request to target a different chapterId/topicId than shown in the UI',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      true,
      "Not executed against the live shared QA backend -- a successful tamper (the exact defect being tested for) would create an orphaned resource with no safe way to clean it up from this account's own UI. Needs a dedicated pass with a disposable account."
    );
    expect(true).toBe(false);
  }
);
