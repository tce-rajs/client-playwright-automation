...
test.describe('Core (ADD-CORE-01..04)', () => {
  ...
  test(
    'ADD-CORE-03: Reopening "+" while a source popup is already open stacks a second picker instead of closing the first',
    { tag: ['@ui-state', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
      ...
      const galleryStillOpen = await page
        .locator('[data-qa-id="gallery-close-btn"]')
        .isVisible()
        .catch(() => false);
      const pickerAlsoOpen = await page
  ...
  test(
    'ADD-CORE-04: Closing an individual resource-source popup via its own close control',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
...
test.describe('Extended coverage (gap-analysis pass)', () => {
  ...
  test(
    'ADD-WB-01: The Whiteboard action card opens a Save/Download card for the CURRENT board, not a new whiteboard',
    { tag: '@ui-state' },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
  ...
  test(
    'ADD-XREF-01: Gallery, Drop It, AI Assist, and Library are each covered in their own dedicated module workbooks',
    { tag: '@cross-cutting' },
    async ({ page }) => {
  ...
  test(
    'AR-CYP-01: Chapter & Topic / Grade & Subject fields are dead-code-disabled, not just visually read-only',
    { tag: '@cross-cutting' },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
      ...
      const pickerOpened = await page
        .locator('[data-qa-id="playlist-chapter-tp-popup"], [data-qa-id="playlist-select-chapter"]')
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test(
    'AR-CYP-02: Resubmitting an identical Title + filename silently de-duplicates',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      ...
      const duplicateWarningShown = await ar.createForm
  ...
  test(
    'AR-CYP-03: A newly created resource auto-opens its own preview immediately after Submit',
    { tag: ['@ui-state', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        'Verifying the post-submit auto-preview needs a real Submit, deliberately not executed on the shared QA account (same reasoning as ADD-CRT-09)'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'AR-CYP-04: Not every attached resource can be removed by the teacher through any UI path',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      ...
      if (nativeCount > 0) {
        ...
        nativeRemovable = await pl.resourceRemoveBtn
          .first()
      ...
      if (assetOverflowCount > 0) {
        await pl.assetOverflowIconBtn
          .first()
          .click({ force: true, timeout: 3000 })
          .catch(() => {});
        await page.waitForTimeout(400);
        assetRemovable = await pl.assetRemoveBtn
          .first()
          .isVisible({ timeout: 2000 })
          .catch(() => false);
  ...
  test(
    "AR-CYP-05: Drop It's Close button can visually cover the Add Resource FAB if left open",
    { tag: '@cross-cutting' },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
  ...
  test(
    'AR-CYP-06: Oversized file is rejected with the exact confirmed error text',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
  ...
  test(
    "AR-CYP-07: Whiteboard's Save to Playlist action is unreachable/non-functional; Download PDF is the only working save action",
    ...
    async ({ page }) => {
      ...
      if (savePlaylistVisible) {
        ...
        await page.waitForTimeout(1000);
        saveClickWorked = await page
          .getByText(/saved|added to playlist/i)
  ...
  test(
    'AR-GAP-01: Overflow-menu responsiveness at 200+ accumulated assets',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
  ...
  test(
    'AR-GAP-02: Toggling "Replace File" ON reveals a hidden File input on Edit',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const pl = new PlaylistPage(page);
      ...
      for (let i = 0; i < Math.min(count, 15) && !editOpened; i++) {
        const overflow = cards
          .nth(i)
          .locator('[data-qa-id="playlist-resource-overflow-btn"], button:has(mat-icon:has-text("more_vert"))')
          .first();
        ...
        if (await editOption.isVisible({ timeout: 1500 }).catch(() => false)) {
          ...
          await page.waitForTimeout(800);
          editOpened = await page
            .locator('app-add-custom-asset .add-custom-asset')
            .isVisible({ timeout: 3000 })
            .catch(() => false);
        } else {
          await page.keyboard.press('Escape');
  ...
  test(
    "AR-CYP-08: The custom-asset endpoint rejects a tampered chapterId/topicId not matching the teacher's authorized context",
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        true,
        'No known foreign chapterId/topicId available to substitute, and forging the raw create/upload request needs its exact shape captured live — not available without a second reference account'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'ADD-EXP-01: Submitting Create with a Title but no Chapter & Topic is blocked with a clear validation message',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
  ...
  test(
    'ADD-EXP-02: Choosing a file then changing Grade & Subject does not orphan the already-selected file',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
  ...
  test(
    'ADD-EXP-03: A Library search that legitimately returns zero results shows a clear "no results" state',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
  ...
  test(
    'ADD-EXP-04: The 10MB file-size limit is enforced (client-side check confirmed; same as AR-CYP-06/ADD-CRT-10)',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
      ...
      const errorVisible = await ar.createForm
      ...
      test.fail(
        ...
        'The file-size limit is not enforced even client-side for a file just over 10MB'
      );
  ...
  test(
    'ADD-EXP-05: A Library search of only whitespace is treated the same as an empty query',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
      ...
      const treatedAsValidSearch = await page
  ...
  test(
    'ADD-EXP-06: The Create Title field has no maxlength, and a 200+ char title does not break the layout once created',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
      ...
      test.fail(
        true,
        'Field-level maxlength confirmed absent above; verifying the resulting resource CARD does not break layout needs a real submit, deliberately not executed on the shared QA account'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'ADD-EXP-07: Rapidly clicking between source cards before any fully opens settles on exactly the last-clicked source',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
      ...
      const galleryOpen = await page
        .locator('[data-qa-id="gallery-close-btn"]')
        .isVisible()
        .catch(() => false);
      const aiAssistOpen = await page
        .locator('[data-qa-id="ai-assist-close-btn"]')
        .isVisible()
        .catch(() => false);
      ...
      test.fail(
        ...
        `Expected exactly the last-clicked source (AI-Assist) open; got ${openCount} source(s) open, AI-Assist open: ${aiAssistOpen}`
      );
  ...
  test(
    "ADD-EXP-08: The Share toggle's ON default is preserved after other form interactions",
    { tag: '@boundary' },
    async ({ page }) => {
      const ar = new AddResourcePage(page);
      await ar.openPicker();
