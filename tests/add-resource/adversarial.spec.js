...
test(
  'AR-BREAK-01: submitting Create with a zero-byte (empty) file is rejected or handled gracefully, not silently accepted as valid',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    ...
    const fileErrorVisible = await page
      .locator('.invalid-file')
      .first()
    ...
    if (submitEnabled) {
      ...
      test.fail(
        !formStillOpen,
        'CONFIRMED: a zero-byte file submission silently succeeds (form closes) with no validation feedback at all -- likely creates a broken/empty resource on the Playlist'
      );
...
test(
  'AR-BREAK-02: a double-extension filename ("notes.pdf.exe") is evaluated by real content/type, not just a naive extension string match',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    ...
    await ar.fileInput.setInputFiles({
      ...
      buffer: Buffer.from('MZ fake exe header bytes for adversarial testing'),
    });
    ...
    test.fail(
      ...
      'A file named "notes.pdf.exe" (double extension, ending in a disallowed type) is accepted with no validation error -- suggests filename-suffix matching rather than a real type check'
    );
...
test(
  'AR-BREAK-03: an emoji + 150-character filename does not crash the Create form or corrupt the displayed filename',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const ar = new AddResourcePage(page);
    ...
    await ar.fileInput.setInputFiles({
      ...
      buffer: Buffer.from('%PDF-1.4 fake pdf content for adversarial testing'),
    });
    await page.waitForTimeout(500);
    ...
    test.fail(
      ...
      'An extreme emoji+150-character filename crashes the Create form or breaks its layout'
    );
...
test(
  'AR-BREAK-04: clicking where the first source card WILL be, immediately (0ms) after clicking the "+" FAB, does not misfire on stale/wrong content',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    ...
    const clickResult = await ar.actions.create
    ...
    test.fail(
      !noOtherSourceAccidentallyOpened,
      'Clicking the instant the Add Resources FAB opens can misfire onto the WRONG source card (opened Library instead of/alongside Create)'
    );
...
test(
  "AR-BREAK-05: two browser tabs on the same account both submitting a Create resource to the same Topic at nearly the same instant do not corrupt each other's upload",
  ...
  async ({ page, context }) => {
    ...
    await ar1.fileInput.setInputFiles({
      name: 'concurrent-a.pdf',
      mimeType: 'application/pdf',
      buffer: Buffer.from('%PDF-1.4 concurrent A'),
    });
    ...
    await ar2.fileInput.setInputFiles({
      name: 'concurrent-b.pdf',
      mimeType: 'application/pdf',
      buffer: Buffer.from('%PDF-1.4 concurrent B'),
    });
    ...
    test.fail(
      !aVisible || !bVisible,
      'Two nearly-simultaneous Create submissions from two tabs on the same account result in one of them silently missing/lost instead of both landing'
    );
