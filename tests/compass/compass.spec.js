...
test(
  'CMP-TRIG-01: The Compass floating trigger button renders and opens a real popover in Teaching mode',
  { tag: '@ui-state' },
  async ({ page }) => {
...
test(
  'CMP-TRIG-02: AnalyseIt only renders if the teacher is assigned to the current class/subject',
  { tag: '@security' },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.openTrigger();
...
test(
  'CMP-TRIG-03: A "no homework" message renders gracefully when AnalyseIt has no assignment data',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.openTrigger();
...
test(
  'CMP-TRIG-04: AnalyseIt detail-view navigation controls work when real assignment data exists',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.openTrigger();
    ...
    test.fail(
      !listBtnVisible,
      'No real assignment data exists on this class/subject this pass -- cannot exercise the detail-view list/questions/cancel navigation chain'
    );
    if (!listBtnVisible) {
...
test(
  'CMP-TRIG-05: ExploreIt renders real widget tiles when the chapter has 1+ widgets',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.openTrigger();
...
test(
  'CMP-TRIG-05B: Zero-widget chapter rendering (ExploreIt) is checked on a chapter likely to have none',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const nav = new NavigationPage(page);
    const cmp = new CompassPage(page);
...
test(
  'CMP-TRIG-06: Revision Tests entry is gated on enableStudentTest AND studentTests.length > 0',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.openTrigger();
    ...
    test.fail(
      true,
      'Isolating enableStudentTest and studentTests.length independently needs specific test data (a class with exactly one flag true) not identified in this account this pass -- only the combined real-world state was observed'
    );
...
test(
  'CMP-TRIG-07: Assignment Questions pagination controls have no stable selectors (confirmed tech debt)',
  ...
  async ({ page }) => {
    ...
    test.fail(
      nextCount === 0,
      'No Assignment Questions pagination controls reachable this pass -- no real assignment data exists in this account to paginate through, so the confirmed tech-debt finding below cannot be re-checked'
    );
    if (nextCount === 0) {
    ...
    test.fail(
      dataQaIdCount === 0,
      'CONFIRMED tech debt: Assignment Questions pagination controls have no stable data-qa-id, only fragile title/class attributes -- inconsistent with every other Compass/Player control in this app'
    );
...
test(
  'CMP-RECONCILE-01: Teaching-mode trigger and Planning-mode Question Bank are two separate, non-overlapping surfaces',
  { tag: '@cross-cutting' },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await expect(cmp.triggerBtn).toBeVisible({ timeout: 10000 });
...
test(
  "CMP-ACCESS-01: Planning mode's Question Bank is reachable via the profile popover's Classroom Mode switch",
  { tag: '@positive' },
  async ({ page }) => {
    const cmp = new CompassPage(page);
...
test(
  'CMP-QBANK-01: Question cards display with metadata (difficulty, category, source tag)',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.switchToPlanningMode();
...
test(
  'CMP-QBANK-02: Clicking a question card opens a preview/answer dialog',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.switchToPlanningMode();
    ...
    await page.waitForTimeout(800);
    const submitAnswerVisible = await page
    ...
    test.fail(
      !submitAnswerVisible,
      'Clicking the first question card did not open a preview dialog with a "Submit Answer" button this pass -- likely a non-MCQ/CUSTOM question type whose preview offers different actions, not confirmed as a dialog failure'
    );
...
test(
  "CMP-QBANK-03: A question card's checkbox hit target is small/finicky (clicking near it opens the preview instead)",
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.switchToPlanningMode();
...
test(
  'CMP-QUIZ-01: Create Quiz flow works end-to-end up to (not including) a real Add Quiz submit',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.switchToPlanningMode();
...
test(
  'CMP-REVTEST-01: Create Revision Test shows a validation error for a non-STEM subject (Accountancy)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.switchToPlanningMode();
...
test(
  'CMP-EXP-03: Revision Test on a different non-STEM subject fails with the SAME error message (cross-feature consistency check)',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
...
test(
  'CMP-ANALYSEIT-01: AnalyseIt is located and shown to render real content or a real empty state',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.openTrigger();
    ...
    test.fail(
      !hasContent,
      'Clicking AnalyseIt (labeled "No Homework") hides the top-level empty-state banner without opening any detail view in its place -- same confirmed finding as CMP-TRIG-03'
    );
...
test(
  'CMP-EXPLOREIT-01: ExploreIt is located inside the Planning-mode workspace area',
  { tag: '@positive' },
  async ({ page }) => {
...
test(
  'CMP-EXPLOREIT-02: ExploreIt\'s "Open Widgets" link opens a broader, cross-discipline widget browser (not chapter-locked)',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.openTrigger();
    ...
    await page.waitForTimeout(1000);
    const disciplineSelectVisible = await page
      .getByText(/discipline/i)
      .isVisible({ timeout: 3000 })
      .catch(() => false);
...
test(
  'CMP-ASSIGN-01: Assignment list shows created quizzes/revision tests',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.openTrigger();
...
test(
  'CMP-ASSIGN-02: Assignment details view shows per-student results',
  { tag: ['@positive', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Needs an assignment already given to students with at least one response -- no such assignment exists in this account's reachable data this pass"
    );
    expect(true).toBe(false);
  }
);
...
test(
  'CMP-CLASSMODE-01: Switching Teaching <-> Planning mode may change the active class unexpectedly',
...
test(
  'CMP-ADV-01: Rapid double-click on AnalyseIt before data loads opens only one detail view',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.openTrigger();
...
test(
  'CMP-ADV-02: Compass trigger visibility for a class the teacher is not assigned to (cross-tenant)',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    ...
    test.fail(
      true,
      'No known limited-assignment teacher account available in this project to identify an unassigned Grade/Subject combination against -- same blocker as NAV-SEC-03'
    );
    expect(true).toBe(false);
  }
);
...
test(
  'CMP-ADV-03: The Compass detail view survives a hard refresh cleanly',
...
test(
  'CMP-EXP-01: A network failure while AnalyseIt is loading shows a clear error, not indefinite blank state',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    ...
    await cmp.analyseItItem.click({ force: true });
    await page.waitForTimeout(1500);
    const errorStateVisible = await page
    ...
    test.fail(
      !errorStateVisible,
      'A blocked AnalyseIt fetch shows no explicit error/retry state -- either silently falls back to the same "no homework" empty-state message (indistinguishable from a real failure) or, per the confirmed CMP-TRIG-03/CMP-ANALYSEIT-01 finding, shows neither because clicking AnalyseIt already dismissed that banner'
    );
...
test(
  "CMP-EXP-02: A widget that fails to load in ExploreIt's browser shows a clear broken-widget state",
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    ...
    test.fail(
      widgetTiles === 0,
      'No widget tiles reachable to attempt opening a failing one this pass -- the widget-list fetch itself may have been blocked by the same route interception'
    );
...
test(
  'CMP-EXP-04: AnalyseIt data is scoped strictly to the current class/subject, no cross-class bleed',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Needs a class with real, completed, analyzable homework data to compare against a second class -- AI Homework's Ready to Send was deliberately never clicked elsewhere in this suite (destructive-action avoidance), so no such data exists yet"
    );
    expect(true).toBe(false);
  }
);
...
test(
  'CMP-EXP-05: Rapidly clicking the "Homework?" create-link in AnalyseIt\'s empty state opens only one composer',
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.openTrigger();
...
test(
  'CMP-EXP-06: A chapter/topic with many ExploreIt widgets remains scrollable without layout breakage',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await cmp.openTrigger();
    ...
    const overflowInfo = await widgetList
      .evaluate((el) => ({
    ...
    test.fail(
      !scrollableOrFits,
      'ExploreIt widget list overflows its container without a scroll affordance (overflow-x: visible) -- widgets beyond the fold would be unreachable/clipped'
    );
...
test(
  'CMP-EXP-07: The floating compass-trigger-btn remains clickable at a mobile (375px) viewport width',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
...
test(
  'CMP-EXP-08: Revision Tests visibility with each gate flag (enableStudentTest, studentTests.length) tested independently',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.fail(
      true,
      "Needs a class/subject with exactly one of the two gate flags true and the other false -- no such isolated test data identified in this account's reachable classes this pass"
    );
    expect(true).toBe(false);
  }
);
