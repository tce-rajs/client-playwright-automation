...
test(
  'CMP-BREAK-01: rapidly clicking the Compass trigger 6 times in quick succession never stacks more than one open popover',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
...
test(
  'CMP-BREAK-02: pressing the browser Back button while the Compass popover is open does not leave a stuck overlay behind',
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    const cmp = new CompassPage(page);
    await expect(cmp.triggerBtn).toBeVisible({ timeout: 10000 });
    ...
    if (!opened) {
      ...
      test.fail(
        true,
        'Compass trigger/popover could not be opened this run, so the Back-button-while-open scenario could not be exercised'
      );
    ...
    const menuStillShowing = await page
      .locator('.compass-menu.open')
      .isVisible({ timeout: 2000 })
      .catch(() => false);
    const pageUsable = await page
      .locator('body')
      .isVisible()
      .catch(() => false);
    const url = page.url();
    ...
    test.fail(
      ...
      'Pressing Back while the Compass popover is open leaves a stuck overlay on an otherwise unusable page'
    );
...
test(
  "CMP-BREAK-03: an extreme 250-character Quiz title in Planning mode's Create Quiz form does not break the form layout",
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    ...
    if (createQuizVisible) {
      ...
      if (titleVisible) {
        ...
        test.fail(
          true,
          'Quiz title input was not reachable this run (Create Quiz clicked but its title field never appeared) -- could not exercise the 250-char boundary case'
        );
      ...
      test.fail(
        true,
        "The Create Quiz control was not reachable this run (Planning mode's Question Bank sidebar did not surface it) -- could not exercise the 250-char Quiz title boundary case"
      );
...
test(
  'CMP-BREAK-04: two full rapid Teaching<->Planning mode switch cycles back to back do not compound into a broken state',
...
test(
  'CMP-BREAK-05: an HTML/script-tag string in the Create Quiz title field is treated as literal text, never executed',
  { tag: ['@security', '@bug'] },
  async ({ page }) => {
    test.setTimeout(60000);
    const cmp = new CompassPage(page);
    ...
    if (createQuizVisible) {
      ...
      if (titleVisible) {
        ...
        test.fail(
          true,
          'Quiz title input was not reachable this run (Create Quiz clicked but its title field never appeared) -- could not exercise the XSS-payload case'
        );
        expect(titleVisible).toBe(true);
      }
    } else {
      test.fail(
        true,
        "The Create Quiz control was not reachable this run (Planning mode's Question Bank sidebar did not surface it) -- could not exercise the XSS-payload case"
      );
      expect(createQuizVisible).toBe(true);
    }
    await cmp.switchToTeachingMode();
