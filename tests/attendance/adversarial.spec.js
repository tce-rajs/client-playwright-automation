...
test(
  "ATT-BREAK-01: blocking the Attendance micro-frontend's own network requests shows a distinguishable error, not an indistinguishable-from-normal infinite spinner",
  { tag: ['@negative', '@bug'] },
  async ({ page }) => {
    ...
    const anyErrorTextVisible = await page
      .locator('text=/error|failed|try again|something went wrong/i')
      .first()
    ...
    test.fail(
      spinnerShowing && !anyErrorTextVisible,
      'A fully-blocked network request for Attendance produces the exact same silent infinite spinner as a real backend problem, with no distinguishable error/retry affordance'
    );
...
test(
  "ATT-BREAK-02: switching Class while Attendance is still on its loading spinner does not leave a stuck overlay/spinner over the new class's whiteboard",
  { tag: ['@ui-state', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const att = new AttendancePage(page);
    ...
    await nav
      ...
      .catch((e) =>
        console.log('resetToClass threw (may itself be blocked by an open overlay):', e.message.split('\n')[0])
      );
    ...
    test.fail(
      leftoverOverlay,
      "Switching class while Attendance is still loading leaves its container/spinner stuck on top of the new class's whiteboard instead of cleanly unmounting"
    );
...
test(
  'ATT-BREAK-03: refreshing the page while Attendance is stuck on its loading spinner recovers to a single clean state, not a duplicated/worse one',
  ...
  async ({ page }) => {
    ...
    test.fail(
      ...
      'Refreshing while Attendance is stuck loading leaves a duplicated container or an unusable page instead of a single clean reload'
    );
...
test(
  'ATT-BREAK-04: opening the Magnet submenu, closing it, and reopening it 6 times in rapid alternation never leaves the submenu stuck open or duplicated',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const att = new AttendancePage(page);
    ...
    test.fail(
      ...
      'Rapidly toggling the Magnet tool 6 times mounts more than one instance of its own submenu items'
    );
...
test(
  'ATT-BREAK-05: an extremely small viewport (375px mobile width) does not let the stuck loading spinner overflow or break the surrounding layout',
  { tag: ['@boundary', '@bug'] },
  async ({ page }) => {
    test.setTimeout(45000);
    const att = new AttendancePage(page);
    ...
    test.fail(
      overflowsViewport,
      'The Attendance panel (even in its stuck-loading state) causes horizontal overflow at a real mobile viewport width'
    );
