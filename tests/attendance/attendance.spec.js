...
test.beforeEach(async ({ page }) => {
  ...
  await page
    .locator('[data-qa-id="common-select-subject-btn"]')
    .filter({ hasText: /^\s*Physics\s*$/ })
    .click();
...
test.describe('Core (ATT-ACCESS-01, ATT-PLAN-01..08)', () => {
  test(
    'ATT-ENTRY-01: Attendance is reachable via the Magnet tool (corrects the earlier "entry point not located" finding)',
    { tag: '@positive' },
    async ({ page }) => {
  ...
  test(
    'ATT-ACCESS-01: Opening Attendance eventually shows real content, not an indefinite loading spinner',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        !contentAppeared,
        'Attendance never progresses past its loading spinner -- confirmed reproducible across 2 subjects (Physics, Mathematics) in Class 12A, waited up to 50s in manual exploration, no JS error thrown'
      );
  ...
  test(
    'ATT-PLAN-01: Opening Attendance shows the class roster for marking',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
  ...
  test('ATT-PLAN-02: An empty class roster is handled gracefully', { tag: ['@negative', '@bug'] }, async ({ page }) => {
    ...
    if (!contentAppeared) {
      test.fail(
        true,
        'Blocked by ATT-ACCESS-01 (stuck loading spinner) -- cannot reach any roster, empty or otherwise'
      );
      expect(contentAppeared).toBe(true);
      return;
    }
  ...
  test(
    'ATT-PLAN-03: Refreshing before Submit does not silently lose marked attendance',
    ...
    async ({ page }) => {
      ...
      if (!contentAppeared) {
        test.fail(
          true,
          'Blocked by ATT-ACCESS-01 (stuck loading spinner) -- cannot mark anything to test refresh-persistence against'
        );
        expect(contentAppeared).toBe(true);
        return;
      }
      ...
      await page.waitForTimeout(1500);
      const classAfter = await att.gridCells
        .first()
        .getAttribute('class')
        .catch(() => null);
      const warnedInstead = await page
  ...
  test(
    'ATT-PLAN-04: Navigating away (class switch) before Submit does not silently lose marked attendance',
    { tag: ['@state-persistence', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const nav = new NavigationPage(page);
      ...
      if (!contentAppeared) {
        test.fail(
          true,
          'Blocked by ATT-ACCESS-01 (stuck loading spinner) -- cannot mark anything to test navigate-away-persistence against'
        );
        expect(contentAppeared).toBe(true);
        return;
      }
  ...
  test(
    'ATT-PLAN-05: Closing Attendance with unsaved marks triggers a confirmation (or auto-saves)',
    { tag: ['@ui-state', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      if (!contentAppeared) {
        test.fail(
          true,
          'Blocked by ATT-ACCESS-01 (stuck loading spinner) -- cannot mark anything to test close-confirmation against'
        );
        expect(contentAppeared).toBe(true);
        return;
      }
  ...
  test(
    'ATT-PLAN-06: Submitting attendance persists correctly and is reflected on reopen',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        true,
        'This account is mark-only and cannot Submit attendance (confirmed via the reference project); Submit-flow coverage was deliberately deferred pending a decision on which account to use for it'
      );
  ...
  test(
    'ATT-PLAN-07: Submitting attendance twice (double-click) does not create duplicate records',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      test.fail(
        true,
        'Same limitation as ATT-PLAN-06 -- this account cannot Submit attendance to test double-submission against'
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'ATT-PLAN-08: Attendance data is scoped to the correct class and never leaks to/from another class',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const nav = new NavigationPage(page);
      ...
      if (!contentAppeared) {
        test.fail(
          true,
          'Blocked by ATT-ACCESS-01 (stuck loading spinner) -- cannot reach a roster under either class to compare'
        );
        expect(contentAppeared).toBe(true);
        return;
      }
...
test.describe('Extended coverage (gap-analysis pass)', () => {
  test(
    'ATT-PANEL-01: CRITICAL -- the Attendance panel hangs indefinitely on a loading spinner with no way to exit',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        !contentAppeared,
        'CRITICAL, confirmed reproducible: the Attendance panel hangs on its loading spinner indefinitely, with no way to exit back to the whiteboard from inside the panel'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    'ATT-ACCESS-02: The Magnet gate is per-account, not per-class (regression re-check on this account)',
    { tag: '@cross-cutting' },
    async ({ page }) => {
      const att = new AttendancePage(page);
      await att.openMagnetSubmenu();
  ...
  test(
    'ATT-SEL-01: The confirmed plain-CSS selector map is reachable at least at the container level',
    { tag: '@ui-state' },
    async ({ page }) => {
      const att = new AttendancePage(page);
      await att.open();
  ...
  test(
    'ATT-BUS-01: Marking a student creates a real, persistent draft for today',
    ...
    async ({ page }) => {
      ...
      test.fail(
        !contentAppeared,
        'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach the roster to mark anything'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    'ATT-BUS-02: A tap toggles present<->absent only, with no way back to unmarked',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach the roster to tap a student cell'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    'ATT-BUS-03: Once submitted, buttons swap to Edit/Submit (locked view) and Edit re-seeds previous marks',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach Submit to test the locked/Edit state'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test('ATT-BUG-01: Close discards in-progress unsaved marks', { tag: ['@negative', '@bug'] }, async ({ page }) => {
    ...
    test.fail(
      !contentAppeared,
      'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot mark anything to test Close-discard behavior against'
    );
    expect(contentAppeared).toBe(true);
  });

  test(
    'ATT-CLOSE-01: No Close control exists while the panel is stuck loading',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      await att.open();
  ...
  test(
    'ATT-DBL-01: Submitting attendance twice in quick succession does not create duplicate records',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach Submit to test double-submission against'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    'ATT-SEC-01: Attendance data is scoped to the correct class and never leaks to/from another class',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach a roster under either class to compare'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    'ATT-SEC-02: A teacher cannot submit attendance for a class they are not assigned to teach',
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach Submit at all on any class to test authorization against'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    'ATT-QUIRK-01: A tap on a roll-number cell occasionally does not register',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach the roster to tap a cell'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    "ATT-GAP-01: Submit's real network request is unreachable to network-interception tooling (confirmed tooling gap, not a defect)",
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      ...
      test.fail(
        true,
        "Confirmed cross-repo: the attendance micro-frontend's real Submit call uses a postMessage-style internal event bus, not a normal fetch/XHR Playwright/Cypress network interception can capture -- this blocks any network-failure-simulation test for Submit specifically"
      );
      expect(true).toBe(false);
    }
  );
  ...
  test(
    'ATT-SUMM-01: The Total row in the summary table shows only ONE number, not a Present/Absent/Total triple',
    { tag: ['@negative', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach the summary table to inspect its Total row'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    "ATT-BDAY-01: A student's birthday triggers a birthday popup during Play Attendance",
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        "Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach Play Attendance's calling-marks flow"
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    "ATT-PASTDATE-01: Viewing a past date's attendance is view-only and cannot be re-edited",
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        "Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach the Play screen's date-nav controls"
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    'ATT-DRAG-01: Drag-to-mark-all-present gesture works as an alternative to individually tapping',
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach the roster to perform the drag gesture'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    "ATT-PLAY-01: Play Attendance's own calling-marks carousel functions correctly",
    { tag: ['@positive', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        "Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach Play Attendance's .btn-start control"
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    'ATT-EXP-01: The panel\'s behavior on a class where attendance was ALREADY marked today (not "Pending")',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const nav = new NavigationPage(page);
      ...
      const badgeText =
        (await page
          .locator('text=/Pending|Submitted|Marked/i')
          .first()
          .textContent()
          .catch(() => '')) || '';
      test.info().annotations.push({
      ...
      test.fail(
        !foundNonPending,
        'Every class/subject combination tried this pass still shows a "Pending" Attendance badge -- no already-marked-today state was found to test against'
      );
  ...
  test(
    'ATT-EXP-02: A large-roster class (30+ students) does not degrade Attendance panel performance',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach any roster to measure scroll/mark performance against, regardless of class size'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    'ATT-EXP-03: Rapidly double-clicking Attendance in the Magnet menu does not open two conflicting loading attempts',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      await att.openMagnetSubmenu();
  ...
  test(
    'ATT-EXP-04: Marking every student Absent (all-zero-present boundary) is accepted and reflected correctly',
    { tag: ['@boundary', '@bug'] },
    async ({ page }) => {
      const att = new AttendancePage(page);
      const contentAppeared = await openAndWaitForContent(att);
      test.fail(
        !contentAppeared,
        'Blocked by ATT-PANEL-01 (stuck loading spinner) -- cannot reach the roster to mark every student Absent'
      );
      expect(contentAppeared).toBe(true);
    }
  );
  ...
  test(
    "ATT-EXP-05: A teacher cannot submit attendance for a class by directly manipulating the request's class ID",
    { tag: ['@security', '@bug'] },
    async ({ page }) => {
      ...
      test.fail(
        true,
        "Blocked by both the same forging-tooling gap as NAV-SEC-01/EXP-06, and ATT-GAP-01's own confirmed finding that this endpoint is not a normal interceptable fetch/XHR request"
      );
      expect(true).toBe(false);
    }
  );
